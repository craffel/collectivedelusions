...
- **`--qos=low`**: lower scheduling priority, but **no usage cap**. Use this for hyperparameter sweeps, ablations, baselines, multi-seed runs.
...
1. **Restore State:** Your conversational memory is wiped between invocations. **You MUST begin every invocation by reading `progress.md`** to understand what has already been accomplished, which jobs you have submitted, and what your next step should be. **Pay special attention to the most recent entries: if you were sent back to Phase 2 from Phase 4, the requested empirical fixes and ablation studies will be documented there.**
...
- **Input Validation:** Check if `revision_plan.md` and `mock_review.md` exist in your workspace.
  - **If they DO exist (Empirical Pivot):** Read them carefully. Your goal is NOT to re-run the original experiments. You must implement the specific code changes, ablations, or baseline additions requested in the `revision_plan.md` to address the mock reviewer's concerns.
  - **If they DO NOT exist (First Pass):** Read `final_idea.md` to understand the hypothesis and methodology.
- **Formulation:** Design experiments to test the hypothesis or address the required revisions. Methodology must reflect `persona.md`.
- **Implementation:** 
  1. `git clone <repository_url>` a robust, existing codebase identified in Phase 1 (skip if already cloned).
  2. Review the `README.md` and data loading scripts.
  3. Set up the local `uv` environment using the repository's requirements.
  4. Write a patch or modify the core model files to inject your novel hypothesis or the requested ablations. Avoid writing training loops from scratch.
...
- **Handoff Artifact:** You MUST create `experiment_results.md` detailing metrics, links to generated plots, and ablation tables. **If you are pivoting, append your new results and ablation tables to the existing `experiment_results.md` (do not delete the old results).**
- **State Management:** When finished with Phase 2 and `experiment_results.md` is updated, update the `progress.json` file to set `{"phase": 3}`.
