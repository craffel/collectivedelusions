...
def main():
    print("Starting Experimental Sweep...")

    # Configurations to sweep
    # 1. Weight Decay sweep (epochs = 5)
    wd_sweep = [0.0, 1e-4, 1e-2, 1e-1]
    # 2. Epochs sweep (wd = 1e-2)
    epoch_sweep = [1, 20] # 5 is covered in the wd sweep

    datasets = ["mnist", "fmnist", "cifar10"]
    configs = []

    # Assemble all unique configurations
    for wd in wd_sweep:
        configs.append({"epochs": 5, "wd": wd})
    for epochs in epoch_sweep:
        configs.append({"epochs": epochs, "wd": 1e-2})

    print(f"Total configurations to run: {len(configs)}")
    for i, cfg in enumerate(configs):
        print(f"  Config {i+1}: Epochs={cfg['epochs']}, Weight Decay={cfg['wd']}")

    # Step 1: Train all experts
    print("\n=== STEP 1: TRAINING EXPERT MODELS ===")
    for cfg in configs:
        epochs = cfg["epochs"]
        wd = cfg["wd"]
        for ds in datasets:
            cmd = f"python train.py --dataset {ds} --epochs {epochs} --weight_decay {wd} --seed 42"
            run_cmd(cmd)

    # Step 2: Merge and evaluate
    print("\n=== STEP 2: MERGING AND EVALUATING MODELS ===")
    ...
    print("\n=== STEP 3: COMPILING RESULTS SUMMARY ===")
    summary = []
    for cfg in configs:
        epochs = cfg["epochs"]
        wd = cfg["wd"]
        for bn in bn_modes:
            ...
            if os.path.exists(results_path):
                ...
                # Extract merging method accuracies
                res = data["results"]
    ...
    # Print summary as a markdown table
