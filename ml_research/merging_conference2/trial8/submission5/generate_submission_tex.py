...
\begin{itemize}
    ...
    \item We identify a critical boundary for zero-shot PTQ model merging: per-tensor uniform 4-bit quantization causes a complete collapse of all methods, establishing the limits of zero-shot deployment.
\end{itemize}
...
\end{equation}
where $\gamma$ is a hyperparameter controlling the tightness of outlier rejection (we set $\gamma = 1.5$ as a robust default), and we cap the upper bound at $4.0$ (instead of $10.0$) to prevent extreme dynamic range inflation under quantization.
...
By dynamically adjusting the clamping boundaries based on layer-wise MAD, QR-IPR effectively suppresses unstable outlier scale factors, maintaining a compact dynamic range of parameters and drastically reducing quantization noise while retaining the full accuracy benefits of representation-scale restoration.
...
\begin{figure}[ht]
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{quantization_robustness.png}}
...
\begin{figure}[ht]
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{noise_robustness.png}}
...
Based on our empirical findings, we offer the following recommendations for deploying merged models in the wild:
\begin{itemize}
