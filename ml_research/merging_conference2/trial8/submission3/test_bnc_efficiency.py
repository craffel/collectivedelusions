...
def calibrate_batchnorm_custom(backbone, heads, train_loaders, device, num_samples_per_task=64, batch_size=32, epochs=20, momentum=0.1):
    """Calibrates the BatchNorm statistics with custom parameters to study convergence."""
...
def main():
    ...
    # Let's sweep epochs and momentum for size=64
    configs = [
        {"epochs": 5, "momentum": 0.1},
        {"epochs": 10, "momentum": 0.1},
        {"epochs": 20, "momentum": 0.1},
        {"epochs": 40, "momentum": 0.1},
        {"epochs": 20, "momentum": 0.05},
        {"epochs": 40, "momentum": 0.05},
        {"epochs": 100, "momentum": 0.01},
    ]
    
    print("\nEvaluating JBC efficiency with only 64 samples per task:")
    print(f"{'Epochs':<8} | {'Momentum':<10} | {'MNIST':<8} | {'F-MNIST':<8} | {'CIFAR-10':<8} | {'Average':<8}")
    print("-"*60)
    ...
    for config in configs:
        ...
        print(f"{config['epochs']:<8} | {config['momentum']:<10.2f} | {accs['mnist']:<8.2f}% | {accs['fmnist']:<8.2f}% | {accs['cifar10']:<8.2f}% | {avg:<8.2f}%")
