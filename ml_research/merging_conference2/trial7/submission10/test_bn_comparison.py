...
def standard_bn_calibration(model, calibration_loaders, num_batches=100):
    model.train()
...
# Get averaged BN stats
avg_bn_vars = {}
for name, module in m_model.named_modules():
    if isinstance(module, nn.BatchNorm2d):
        avg_bn_vars[name] = module.running_var.clone()

# Get calibrated BN stats
calib_loaders = [
...
calib_bn_vars = {}
for name, module in m_model_calib.named_modules():
    if isinstance(module, nn.BatchNorm2d):
        calib_bn_vars[name] = module.running_var.clone()

# Print comparison
print(f"{'BatchNorm Layer':<20} | {'Mean Calibrated Var':<20} | {'Mean Averaged Var':<20} | {'Mean FNBC Var':<20} | {'Ratio Calib/Avg':<20} | {'Ratio FNBC (R_l)':<20}")
print("-" * 130)
for name in avg_bn_vars:
    calib_mean = calib_bn_vars[name].mean().item()
    avg_mean = avg_bn_vars[name].mean().item()
    R_l = ratios[name].item()
    fnbc_mean = avg_mean * R_l
    print(f"{name:<20} | {calib_mean:<20.6f} | {avg_mean:<20.6f} | {fnbc_mean:<20.6f} | {calib_mean/avg_mean:<20.6f} | {R_l:<20.6f}")
