...
def standard_bn_calibration(model, calibration_loaders, num_batches=100):
    model.train()
    # Reset running stats of BatchNorm layers
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.reset_running_stats()
            m.momentum = None # Calculate cumulative average
            
    # Combine calibration loaders
    # We will pass batches from each calibration loader
    with torch.no_grad():
...
print("=== Evaluating WA with Standard BN Calibration ===")
...
for i, (loader, exp) in enumerate(zip(dataloaders, experts)):
    ...
    accs_calib.append(acc)
print(f"Standard BN Calib: MNIST: {accs_calib[0]:.4%} | FMNIST: {accs_calib[1]:.4%} | CIFAR-10: {accs_calib[2]:.4%} | Avg: {sum(accs_calib)/3:.4%}")
