...
def apply_hns_tv(merged_model, expert_model, progenitor_model):
...
def apply_hns_weight(merged_model, expert_model):
    # Holographic Norm Scaling (HNS) in parameter space applied to weights directly
    merged_state = merged_model.state_dict()
    expert_state = expert_model.state_dict()
    ...
    with torch.no_grad():
        for key in merged_state.keys():
            ...
            if len(param_m.shape) >= 2:
                ...
                dim = tuple(range(1, len(param_m.shape)))
                norm_e = torch.norm(param_e_dev, p=2, dim=dim, keepdim=True)
                norm_m = torch.norm(param_m_float, p=2, dim=dim, keepdim=True)
                
                scale = norm_e / (norm_m + 1e-8)
                scale = torch.clamp(scale, min=0.1, max=10.0)
                
                param_m_scaled = param_m_float * scale.view(-1, *([1]*(len(param_m.shape)-1)))
                param_m.copy_(param_m_scaled.to(param_m.dtype))
...
def main():
    ...
    print("\n--- Evaluating HNS-TV (Task Vector Scaling) ---")
    hns_tv_accs = {}
    for task in tasks:
        temp_model = copy.deepcopy(wa_model)
        apply_hns_tv(temp_model, experts[task], progenitor)
        copy_bn_and_fc(temp_model, experts[task])
        _, acc = evaluate_model(temp_model, loaders[task]['test'])
        hns_tv_accs[task] = acc
        print(f"HNS-TV Accuracy on {task.upper()}: {acc:.2f}%")
    print(f"Average HNS-TV Accuracy: {sum(hns_tv_accs.values())/3:.2f}%")
    
    print("\n--- Evaluating HNS-W (Direct Weight Scaling) ---")
    hns_w_accs = {}
    for task in tasks:
        temp_model = copy.deepcopy(wa_model)
        apply_hns_weight(temp_model, experts[task])
        copy_bn_and_fc(temp_model, experts[task])
        _, acc = evaluate_model(temp_model, loaders[task]['test'])
        hns_w_accs[task] = acc
        print(f"HNS-W Accuracy on {task.upper()}: {acc:.2f}%")
    print(f"Average HNS-W Accuracy: {sum(hns_w_accs.values())/3:.2f}%")
