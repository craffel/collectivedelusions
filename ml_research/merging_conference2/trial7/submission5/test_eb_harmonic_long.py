...
def calibrate_running_stats_eb_long(model, expert_state_dicts, task_name, calibration_data, epochs=100, momentum=0.1, device='cpu', alpha=1.0, reset=True):
    model.train()
    ...
    # Post-processing: expert-bounding
    ...
    with torch.no_grad():
        for name, m in model.named_modules():
            if isinstance(m, nn.BatchNorm2d):
                ...
                # Mean bounding for quiet channels
...
def main():
    ...
    # 1. EB-HarmonicCalib (100 epochs, reset=True)
    print("\n--- EB-HarmonicCalib (100 epochs, reset=True, alpha=1.0) ---")
    ts_results_100 = {}
    for task in task_names:
        ...
        calibrate_running_stats_eb_long(model, expert_state_dicts, task, harmonic_data, epochs=100, momentum=0.1, device=device, alpha=1.0, reset=True)
        
        ts_results_100[task] = evaluate_on_task(model, test_datasets_dict[task], task, device)
        print(f"  Task {task.upper()}: {ts_results_100[task]:.2f}%")
    print(f"  Average: {sum(ts_results_100.values())/3:.2f}%")
    
    # 2. EB-HarmonicCalib (30 epochs, reset=False)
    print("\n--- EB-HarmonicCalib (30 epochs, reset=False, momentum=0.2, alpha=1.0) ---")
    ts_results_no_reset = {}
    for task in task_names:
        model = MultiTaskResNet18().to(device)
        model.load_state_dict(merged_backbone_state, strict=False)
        ...
        harmonic_data = generate_harmonic_patterns(num_samples=256, img_size=32, device=device)
        calibrate_running_stats_eb_long(model, expert_state_dicts, task, harmonic_data, epochs=30, momentum=0.2, device=device, alpha=1.0, reset=False)
        
        ts_results_no_reset[task] = evaluate_on_task(model, test_datasets_dict[task], task, device)
        print(f"  Task {task.upper()}: {ts_results_no_reset[task]:.2f}%")
    print(f"  Average: {sum(ts_results_no_reset.values())/3:.2f}%")
