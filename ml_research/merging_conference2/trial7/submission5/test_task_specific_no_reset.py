...
from run_experiments import MultiTaskResNet18, get_datasets, merge_expert_models, evaluate_merged_model
...
def calibrate_running_stats_no_reset(model, calibration_data, epochs=10, device='cpu'):
    ...
    # 1. DO NOT reset, but set momentum for rolling EMA updates
...
def main():
    ...
    print("\n--- Testing Task-Specific Calibration WITHOUT Resetting Stats ---")
    
    task_specific_results = {}
    for task in task_names:
        print(f"\nCalibrating specifically for task: {task.upper()}")
        
        # Load merged weights and merged stats
        ...
        # Calibrate with 256 real samples of THIS task
        ...
        print(f"Task {task.upper()} accuracy after task-specific calibration (no reset): {acc:.2f}%")
        
    avg_acc = sum(task_specific_results.values()) / len(task_specific_results)
    print(f"\nTask-Specific Real Calibration Average Accuracy (no reset): {avg_acc:.2f}%")
