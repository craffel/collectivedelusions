...
def compute_cka(X, Y):
    ...
    if den == 0 or torch.isnan(num) or torch.isnan(den):
        return 0.0
    return (num / den).item()
...
def run_experiment_for_K(K, method="WA", lam=0.3):
    ...
    # Check for NaNs
    has_nan = any(np.isnan(val) for val in avg_cka.values())
    if has_nan:
        print("Warning: CKA contains NaNs due to activation explosion!")
        merged_extractor.clear()
        return None
...
if __name__ == "__main__":
    ...
    # 1. Evaluate Weight Averaging (WA)
    results["WA"] = []
    for K in K_values:
        res = run_experiment_for_K(K, method="WA", lam=0.3)
        results["WA"].append(res)
        
    # 1b. Evaluate Weight Averaging with BatchNorm Recalibration (WA_BN)
    results["WA_BN"] = []
    for K in K_values:
        res = run_experiment_for_K(K, method="WA_BN", lam=0.3)
        results["WA_BN"].append(res)
        
    # 2. Evaluate Task Arithmetic (TA) with Tuning
    results["TA"] = []
    lam_candidates = [0.01, 0.03, 0.05, 0.08, 0.1, 0.15, 0.2, 0.25, 0.3]
    
    for K in K_values:
        print(f"\n--- Tuning Task Arithmetic (TA) for K = {K} ---")
        best_res = None
        best_oracle_acc = -1.0
        best_lam = None
        
        for lam in lam_candidates:
            res = run_experiment_for_K(K, method="TA", lam=lam)
            if res is not None:
                if res["oracle_acc"] > best_oracle_acc:
                    best_oracle_acc = res["oracle_acc"]
                    best_res = res
                    best_lam = lam
                    
        if best_res is None:
            print(f"Error: All lam candidates failed for K = {K}! Using default lam = 0.01.")
            best_res = run_experiment_for_K(K, method="TA", lam=0.01)
            best_lam = 0.01
            
        print(f"--> Best lambda for K = {K}: {best_lam} (Oracle Acc: {best_oracle_acc:.2f}%)")
        results["TA"].append(best_res)
        
    # 2b. Evaluate Task Arithmetic with BatchNorm Recalibration (TA_BN) with Tuning
    results["TA_BN"] = []
    for K in K_values:
        print(f"\n--- Tuning Task Arithmetic with BN Calibration (TA_BN) for K = {K} ---")
        best_res = None
        best_oracle_acc = -1.0
        best_lam = None
        
        for lam in lam_candidates:
            res = run_experiment_for_K(K, method="TA_BN", lam=lam)
            if res is not None:
                if res["oracle_acc"] > best_oracle_acc:
                    best_oracle_acc = res["oracle_acc"]
                    best_res = res
        ...
        if best_res is None:
            print(f"Error: All lam candidates failed for K = {K}! Using default lam = 0.01.")
            best_res = run_experiment_for_K(K, method="TA_BN", lam=0.01)
            best_lam = 0.01
            
        print(f"--> Best lambda for K = {K}: {best_lam} (Oracle Acc: {best_oracle_acc:.2f}%)")
        results["TA_BN"].append(best_res)
        
    # Save tuned results as JSON
    with open("experiment_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    print("\nTuned and Calibrated experiments complete! Results saved to experiment_results.json")
