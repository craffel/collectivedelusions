...
def run_sft_exp(model, ta_state, expert_heads, calib_loaders, test_loaders, tasks, device, init_mode='expert'):
    ...
    for task in tasks:
        ...
        if init_mode == 'expert':
            ...
            nn.init.kaiming_normal_(head_copy.weight, nonlinearity='linear')
            nn.init.constant_(head_copy.bias, 0)
...
def main():
    ...
    seeds = [42, 100, 2026]
    results = {
        128: {'SFT_Expert': [], 'SFT_Random': [], 'TTA_Expert': []},
        512: {'SFT_Expert': [], 'SFT_Random': [], 'TTA_Expert': []}
    }

    print("\nStarting Multi-Seed Evaluation...")
    for seed in seeds:
        print(f"\nEvaluating Seed {seed}...")
        for N in [128, 512]:
            sub_train, sub_test, sub_calib = create_subsets(train_ds, test_ds, n_calib=N, seed=seed)
            ...
            # SFT Expert
            acc = run_sft_exp(model, ta_state, expert_heads, calib_loaders, test_loaders, tasks, device, init_mode='expert')
            results[N]['SFT_Expert'].append(acc)
            
            # SFT Random
            acc = run_sft_exp(model, ta_state, expert_heads, calib_loaders, test_loaders, tasks, device, init_mode='random')
            results[N]['SFT_Random'].append(acc)

            # TTA Expert
            acc = run_tta_exp(model, ta_state, expert_states, expert_heads, calib_loaders, test_loaders, tasks, device)
            results[N]['TTA_Expert'].append(acc)
            
            print(f"  N={N}: SFT_Expert={results[N]['SFT_Expert'][-1]:.2f}%, SFT_Random={results[N]['SFT_Random'][-1]:.2f}%, TTA_Expert={results[N]['TTA_Expert'][-1]:.2f}%")

    print("\n=== FINAL ROBUSTNESS SUMMARY ===")
    with open('multi_seed_results.txt', 'w') as f:
        for N in [128, 512]:
            header = f"\n--- Calibration Budget N = {N} ---"
            print(header)
            f.write(header + '\n')
            for key in ['SFT_Expert', 'SFT_Random', 'TTA_Expert']:
                acc_list = results[N][key]
                mean_acc = np.mean(acc_list)
                std_acc = np.std(acc_list)
                line = f"  {key:<12}: Mean = {mean_acc:.2f}%, Std = {std_acc:.2f}%, Values = {[f'{x:.2f}%' for x in acc_list]}"
                print(line)
                f.write(line + '\n')

    print("\nRobustness checks written to multi_seed_results.txt successfully!")
