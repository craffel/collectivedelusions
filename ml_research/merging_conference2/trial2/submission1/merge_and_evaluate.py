...
class ActivationExtractor:
    ...
    def register_hook(self):
        ...
        def hook_fn(module, input, output):
            # If output is a tuple (some layers might return a tuple), get first element
            if isinstance(output, tuple):
                output = output[0]
            # Flatten activations (B, C, H, W) -> (B, C * H * W)
...
def run_experiment(cal_size=128, target_layer="layer4", seed=42):
    print(f"\n==========================================")
    print(f"Running Experiment: Cal Size={cal_size} | Layer={target_layer} | Seed={seed}")
    print(f"==========================================\n")
    ...
    # 4. Sweep lambdas and compute activation distances and full test accuracies
    # Let's prepare full test loaders for Oracle evaluation
    test_loaders = {}
    for t in tasks:
        dataset = get_dataset(t, train=False)
        test_loaders[t] = DataLoader(dataset, batch_size=256, shuffle=False, num_workers=4)
        
    lambdas = np.linspace(0.0, 1.5, 31) # 0.0 to 1.5 with 0.05 step
    ...
    for lmbda in lambdas:
        ...
        # A. Compute activation distances
        ...
        # B. Compute actual test accuracy (Oracle)
        lmbda_accs = {}
        for t in tasks:
    ...
    # 5. Find optimal lambda for each metric (minimizing distance)
    opt_lambdas = {}
    opt_lambda_accs = {}
    for m in metrics:
    ...
    # 6. Find Oracle optimal lambda (maximizing actual average test accuracy)
    ...
    # Let's also evaluate the Weight Averaging baseline (WA)
    # WA is equivalent to task arithmetic where lmbda is some average or equivalent.
    # But wait, we can implement exact Weight Averaging where backbone weights are simply averaged:
    # theta_merged = (theta_1 + theta_2 + theta_3) / 3
    wa_bb = {}
    for k in base_backbone.keys():
    ...
    print(f"Weight Averaging Backbone Baseline - Average Test Accuracy: {wa_avg_acc:.2f}%")
...
if __name__ == "__main__":
    # We will run a quick validation or a full sweep depending on command args
    import sys
    if len(sys.argv) > 1:
        ...
        # Save specific results file
        filename = f"results_size{cal_size}_layer{target_layer}_seed{seed}.json"
        with open(filename, "w") as f:
            json.dump(results, f, indent=4)
        print(f"Saved results to {filename}")
    else:
        # Default runs
        # Run the standard configuration: 128 samples, layer4, seed 42
        results = run_experiment(128, "layer4", 42)
        with open("results_default.json", "w") as f:
            json.dump(results, f, indent=4)
        print("Saved default results to results_default.json")
