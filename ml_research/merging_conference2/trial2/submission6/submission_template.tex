...
\begin{abstract}
  ...
  Furthermore, we expose a critical methodological bug in prior hook-based baseline implementations of TCAC and REPAIR: they collect merged statistics simultaneously across all layers of an uncalibrated model, ignoring the sequential dependency of deep representations. By resolving this bug to establish a mathematically rigorous, sequential Offline BN Recalibration baseline, we raise baseline performance from random guessing ($\sim$11\%) to a highly competitive {{REPAIR_AVG}}\% average accuracy. Despite this, our proposed zero-shot TTBN still outperforms this corrected baseline (achieving {{TATTBN_AVG}}\% average accuracy) while completely eliminating the need for offline calibration sets, forward passes, and task-specific parameter storage. We urge the community to prioritize simple, dynamic test-time normalization over convoluted, offline activation-patching pipelines.
\end{abstract}
...
\midrule
Individual Experts (Upper Bound) & {{EXPERT_MNIST}}\% & {{EXPERT_FASHION}}\% & {{EXPERT_CIFAR}}\% & {{EXPERT_AVG}}\% \\
\midrule
Weight Averaging (WA) & {{WA_MNIST}}\% & {{WA_FASHION}}\% & {{WA_CIFAR}}\% & {{WA_AVG}}\% \\
WA + TTBN (Ours) & {{WATTBN_MNIST}}\% & {{WATTBN_FASHION}}\% & {{WATTBN_CIFAR}}\% & {{WATTBN_AVG}}\% \\
Task Arithmetic (TA) ($\lambda = {{BEST_TA_LAMBDA}}$) & {{TA_MNIST}}\% & {{TA_FASHION}}\% & {{TA_CIFAR}}\% & {{TA_AVG}}\% \\
\textbf{TA + TTBN (Ours, Proposed)} ($\lambda = {{BEST_TATTBN_LAMBDA}}$) & \textbf{{{TATTBN_MNIST}}\%} & \textbf{{{TATTBN_FASHION}}\%} & \textbf{{{TATTBN_CIFAR}}\%} & \textbf{{{TATTBN_AVG}}\%} \\
\midrule
REPAIR (Simultaneous, Broken) & {{TCAC_MNIST}}\% & {{TCAC_FASHION}}\% & {{TCAC_CIFAR}}\% & {{TCAC_AVG}}\% \\
TCAC (Simultaneous, Broken) & {{TCAC_MNIST}}\% & {{TCAC_FASHION}}\% & {{TCAC_CIFAR}}\% & {{TCAC_AVG}}\% \\
REPAIR (Offline Recalibration, Corrected Baseline) & {{REPAIR_MNIST}}\% & {{REPAIR_FASHION}}\% & {{REPAIR_CIFAR}}\% & {{REPAIR_AVG}}\% \\
\midrule
TSBM (Ours, Decoupled Baseline) ($\lambda = {{BEST_TSBM_LAMBDA}}$) & {{TSBM_MNIST}}\% & {{TSBM_FASHION}}\% & {{TSBM_CIFAR}}\% & {{TSBM_AVG}}\% \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
While Task-Specific BatchNorm Merging (TSBM) keeps the expert's unmerged BatchNorm parameters and running statistics, it fails to completely recover performance (achieving only {{TSBM_AVG}}\%). This is because the convolutional weights in the backbone are merged, which inevitably shrinks the magnitude of their features.
