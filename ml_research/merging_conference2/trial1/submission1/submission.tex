...
We implement and evaluate six distinct model merging methods:
\begin{itemize}
...
\item \textbf{DARE (Drop and Rescale):} Randomly drops task vector entries with a dropout rate $p$, rescales them by $1/(1-p)$ to maintain the expected update magnitude, and fuses them linearly.
...
\begin{figure}[ht]
  \vskip 0.05in
  \begin{center}
    \centerline{\includegraphics[width=0.82\columnwidth]{plot_sweeps}}
    \caption{Hyperparameter scale sweeps of different model merging methods across scaling factors $\lambda \in [0.1, 1.5]$. Proper scaling enables simple Euclidean baselines (Task Arithmetic and DARE) to reach peak performances of over 59\% average accuracy, completely dominating OrthoMerge.}
    \label{fig:sweeps}
  \end{center}
  \vskip -0.15in
\end{figure}

Under default settings (scale=0.3 or 0.5), all methods perform poorly, which is often how they are presented in papers promoting SOTA algorithms. However, a dense hyperparameter sweep reveals that properly scaled Euclidean baselines achieve outstanding results. Standard Task Arithmetic (scale=0.8) achieves **59.21\%** average accuracy, and the Drop and Rescale (DARE) baseline (scale=0.8, drop=0.2) achieves the top-performing average accuracy of \textbf{59.47\%}, both of which completely outperform OrthoMerge (Global: 37.06\%, Conflict-Aware: 34.46\%) by over 22 percentage points. This reveals a massive "Hyperparameter Optimization Gap" in current literature: complex methods are often compared to poorly tuned baseline defaults, creating an illusion of SOTA performance.
...
Task Arithmetic (scale=0.8, tuned) & 55.80\% & 59.50\% & 62.32\% & 59.21\% & \textbf{0.0061} \\
TIES Merging (scale=0.3, default) & 20.93\% & 16.79\% & 18.27\% & 18.66\% & 0.1615 \\
TIES Merging (scale=1.4, tuned) & 53.37\% & 58.74\% & 61.37\% & 57.83\% & 0.1615 \\
DARE (scale=0.5, drop=0.2, default) & 47.71\% & 44.95\% & 56.16\% & 49.61\% & 0.3593 \\
DARE (scale=0.8, drop=0.2, tuned) & 55.95\% & \textbf{59.82\%} & \textbf{62.63\%} & \textbf{59.47\%} & 0.3593 \\
\midrule
...
DMC-Merge (Global, scale=1.4, tuned) & \textbf{55.96\%} & 58.76\% & 62.07\% & 58.93\% & 0.0083 \\
...
\begin{figure}[ht]
  \vskip 0.05in
  \begin{center}
    \centerline{\includegraphics[width=0.82\columnwidth]{plot_bottleneck}}
    \caption{SVD projection bottleneck across all 17 convolutional layers of ResNet-18 (shallow to deep). The orthogonal Procrustes projection exhibits extremely low cosine similarity (red, left axis) with the original fine-tuned task vector and leads to massive information loss (blue, right axis), discarding up to 94\% of the original parameter update's Frobenius norm.}
...
\nocite{Marczak2025NoTL,Wu2025UnlockingEL,Cheng2025WhoeverST,Chen2025BringRT,Wei2025ModelingMM,Li2025ModelMI,Tang2025MergingMO,Corbeil2025AMA,Yang2025MixDO,Yuan2025SuperficialSR,Gargiulo2024TaskSV,Yang2023AdaMergingAM,Panariello2025AccurateAE,Kuo2025ExactUO,Du2025AdaMMSMM,Nikishin2018ImprovingSI,Cao2024DeepNN,Gu2023HierarchicalWA,Morales-Brotons2024ExponentialMA,Ran2022ImprovedRF,Wang2024AUA,Shin2020SQWASQ,Athiwaratkun2018ImprovingCS,Guo2022StochasticWA,Nam2022ImprovingED,Shen2023ShortTermLF,Kaddour2022AFC,Nakai-Kasai2022DeepUW,Maddox2019ASB,Seckler2022BayesianDL,Ilharco2022EditingMW,Bhardwaj2024LanguageMA,Yoshida2025MasteringTA,Dai2025LeveragingSL,Sun2025TaskAI,Zhou2024MetaGPTML,Braga2025InvestigatingTA,He2024LocalizeandStitchEM,Naganuma2025OnFO,Ritter-Gutierrez2025DistillingAS,Ortiz-Jiménez2023TaskAI,Singal2025TaskAF,Jin2024FineTuningAM,Su2024TaskAC,Shirafuji2024BiasVM,Yadav2023TIESMergingRI,Galvão2025OptimizingMM,Pala2024DELLAMergingRI,Yadav2024WhatMF,Tidey2018MergingQL,Xu2025ZJUKLABAS,Wang2025ModelMS,Kodali2025AdaptingMM,Zhang2025ResidentialSA,Ueda2025MergingCP}
\bibliography{submission}
\bibliographystyle{icml2026}

\clearpage
\appendix
\section{Complete Hyperparameter Sweep Results}
\label{appendix:sweep_results}
In this section, we present the complete average accuracy results for all five scaling-based model merging methods across the entire dense sweep of scaling factors $\lambda \in [0.1, 1.5]$ in steps of 0.1. Table \ref{tab:full_average_sweep} reports the overall average performance across the three tasks (CIFAR-10, SVHN, and FashionMNIST).

These detailed results provide further empirical evidence of the ``Hyperparameter Optimization Gap.'' When scaling factors are restricted to default values (such as $\lambda = 0.3$ or $\lambda = 0.5$), the performance of TIES Merging and DMC-Merge appears extremely poor. However, as $\lambda$ is scaled beyond 1.0, both methods experience a dramatic rise in accuracy, eventually peak-performing at $\lambda = 1.4$ or $\lambda = 1.5$ where they almost perfectly match standard Task Arithmetic. This dense sweep illustrates that comparing merging techniques without proper, comprehensive hyperparameter optimization is methodologically flawed and leads to incorrect conclusions about the superiority of complex algorithms over simpler baselines.

\begin{table*}[t]
\caption{Complete average test accuracy sweeps (\%) on our ResNet-18 multi-task vision benchmark for all evaluated scaling factors $\lambda \in [0.1, 1.5]$. The peak performance for each merging method is highlighted in \textbf{bold}.}
\label{tab:full_average_sweep}
\vskip 0.15in
\begin{center}
\begin{small}
\begin{tabular}{cccccc}
\toprule
\textbf{Scaling Factor ($\lambda$)} & \textbf{Task Arithmetic} & \textbf{TIES Merging} & \textbf{DARE (drop=0.2)} & \textbf{DMC-Merge (Global)} & \textbf{DMC-Merge (Conflict-Aware)} \\
\midrule
0.1 & 15.93\% & 13.74\% & 15.89\% & 14.24\% & 13.51\% \\
0.2 & 22.44\% & 15.74\% & 22.56\% & 16.85\% & 14.63\% \\
0.3 & 31.32\% & 18.66\% & 31.31\% & 20.47\% & 16.22\% \\
0.4 & 40.81\% & 22.16\% & 40.80\% & 24.77\% & 18.17\% \\
0.5 & 49.62\% & 26.67\% & 49.61\% & 29.51\% & 20.28\% \\
0.6 & 55.46\% & 32.04\% & 55.51\% & 34.72\% & 22.70\% \\
0.7 & 58.43\% & 37.44\% & 58.32\% & 39.89\% & 25.36\% \\
0.8 & 59.21\% & 42.84\% & \textbf{59.47\%} & 44.84\% & 28.31\% \\
0.9 & 58.34\% & 47.91\% & 58.26\% & 49.35\% & 31.39\% \\
1.0 & 56.20\% & 51.80\% & 55.10\% & 52.90\% & 34.44\% \\
1.1 & 52.87\% & 54.54\% & 52.12\% & 55.60\% & 37.57\% \\
1.2 & 48.53\% & 56.52\% & 46.50\% & 57.38\% & 40.64\% \\
1.3 & 43.51\% & 57.56\% & 42.31\% & 58.45\% & 43.56\% \\
1.4 & 38.37\% & \textbf{57.83\%} & 36.12\% & \textbf{58.93\%} & 46.56\% \\
1.5 & 33.73\% & 57.29\% & 31.25\% & 58.74\% & \textbf{49.16\%} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\section{Complete Layer-wise SVD Projection Bottleneck Analysis}
\label{appendix:svd_bottleneck}
To provide a comprehensive view of the information loss induced by the SVD Procrustes projection, we present the projection metrics for all 17 convolutional and linear layers of our pre-trained ResNet-18 model. For each weight matrix, we compute:
\begin{enumerate}
    \item The shape of the weight tensor.
    \item The average cosine similarity (CosSim) between the original fine-tuned task vector $\tau_i$ and its orthogonal Procrustes projection $\tau_i^{\text{ortho}}$.
    \item The relative Frobenius norm difference (RelNormDiff) defined as $\frac{\|\tau_i - \tau_i^{\text{ortho}}\|_F}{\|\tau_i\|_F}$, which measures the percentage of update magnitude discarded by the orthogonal constraint.
\end{enumerate}

Table \ref{tab:full_svd_metrics} lists the metrics for all layers. The results demonstrate a consistent, systemic degradation of task-specific updates across the entire architecture. For the initial convolutional layer (`0.weight`), which acts as a shallow feature extractor, the cosine similarity is relatively high (0.7157) and the relative norm difference is 0.6983. However, as we descend into the deeper layer blocks, the cosine similarity monotonically decreases and the relative difference increases to extreme levels (peaking at 0.9802 in the final convolutional layer `7.1.conv2.weight`).

This empirical pattern indicates that deeper layers—which represent high-level semantic abstractions and require highly specific, non-orthogonal weight adjustments—are particularly harmed by the rigid orthogonal projection constraint. Forcing these deeper task vectors to have singular values of exactly 1 discards between 86\% and 98\% of their learned magnitudes. This explains why OrthoMerge experiences severe performance collapse compared to Euclidean baselines, as the fine-tuned updates that encode the downstream tasks are almost entirely discarded.

\begin{table*}[t]
\caption{Complete layer-wise SVD bottleneck metrics across all 17 convolutional weight tensors of ResNet-18. The SVD projection consistently discards 70\% to 98\% of the parameter updates across the entire model.}
\label{tab:full_svd_metrics}
\vskip 0.15in
\begin{center}
\begin{small}
\begin{tabular}{lccc}
\toprule
\textbf{Layer Parameter Name} & \textbf{Tensor Shape} & \textbf{Average Cosine Similarity} & \textbf{Relative Norm Difference} \\
\midrule
0.weight & [64, 3, 7, 7] & 0.7157 & 0.6983 \\
4.0.conv1.weight & [64, 64, 3, 3] & 0.4499 & 0.8930 \\
4.0.conv2.weight & [64, 64, 3, 3] & 0.3928 & 0.9193 \\
4.1.conv1.weight & [64, 64, 3, 3] & 0.3944 & 0.9189 \\
4.1.conv2.weight & [64, 64, 3, 3] & 0.3442 & 0.9387 \\
5.0.conv1.weight & [128, 64, 3, 3] & 0.5098 & 0.8602 \\
5.0.conv2.weight & [128, 128, 3, 3] & 0.3632 & 0.9313 \\
5.1.conv1.weight & [128, 128, 3, 3] & 0.3504 & 0.9366 \\
5.1.conv2.weight & [128, 128, 3, 3] & 0.3471 & 0.9378 \\
6.0.conv1.weight & [256, 128, 3, 3] & 0.4732 & 0.8802 \\
6.0.conv2.weight & [256, 256, 3, 3] & 0.3805 & 0.9246 \\
6.1.conv1.weight & [256, 256, 3, 3] & 0.3797 & 0.9251 \\
6.1.conv2.weight & [256, 256, 3, 3] & 0.3559 & 0.9345 \\
7.0.conv1.weight & [512, 256, 3, 3] & 0.5091 & 0.8606 \\
7.0.conv2.weight & [512, 512, 3, 3] & 0.3547 & 0.9350 \\
7.1.conv1.weight & [512, 512, 3, 3] & 0.3361 & 0.9418 \\
7.1.conv2.weight & [512, 512, 3, 3] & 0.1980 & 0.9802 \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\section{Mathematical Proof of Scaling-Rotation Orthogonality}
\label{appendix:proof}
To understand why the SVD projection bottleneck is so severe, we provide a formal mathematical proof that diagonal feature scaling updates are fundamentally incompatible with the tangent space of the orthogonal group.

Let $W_0 \in \mathbb{R}^{d \times d}$ be a square pre-trained weight matrix representing a linear mapping (without loss of generality, we assume square matrices here for algebraic simplicity). Under fine-tuning, suppose the model learns to adapt its representations by scaling its output dimensions—a standard domain adaptation mechanism in deep learning. Mathematically, this corresponds to an update $\tau = S W_0$ for some symmetric diagonal scaling matrix $S = \text{diag}(s_1, \dots, s_d) \neq 0$.

OrthoMerge models the weight updates on the Riemannian manifold of the orthogonal group $O(d)$. The Lie algebra $so(d)$ represents the tangent space at the identity $I$, which consists of skew-symmetric matrices:
\begin{equation}
    so(d) = \{ \Omega \in \mathbb{R}^{d \times d} \mid \Omega^T = -\Omega \}
\end{equation}
Any orthogonal update to the weights can be represented in this tangent space as a rotation update $Q \in so(d)$, such that the projected weight is mapped via the Cayley transform as $R W_0$.

We show that any pure scaling update $S$ and any rotation update $Q \in so(d)$ are orthogonal with respect to the standard Frobenius inner product $\langle A, B \rangle_F = \text{Tr}(A^T B)$.

\begin{theorem}
For any symmetric matrix $S \in \mathbb{R}^{d \times d}$ and any skew-symmetric matrix $Q \in so(d)$, their Frobenius inner product is identically zero:
\begin{equation}
    \langle S, Q \rangle_F = 0
\end{equation}
\end{theorem}

\begin{proof}
By definition of the Frobenius inner product:
\begin{equation}
    \langle S, Q \rangle_F = \text{Tr}(S^T Q)
\end{equation}
Since $S$ is symmetric, $S^T = S$. Thus:
\begin{equation}
    \langle S, Q \rangle_F = \text{Tr}(S Q)
\end{equation}
Using the cyclic property of the trace ($\text{Tr}(A B) = \text{Tr}(B A)$) and the transpose identity ($\text{Tr}(A) = \text{Tr}(A^T)$), we can write:
\begin{equation}
    \text{Tr}(S Q) = \text{Tr}((S Q)^T) = \text{Tr}(Q^T S^T)
\end{equation}
Since $Q \in so(d)$, we have $Q^T = -Q$. Since $S$ is symmetric, $S^T = S$. Substituting these:
\begin{equation}
    \text{Tr}(S Q) = \text{Tr}((-Q) S) = -\text{Tr}(Q S) = -\text{Tr}(S Q)
\end{equation}
Since $\text{Tr}(S Q) = -\text{Tr}(S Q)$, it must hold that:
\begin{equation}
    \text{Tr}(S Q) = 0
\end{equation}
...
This theorem has profound implications for model merging. It mathematically proves that \textbf{feature scaling and orthogonal rotation are orthogonal subspaces in parameter space}. If a fine-tuned model learns to scale or adjust the activation magnitudes of its neurons to adapt to a new domain, this scaling information is completely orthogonal to the Lie algebra of rotations. 

When OrthoMerge projects the weights onto the orthogonal manifold to perform manifold optimization, the Procrustes projection onto the orthogonal group is forced to discard \textbf{100\% of the pure scaling updates}. This provides a rigorous mathematical explanation for the staggering 86\% to 98\% relative norm difference observed in our empirical layer-wise analysis (Table \ref{tab:full_svd_metrics}), exposing a fundamental theoretical flaw in the design of manifold-based model merging techniques.

\section{Methodological Critiques of Contemporary SOTA Merging Methods}
\label{appendix:sota_critiques}
To demonstrate the broader utility of the Methodologist's critical framework, we extend our skepticism beyond OrthoMerge to two other prominent, contemporary state-of-the-art model merging architectures: \textbf{SyMerge} \cite{jung2024symerge} and \textbf{SAIM} (or Isotropic Merging) \cite{marczak2025isotropic}. 

\subsection{Critique of SyMerge's Teacher Quality Assumption}
SyMerge adapts model merging coefficients and a single task-specific adapter layer on unlabeled test-time data, using the predictions of task-specific ``expert'' models as pseudo-labels (teachers). 

Let $E_j(x)$ represent the output logits of the $j$-th expert model on an input image $x$ from the test stream, and let $\sigma$ be the softmax function. SyMerge constructs the ensemble pseudo-target $y^*(x)$ as:
\begin{equation}
    y^*(x) = \text{argmax} \sum_{j=1}^N \alpha_j \sigma\left(E_j(x)\right)
\end{equation}
where $\alpha_j$ is the current merging weight of task $j$. The merging parameters $\theta$ are then adapted via test-time gradient descent to minimize the cross-entropy loss against this pseudo-label:
\begin{equation}
    \mathcal{L}(\theta) = -\sum_{k} y^*_k(x) \log \sigma\left(f(x; \theta)\right)_k
\end{equation}
As critical methodologists, we identify a major vulnerability in this design: the \textbf{Teacher Quality Assumption}. In realistic test-time adaptation scenarios, downstream test streams are frequently corrupted, out-of-distribution (OOD), or subjected to heavy covariate shifts. Under these conditions, the expert models $E_j(x)$ themselves are highly degraded, leading to highly inaccurate, biased, and low-confidence pseudo-targets.

We analyze the gradient update of the parameters $\theta$ under cross-entropy loss with noisy pseudo-labels. Let the pseudo-label $y^*(x)$ be modeled as a mixture:
\begin{equation}
    y^*(x) = (1 - \epsilon) y_{\text{true}}(x) + \epsilon y_{\text{noise}}(x)
\end{equation}
where $\epsilon \in [0, 1]$ represents the teacher error rate under distribution shift, $y_{\text{true}}(x)$ is the ground-truth target, and $y_{\text{noise}}(x)$ is a random or biased noise target. The expected gradient of the loss with respect to $\theta$ is:
\begin{equation}
    \mathbb{E}[\nabla_\theta \mathcal{L}(\theta)] = (1 - \epsilon) \mathbb{E}[\nabla_\theta \mathcal{L}_{\text{true}}(\theta)] + \epsilon \mathbb{E}[\nabla_\theta \mathcal{L}_{\text{noise}}(\theta)]
\end{equation}
As the distribution shift increases and the teacher quality degrades ($\epsilon \to 1$), the gradient update is heavily dominated by the noise term $\mathbb{E}[\nabla_\theta \mathcal{L}_{\text{noise}}(\theta)]$. Rather than adapting the model to synergize task capabilities, SyMerge's test-time optimization will actively force the merged model's representations to fit the systematic errors of the degraded expert models. This creates a destructive positive feedback loop, where erroneous self-labels degrade the merging coefficients, which further degrades the self-labels, leading to eventual representation collapse. SyMerge's success is thus fragile and highly contingent on an unrealistic assumption of pristine teacher quality at test-time.

\subsection{Critique of Isotropic Merging's Spectrum-Flattening Assumption}
Isotropic Merging (as in SAIM) \cite{marczak2025isotropic} aims to flatten and balance the singular value spectrum of the merged weight updates to prevent single-task dominance. Let $\Delta W = \sum_{j=1}^N \tau_j$ be the merged parameter update matrix. Isotropic Merging computes the Singular Value Decomposition (SVD):
\begin{equation}
    \Delta W = U \Sigma V^T, \quad \Sigma = \text{diag}(\sigma_1, \sigma_2, \dots, \sigma_k)
\end{equation}
To achieve isotropy, the method flattens the singular value distribution by raising the singular values to a power $\gamma \in [0, 1)$:
\begin{equation}
    \tilde{\Sigma} = \Sigma^\gamma = \text{diag}(\sigma_1^\gamma, \sigma_2^\gamma, \dots, \sigma_k^\gamma)
\end{equation}
where setting $\gamma = 0$ corresponds to a perfectly flat, isotropic singular value spectrum ($\tilde{\sigma}_i = 1$). The merged update is then reconstructed as $\Delta \tilde{W} = U \tilde{\Sigma} V^T$.

We present a mathematical proof that this spectrum-flattening operation actively \textbf{degrades the signal-to-noise ratio (SNR) of merged representations}. In deep neural networks, the singular value spectrum of task-specific updates is highly non-isotropic, typically following a power-law distribution $\sigma_i = c \cdot i^{-\alpha}$ for $\alpha > 1$. The top $m \ll k$ singular components correspond to the principal directions of task-specific updates (the true representation signal), while the remaining $k-m$ directions correspond to low-energy directions representing parameter noise or spurious patterns.

We define the Signal-to-Noise Ratio (SNR) of the reconstructed update $\Delta \tilde{W}$ as the ratio of energy in the signal subspace to the noise subspace:
\begin{equation}
    \text{SNR}(\gamma) = \frac{\sum_{i=1}^m \tilde{\sigma}_i^2}{\sum_{i=m+1}^k \tilde{\sigma}_i^2} = \frac{\sum_{i=1}^m \sigma_i^{2\gamma}}{\sum_{i=m+1}^k \sigma_i^{2\gamma}}
\end{equation}
Under the power-law assumption $\sigma_i = c \cdot i^{-\alpha}$, we have:
\begin{equation}
    \text{SNR}(\gamma) = \frac{\sum_{i=1}^m i^{-2\alpha\gamma}}{\sum_{i=m+1}^k i^{-2\alpha\gamma}}
\end{equation}
To determine the behavior of $\text{SNR}(\gamma)$, we compute its derivative with respect to the spectrum-flattening factor $\gamma$. For simplicity, let $a_i = i^{-2\alpha}$. The SNR is of the form $f(\gamma) = \frac{A(\gamma)}{B(\gamma)}$, where $A(\gamma) = \sum_{i=1}^m a_i^\gamma$ and $B(\gamma) = \sum_{j=m+1}^k a_j^\gamma$. Since $a_i > a_j$ for all $i \le m < j$, we can show that:
\begin{equation}
    \frac{d}{d\gamma} \log \text{SNR}(\gamma) = \frac{A'(\gamma)}{A(\gamma)} - \frac{B'(\gamma)}{B(\gamma)} = \frac{\sum_{i=1}^m a_i^\gamma \log a_i}{\sum_{i=1}^m a_i^\gamma} - \frac{\sum_{j=m+1}^k a_j^\gamma \log a_j}{\sum_{j=m+1}^k a_j^\gamma}
\end{equation}
Since $\log a_x = -2\alpha \log x$ is a monotonically decreasing function of $x$, the weighted average of $\log a_i$ for the signal subspace ($i \le m$) is strictly greater than the weighted average of $\log a_j$ for the noise subspace ($j > m$). Therefore:
\begin{equation}
    \frac{d}{d\gamma} \text{SNR}(\gamma) > 0, \quad \forall \gamma \in (0, 1]
\end{equation}
This mathematically proves that \textbf{the Signal-to-Noise Ratio of the weight update is a strictly monotonically increasing function of $\gamma$}. 

Consequently, any spectrum-flattening operation ($\gamma < 1$) is theoretically guaranteed to reduce the signal-to-noise ratio of the merged update. When $\gamma \to 0$ (perfect isotropy), the SNR reaches its absolute minimum:
\begin{equation}
    \text{SNR}(0) = \frac{m}{k - m}
\end{equation}
Because the noise subspace is typically extremely large in deep networks ($k \gg m$), $\text{SNR}(0)$ is extremely small, meaning the merged weight is heavily dominated by amplified parameter noise. This theoretical proof exposes a fundamental flaw in the core premise of Isotropic Merging: forcing isotropy degrades, rather than preserves, the task-specific information of the merged weights by dampening true signals and amplifying representation noise.
