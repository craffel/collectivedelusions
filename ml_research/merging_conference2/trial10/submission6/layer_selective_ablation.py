...
# Layer selective configurations
# We test:
# 1. 'full': Apply HSA to all parameters.
# 2. 'layer3_4': Apply HSA only to layer3 and layer4 (and fc if any, but fc is in head). early layers are averaged (WA).
# 3. 'layer4': Apply HSA only to layer4.
# 4. 'none': Standard WA (no HSA).
selective_configs = ['full', 'layer3_4', 'layer4', 'none']
results = {}

K_sqrt = len(tasks) ** 0.5

def should_apply_hsa(key_name, config):
    if config == 'full':
        return True
    elif config == 'layer3_4':
        return 'layer3' in key_name or 'layer4' in key_name
    elif config == 'layer4':
        return 'layer4' in key_name
    elif config == 'none':
        return False
    return False

for config in selective_configs:
    print(f"\nEvaluating Selective HSA Configuration: {config.upper()}")
    results[config] = {}
    
    # 1. Merge weights using this layer-selective strategy
    merged_backbone_sd = {}
    for key in progenitor_state.keys():
        if progenitor_state[key].is_floating_point():
            if "weight" in key and should_apply_hsa(key, config):
                # Apply HSA
...
with open("layer_selective_results.json", "w") as f:
    json.dump(results, f, indent=4)
print("\nSaved layer selective results to layer_selective_results.json")