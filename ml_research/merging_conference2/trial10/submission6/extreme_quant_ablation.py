...
# Construct standard model structures
# 1. WA (linear average)
print("Constructing WA backbone...")
wa_backbone_sd = {}
...
# 2. TA (Task Arithmetic lambda=0.3)
print("Constructing TA backbone...")
ta_backbone_sd = {}
lam = 0.3
for key in progenitor_state.keys():
    if progenitor_state[key].is_floating_point():
...
# 3. HSA (Dense, s=1.0)
print("Constructing HSA backbone...")
hsa_backbone_sd = {}
...
for key in progenitor_state.keys():
    if progenitor_state[key].is_floating_point():
        if "weight" in key:
            ...
            hsa_backbone_sd[key] = torch.stack([expert_backbones[t][key] for t in tasks]).mean(dim=0)
    else:
        hsa_backbone_sd[key] = progenitor_state[key].clone()

# 4. S-HSA (s=0.5)
print("Constructing S-HSA (s=0.5) backbone...")
shsa_backbone_sd = {}
for key in progenitor_state.keys():
    if progenitor_state[key].is_floating_point():
        if "weight" in key:
            ...
            for idx, t in enumerate(tasks):
                tau = expert_backbones[t][key] - progenitor_state[key]
                tau_sparse = prune_update(tau, 0.5)
                P = get_phase_key(key, progenitor_state[key].shape, idx, device)
                hsa_update += tau_sparse * P
            shsa_backbone_sd[key] = progenitor_state[key] + (hsa_update / K_sqrt)
        else:
            shsa_backbone_sd[key] = torch.stack([expert_backbones[t][key] for t in tasks]).mean(dim=0)
    else:
        shsa_backbone_sd[key] = progenitor_state[key].clone()


methods = {
    'WA + DE-BN': wa_backbone_sd,
    'TA + DE-BN': ta_backbone_sd,
    'HSA + DE-BN (Ours)': hsa_backbone_sd,
    'S-HSA (s=0.5) + DE-BN (Ours)': shsa_backbone_sd
}

results = {}

for num_bits in [3, 2]:
    bit_name = f'INT{num_bits}'
    print(f"\nEvaluating at {bit_name} per-channel PTQ...")
    results[bit_name] = {}
    
    for mname, mbackbone in methods.items():
        ...
        for idx, t in enumerate(tasks):
            ...
            if 'HSA' in mname:
                ...
                for key in progenitor_state.keys():
                    if progenitor_state[key].is_floating_point():
                        if "weight" in key:
                            hsa_update_tensor = mbackbone[key] - progenitor_state[key]
                            P = get_phase_key(key, progenitor_state[key].shape, idx, device)
                            retrieved_sd[key] = progenitor_state[key] + (K_sqrt * hsa_update_tensor * P)
                        else:
                            retrieved_sd[key] = mbackbone[key].clone()
                    else:
                        retrieved_sd[key] = mbackbone[key].clone()
            else:
                retrieved_sd = {k: v.clone() for k, v in mbackbone.items()}
                
            cal_backbone = calibrate_model_bn(retrieved_sd, expert_heads[t], t, num_bits, per_channel=True)
        ...
        results[bit_name][mname] = {
            'tasks': task_accs,
            'average': avg_acc
        }

with open("extreme_quant_results.json", "w") as f:
    json.dump(results, f, indent=4)
print("\nSaved extreme quantization results to extreme_quant_results.json")
