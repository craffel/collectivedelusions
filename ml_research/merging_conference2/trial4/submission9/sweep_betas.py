...
def main():
    ...
    betas = [20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 60.0, 75.0]
    
    print("\n==================== FINE-GRAINED SWEEPING OF BETAS AT LAYER 2 ====================")
    best_avg = 0.0
    best_config = None
    
    for beta in betas:
        avg_acc, accs = run_evaluation_with_anchor(n_taac_backbone, heads, subsets, sorted_tasks, 'layer2', beta)
        print(f"  Beta={beta:<5} | Avg: {avg_acc:.2f}% (MNIST: {accs['mnist']:.2f}%, F-MNIST: {accs['fmnist']:.2f}%, CIFAR: {accs['cifar']:.2f}%)")
        if avg_acc > best_avg:
            best_avg = avg_acc
            best_config = (beta, accs)
                
    print(f"\n=========================================================================")
    print(f"BEST CONFIGURATION:")
    print(f"  Beta: {best_config[0]}")
    print(f"  Best Average Accuracy: {best_avg:.2f}%")
    print(f"  MNIST: {best_config[1]['mnist']:.2f}%, F-MNIST: {best_config[1]['fmnist']:.2f}%, CIFAR: {best_config[1]['cifar']:.2f}%")
    print(f"=========================================================================")
