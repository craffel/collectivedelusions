...
# Prior-Regularized Least-Squares Head Alignment (PR-LSHA)
def apply_pr_lsha(backbone, experts_paths, task_loaders, reg=1.0):
    backbone.eval()
    aligned_heads = {}
    ...
    for task_name, loader in task_loaders.items():
        ...
        if reg == 0.0:
            # Under-regularized closed-form pseudo-inverse
            # (F_bias^T @ F_bias)^-1 F_bias^T @ (Y - F_bias @ W0.t())
            lhs = F_bias.t() @ F_bias + 1e-6 * I # absolute minimum for numerical stability
...
# Standard L2-Regularized Least-Squares Head Alignment (L2-LSHA)
def apply_l2_lsha(backbone, experts_paths, task_loaders, reg=1.0):
    backbone.eval()
    aligned_heads = {}
    ...
    for task_name, loader in task_loaders.items():
        ...
        I = torch.eye(F_bias.size(1), device=device)
        if reg == 0.0:
            lhs = F_bias.t() @ F_bias + 1e-6 * I
            rhs = F_bias.t() @ Y
            W_bias_sol = torch.linalg.solve(lhs, rhs)
        else:
            lhs = F_bias.t() @ F_bias + reg * I
            rhs = F_bias.t() @ Y
            W_bias_sol = torch.linalg.solve(lhs, rhs)
        
        W_new = W_bias_sol[:-1, :].t()
        b_new = W_bias_sol[-1, :]
...
if __name__ == '__main__':
    ...
    # We sweep budgets N in [16, 64, 256]
    # We sweep reg in [0.0, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0]
    # We sweep backbone: uncalibrated (WA), calibrated (WFC)
    # We sweep head: L2-LSHA (towards 0), PR-LSHA (towards W0)
    ...
    for N in budgets:
        ...
        # 1. Compute scaling factors and get WFC calibrated backbone
        ...
        for backbone_type, backbone_model in [('WA', wa_backbone), ('WFC', wfc_backbone)]:
            for head_type in ['L2-LSHA', 'PR-LSHA']:
                for reg in regs:
                    print(f"Running: {backbone_type} + {head_type} (N={N}, reg={reg})...")
                    
                    if head_type == 'PR-LSHA':
                        aligned_heads = apply_pr_lsha(backbone_model, experts_paths, task_loaders_cal, reg=reg)
                    else:
                        aligned_heads = apply_l2_lsha(backbone_model, experts_paths, task_loaders_cal, reg=reg)
                        
                    accs = evaluate_multi_task_aligned_heads(backbone_model, aligned_heads, test_loaders)
                    ...
                    print(f"Result -> Avg: {accs['avg']*100:.2f}% (MNIST: {accs['mnist']*100:.2f} | Fashion: {accs['fashion']*100:.2f} | CIFAR: {accs['cifar']*100:.2f})")
                    
    # Save results to json
    with open('ablation_results.json', 'w') as f:
        json.dump(results, f, indent=4)
        
    print("\nAblation sweep finished and results saved to ablation_results.json!")
