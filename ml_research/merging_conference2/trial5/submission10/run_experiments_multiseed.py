...
raw_data = {algo: {} for algo in merging_algorithms}

for algo in merging_algorithms:
    raw_data[algo]["uncalibrated_standalone"] = {t: [] for t in tasks + ["average"]}
    raw_data[algo]["uncalibrated_head_sft"] = {t: [] for t in tasks + ["average"]}
    for N in calibration_budgets:
        ...
        for cfg in configs:
            raw_data[algo][N][cfg] = {t: [] for t in tasks + ["average"]}

print(f"\nStarting Multi-seed Sweep over seeds {seeds}...")
...
for seed_idx, seed in enumerate(seeds):
    ...
    for algo in merging_algorithms:
        ...
        for t in tasks + ["average"]:
            raw_data[algo]["uncalibrated_standalone"][t].append(base_accs[t])
        ...
        for N in calibration_budgets:
            print(f"--- Seed {seed} | Algo {algo} | N = {N} ---")
            # Prepare calibration datasets for this seed and budget
            calib_datasets = {}
            ...
            if N == 64:
                ...
                for t in tasks + ["average"]:
                    raw_data[algo]["uncalibrated_head_sft"][t].append(sft_accs[t])
            ...
            for config in configs_to_run:
                ...
                for t in tasks + ["average"]:
                    raw_data[algo][N][f"{config}_standalone"][t].append(accs[t])
                print(f"  {config} Standalone: {accs['average']:.2f}%")
                    
                # 2. Calibration + Head SFT (only at N=64)
                if N == 64:
                    ...
                    for t in tasks + ["average"]:
                        raw_data[algo][N][f"{config}_head_sft"][t].append(sft_accs[t])
                    print(f"  {config} + Head SFT: {sft_accs['average']:.2f}%")

# Compile statistics (mean and std)
final_stats = {algo: {} for algo in merging_algorithms}

for algo in merging_algorithms:
    # 1. Uncalibrated Standalone
    final_stats[algo]["uncalibrated_standalone"] = {}
    for t in tasks + ["average"]:
        arr = np.array(raw_data[algo]["uncalibrated_standalone"][t])
        final_stats[algo]["uncalibrated_standalone"][t] = {
            "mean": float(np.mean(arr)),
            "std": float(np.std(arr))
        }
        
    # 2. Uncalibrated Head SFT
    final_stats[algo]["uncalibrated_head_sft"] = {}
    for t in tasks + ["average"]:
        arr = np.array(raw_data[algo]["uncalibrated_head_sft"][t])
        final_stats[algo]["uncalibrated_head_sft"][t] = {
            "mean": float(np.mean(arr)),
            "std": float(np.std(arr))
        }
    ...
    for N in calibration_budgets:
        final_stats[algo][N] = {}
        configs_to_process = ["SP-TAAC_standalone", "FDSA_standalone", "C-FDSA_standalone", "SMAC_standalone", "C-SMAC_standalone",
                               "SP-TAAC_head_sft", "FDSA_head_sft", "C-FDSA_head_sft", "SMAC_head_sft", "C-SMAC_head_sft"]
        for cfg in configs_to_process:
            final_stats[algo][N][cfg] = {}
            for t in tasks + ["average"]:
                arr = np.array(raw_data[algo][N][cfg][t])
                final_stats[algo][N][cfg][t] = {
                    "mean": float(np.mean(arr)),
                    "std": float(np.std(arr))
                }

# Save final aggregated statistics to json
with open("results/all_results_multiseed.json", "w") as f:
    json.dump(final_stats, f, indent=4)
print("\nSaved multi-seed results to results/all_results_multiseed.json")


# Format and print the exact LaTeX table code for easy insertion!
def fmt(stat_dict):
    m = stat_dict["mean"]
    s = stat_dict["std"]
    # If std is basically zero (e.g., uncalibrated standalone), we don't show std, or show small
    if s < 1e-4:
        return f"{m:.2f}"
    else:
...
print("\n" + "="*80)
print("PRINTING LATEX TABLE ROWS (Mean \pm Std across 5 Seeds)")
print("="*80)

for algo in merging_algorithms:
    print(f"\n% --- Merge Algorithm: {algo} ---")
    
    # Oracle Row
    oracle_line = f"Oracle Experts & Individual Task Models (Upper Bound) & {oracle_accs['mnist']:.2f} & {oracle_accs['fashionmnist']:.2f} & {oracle_accs['cifar10']:.2f} & {oracle_accs['average']:.2f} \\\\"
    print(oracle_line)
    print("\\midrule")
    
    # Uncalibrated baseline lines
    uncal_standalone_fmt = f"{algo} & Uncalibrated Baseline & {fmt(final_stats[algo]['uncalibrated_standalone']['mnist'])} & {fmt(final_stats[algo]['uncalibrated_standalone']['fashionmnist'])} & {fmt(final_stats[algo]['uncalibrated_standalone']['cifar10'])} & {fmt(final_stats[algo]['uncalibrated_standalone']['average'])} \\\\"
    uncal_sft_fmt = f"                 & Uncalibrated + Head SFT ($N = 64$) & {fmt(final_stats[algo]['uncalibrated_head_sft']['mnist'])} & {fmt(final_stats[algo]['uncalibrated_head_sft']['fashionmnist'])} & {fmt(final_stats[algo]['uncalibrated_head_sft']['cifar10'])} & {fmt(final_stats[algo]['uncalibrated_head_sft']['average'])} \\\\"
    print(uncal_standalone_fmt)
    print(uncal_sft_fmt)
    
    for N in calibration_budgets:
        print("\\cmidrule{2-6}")
        
        # N=16, 128 only have standalone
        if N in [16, 128]:
            methods_to_print = ["SP-TAAC_standalone", "FDSA_standalone", "C-FDSA_standalone", "SMAC_standalone", "C-SMAC_standalone"]
        else:
            # N=64 has both standalone and head_sft
            methods_to_print = [
                "SP-TAAC_standalone", "SP-TAAC_head_sft",
                "FDSA_standalone", "FDSA_head_sft",
                "C-FDSA_standalone", "C-FDSA_head_sft",
                "SMAC_standalone", "SMAC_head_sft",
                "C-SMAC_standalone", "C-SMAC_head_sft"
            ]
            
        for m_cfg in methods_to_print:
            # Format display name
            display_name = m_cfg.replace("_standalone", "").replace("_head_sft", " + Head SFT")
            # If our method, bold it
            if "SMAC" in display_name:
                if "C-SMAC" in display_name:
                    display_name_tex = display_name.replace("C-SMAC", "C-SMAC (Ours)")
                else:
                    display_name_tex = "\\textbf{" + display_name.replace("SMAC", "SMAC (Ours)") + "}"
            else:
                display_name_tex = display_name
                
            # If our standalone SMAC, bold the whole row values
            is_best = "SMAC" in m_cfg and "C-SMAC" not in m_cfg and "head_sft" not in m_cfg
            
            def cell(task_key):
                val_str = fmt(final_stats[algo][N][m_cfg][task_key])
                if is_best:
                    return "\\textbf{" + val_str + "}"
                else:
                    return val_str
                    
            line = f"                 & {display_name_tex} ($N = {N}$) & {cell('mnist')} & {cell('fashionmnist')} & {cell('cifar10')} & {cell('average')} \\\\"
            print(line)
