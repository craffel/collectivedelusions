...
# 1. Compute initial baseline entropies
print("\n--- Step 1: Computing Baseline Entropy for Each Task ---")
initial_entropies = {}
apply_merged_weights(image_encoder, {name: 0.3 for name in task_names})
image_encoder.eval()
with torch.no_grad():
    for name in task_names:
        ...
        initial_entropies[name] = entropy
        print(f"  Task {name} baseline entropy: {entropy:.4f}")

# 2. Optimize unweighted joint entropy
print("\n--- Step 2: Optimizing Unweighted Joint Entropy (Baseline) ---")
...
unweighted_coeffs = {}
for task_idx, t_name in enumerate(task_names):
    unweighted_coeffs[t_name] = task_coeffs_tensor[task_idx].detach().cpu().numpy().tolist()
unweighted_accs = evaluate_merged_model(image_encoder, unweighted_coeffs)
print(f"  Unweighted Average: {100*np.mean(list(unweighted_accs.values())):.2f}% | SVHN: {100*unweighted_accs['SVHN']:.2f}% | CIFAR10: {100*unweighted_accs['CIFAR10']:.2f}%")

# 3. Optimize weighted joint entropy (scale by 1 / H_init)
print("\n--- Step 3: Optimizing Scale-Normalized (Weighted) Joint Entropy ---")
task_coeffs_tensor = torch.full((len(task_names), 13), 0.3, dtype=torch.float32, device=device, requires_grad=True)
optimizer = torch.optim.Adam([task_coeffs_tensor], lr=1e-2)
...
for step in range(200):
    ...
    for task_idx, t_name in enumerate(task_names):
        ...
        probs = torch.softmax(logits, dim=-1)
        entropy = -(probs * torch.log(probs + 1e-8)).sum(dim=-1).mean()
        # Scale loss by inverse baseline entropy to normalize scales
        loss = loss + (1.0 / initial_entropies[t_name]) * entropy
...
weighted_coeffs = {}
for task_idx, t_name in enumerate(task_names):
    weighted_coeffs[t_name] = task_coeffs_tensor[task_idx].detach().cpu().numpy().tolist()
weighted_accs = evaluate_merged_model(image_encoder, weighted_coeffs)
print(f"  Weighted Average: {100*np.mean(list(weighted_accs.values())):.2f}% | SVHN: {100*weighted_accs['SVHN']:.2f}% | CIFAR10: {100*weighted_accs['CIFAR10']:.2f}%")

# Save comparison JSON
comparison = {
    'baseline_entropies': initial_entropies,
    'unweighted_results': unweighted_accs,
    'unweighted_average': float(np.mean(list(unweighted_accs.values()))),
    'weighted_results': weighted_accs,
    'weighted_average': float(np.mean(list(weighted_accs.values()))),
}
with open('results/weighted_entropy_comparison.json', 'w') as f:
    json.dump(comparison, f, indent=2)

print("\nWeighted Entropy Pilot complete!")
