...
# Sweeping Weight Decay values in Adam optimizer
wd_values = [0.0, 1e-4, 1e-3, 1e-2, 1e-1, 1.0]
wd_results = []

print("\n--- Sweeping Optimizer-Level Weight Decay (Adam) ---")
for wd in wd_values:
    print(f"Optimizing with Weight Decay = {wd}...")
    task_coeffs_tensor = torch.full((len(task_names), 13), 0.3, dtype=torch.float32, device=device, requires_grad=True)
    
    # We pass weight_decay directly to the optimizer
    optimizer = torch.optim.Adam([task_coeffs_tensor], lr=1e-2, weight_decay=wd)
    ...
    print(f"  Weight Decay {wd}: Avg Acc = {100*avg_acc:.2f}% | SVHN = {100*accs['SVHN']:.2f}% | CIFAR10 = {100*accs['CIFAR10']:.2f}%")
...
with open('results/weight_decay_comparison.json', 'w') as f:
    json.dump(wd_results, f, indent=2)

print("\nWeight Decay Pilot complete! Saved to results/weight_decay_comparison.json")
