...
def run_simulation_blocks(seed, num_samples=1000, block_len=50, D=192, K=4, L=14, L_frozen=3, gamma_V=0.05, sigma_layer=0.015):
    set_seed(seed)
    ...
    # Block Streaming Phase
    # Generate continuous task segments of length block_len
    num_blocks = num_samples // block_len
    stream_tasks = []
    for b in range(num_blocks):
        task = b % K
        stream_tasks.extend([task] * block_len)
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        res = run_simulation_blocks(seed, block_len=50)
        seed_data = {}
        for m in res:
    ...
    print("\nBLOCK-STRUCTURED STREAM JITTER RESULTS (BLOCK LEN = 50) ACROSS 10 SEEDS:")
