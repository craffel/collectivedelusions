...
| **RB-FTM (Ours, F=1)** | 70.90% | 96.43% | 4.0727 | Constrains coefficients to first-harmonic Fourier series with spectral Lasso. |
| **RB-FTM (Ours, F=2)** | 68.10% | 96.52% | 8.3555 | Constrains coefficients to second-harmonic Fourier series with spectral Lasso. |

### Table 2: CLIP ViT-B/16 Backbone Performance ($L=13$, $D=768$, $K=4$)
| Method | Categorical Acc. (%) | Soft Alignment Acc. (%) | Parameter Penalty/Norm | Description |
| :--- | :---: | :---: | :---: | :--- |
| **Static Uniform** | 84.55% | 66.85% | 0.0000 | Statically averages task expert parameters ($1/K=0.25$) across all layers. |
| **Globally-Scaled Task Arithmetic** | 79.90% | 75.84% | 0.0000 | Optimizes a single global scaling scalar per task across all layers ($d=0$). |
| **Offline Unconstrained** | 77.05% | 77.84% | 0.0000 | Optimizes unconstrained independent layer-wise coefficients directly. |
| **RBPM (d=2)** | 81.30% | 73.52% | 1.4262 | Constrains coefficients to a quadratic polynomial trajectory with Rademacher penalty. |
| **RB-FTM (Ours, F=1)** | 80.20% | 79.12% | 2.0167 | Constrains coefficients to first-harmonic Fourier series with spectral Lasso. |
| **RB-FTM (Ours, F=2)** | 82.30% | 78.39% | 3.6372 | Constrains coefficients to second-harmonic Fourier series with spectral Lasso. |
