...
class AIR(nn.Module):
    ...
    def forward(self, e_t, return_logits=False):
        ...
        for _ in range(self.N_steps):
            e_pred = F.linear(mu_t, W_matrix)
            pred_err = e_t - e_pred
            weighted_pred_err = Pi_e.unsqueeze(0) * pred_err
            grad_sensory = - F.linear(weighted_pred_err, W_matrix.t())
            
            prior_err = mu_t - mu_t_0
            grad_prior = Pi_s.unsqueeze(0) * prior_err
            
            grad = grad_sensory + grad_prior
            mu_t = mu_t - self.eta_test * grad
...
def extract_pca_bases(v_signatures, sigmas, d=4, num_samples=16):
...
def generate_stream(v_signatures, sigmas, stream_type="homogeneous", T=200, B=16):
    K, D = v_signatures.shape
...
def run_experiment_pipeline():
    ...
    # results_dict[config][stream][method] = {'acc': [], 'jitter': []}
    results_dict = {}
    for cfg in configs:
        results_dict[cfg] = {}
        for stm in streams:
            ...
            for mtd in methods_list:
                results_dict[cfg][stm][mtd] = {'acc': [], 'jitter': []}
    ...
    for cfg in configs:
        ...
        for seed in seeds:
            ...
            V_bases = extract_pca_bases(v_signatures, sigmas)
            
            # Generate calibration stream
            # Let's use a homogeneous-like block-structured sequence of size 32 for robust transitions
            cal_h3, cal_target_y = generate_stream(v_signatures, sigmas, stream_type="homogeneous", T=T_cal, B=B)
            
            # Generate test streams
            test_streams = {}
            for stm in streams:
                test_streams[stm] = generate_stream(v_signatures, sigmas, stream_type=stm, T=T_test, B=B)
