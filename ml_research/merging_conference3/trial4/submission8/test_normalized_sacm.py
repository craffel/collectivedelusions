...
print("Task vector norms (L, K):")
print(N)

print("Testing normalized SACM step...")
...
# Normalized SACM perturbation
# grads_hat = grads / N
grads_hat = grads / (N + 1e-8)
grad_hat_norm = torch.norm(grads_hat) + 1e-12

rho = 0.15
# epsilon = rho * grads_hat / grad_hat_norm / N
epsilon = rho * grads_hat / (grad_hat_norm * (N + 1e-8))
...
print("Perturbation min/max:", epsilon.min().item(), epsilon.max().item())
print("Normalized step completed successfully!")
