...
from experiments.run_merging import (
    ...
    compute_entropy_loss, get_attention_context
)
...
if __name__ == "__main__":
    ...
    # Let's import expert_heads into experiments.run_merging namespace so compute_entropy_loss can access it
    ...
    for i in range(L):
        for j in range(K):
            g = grads[i, j]
            # Print intermediate grad to see if it requires grad
            if i == 0 and j == 0:
                print(f"Grad[0,0] requires grad: {g.requires_grad}")
