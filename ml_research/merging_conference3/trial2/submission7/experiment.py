...
# Backbone Type: Select 'simple_cnn' or 'resnet18'
...
def train_experts(train_loaders, test_loaders):
    ...
    for task_name, loader in train_loaders.items():
        ...
        if os.path.exists(backbone_path) and os.path.exists(head_path):
            ...
            # Freezing early layers for resnet18 to keep the pre-trained manifolds shared
            if BACKBONE_TYPE == 'resnet18':
...
def get_layer_names():
    if BACKBONE_TYPE == 'resnet18':
        # Only merge layer4 parameters that actually changed during fine-tuning
        dummy = get_backbone()
        return [name for name, _ in dummy.named_parameters() if 'layer4' in name]
    else:
...
def run_model_merging_experiments(base_backbone, experts, heads, train_loaders, test_loaders):
    ...
    # Create sequential streaming iterators for true test-time adaptation
    train_iters = {task: iter(train_loaders[task]) for task in tasks}
    def get_streaming_batch(task):
        nonlocal train_iters
        try:
            x, _ = next(train_iters[task])
    ...
    for step in range(TTA_STEPS):
        ...
        # Draw fresh streaming batches for this adaptation step
        ...
        # Calculate entropy on unlabeled streaming batches across all tasks
    ...
    for step in range(TTA_STEPS):
        ...
        for task_idx, task in enumerate(tasks):
            x = step_batches[task]
            # Compute teacher prediction on current batch
            with torch.no_grad():
                experts[task].eval()
                heads[task].eval()
                teacher_features = experts[task](x)
                teacher_logits = heads[task](teacher_features)
    ...
    # Technical Enhancement: Trainable task-wise thermal coupling coefficients tau_k representing local heat capacities
    # of the task-specific heat baths. This maps directly to output-space canonical ensemble probability distributions
    # without introducing ad-hoc weight scaling heuristics.
    ...
    T_start = 2.0
    T_end = 1.0
    beta = 0.40 # Cooling rate
    
    for step in range(TTA_STEPS_TM):
        ...
        # Determine physical global temperature at step t
        T_t = T_end + (T_start - T_end) * np.exp(-beta * step)
        
        lambdas = torch.clamp(lambdas_raw_tm, 0.0, 1.0)
        # Trainable local task-wise scaling factors tau_k clamped to [0.2, 5.0] to prevent division by zero or extreme scaling
        ...
        for l_idx in range(num_layers):
            ...
            for k_idx in range(num_tasks):
                task_update = expert_params_list[k_idx][l_idx] - base_p
                update = update + lambdas[l_idx, k_idx] * task_update
            # Standard model merging parameter formulation (Equation 1)
            merged_params.append(base_p + update)
        ...
        for task_idx, task in enumerate(tasks):
            x = step_batches[task]
            # Task-specific local temperature T_k(t) = tau_k * T_global(t)
            ...
            with torch.no_grad():
                ...
                teacher_features = experts[task](x)
                teacher_logits = heads[task](teacher_features)
                p_expert = F.softmax(teacher_logits / T_k, dim=-1)
                
            features = backbone_temp(x)
            logits = heads[task](features)
            log_q_merged = F.log_softmax(logits / T_k, dim=-1)
            
            # Physical Helmholtz Free Energy Discrepancy (proportional to T_k * KL)
            kl = F.kl_div(log_q_merged, p_expert, reduction='batchmean')
            loss += T_k * kl
            
        # Harmonic Potential Restoring Force (Physical regularization keeping lambdas near TA initialization)
        # This acts as a potential well, preventing representation collapse and overfitting.
        potential_weight = 0.0
        potential_loss = potential_weight * torch.sum((lambdas - 0.3) ** 2)
        loss += potential_loss
        ...
        if (step+1) % 20 == 0:
            print(f"Step {step+1}/{TTA_STEPS_TM} | T_global={T_t:.2f} | T_task0={tau_k[0].item()*T_t:.2f} | Free Energy Discrepancy: {loss.item():.4f}")
