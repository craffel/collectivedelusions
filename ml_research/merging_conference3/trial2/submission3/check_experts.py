...
with torch.no_grad():
    ...
    logits_base_c = img_feat_base_c @ text_feat_base_c.T * clip_model_base.logit_scale.exp()
    ...
    logits_base_g = img_feat_base_g @ text_feat_base_g.T * clip_model_base.logit_scale.exp()
    ...
    # 2. Expert task 1 (CIFAR-10) using base text features
    img_feat_t1_c = clip_model_task1.get_image_features(c_imgs)
    img_feat_t1_c = img_feat_t1_c / img_feat_t1_c.norm(dim=-1, keepdim=True)
    logits_t1_c = img_feat_t1_c @ text_feat_base_c.T * clip_model_base.logit_scale.exp()
    acc_t1_c = (logits_t1_c.argmax(dim=1) == c_lbls).float().mean().item() * 100
    
    print(f"CIFAR-10 Expert CIFAR-10 Accuracy: {acc_t1_c:.2f}%")
    
    # 3. Expert task 2 (GTSRB) using base text features
    img_feat_t2_g = clip_model_task2.get_image_features(g_imgs)
    img_feat_t2_g = img_feat_t2_g / img_feat_t2_g.norm(dim=-1, keepdim=True)
    logits_t2_g = img_feat_t2_g @ text_feat_base_g.T * clip_model_base.logit_scale.exp()
    acc_t2_g = (logits_t2_g.argmax(dim=1) == g_lbls).float().mean().item() * 100
    
    print(f"GTSRB Expert GTSRB Accuracy: {acc_t2_g:.2f}%")
