...
def get_subsets(dataset_name, preprocess, seed):
    ...
    # Deterministically select 1024 indices from test_full for larger sample size
    test_indices = list(range(len(test_full)))
    random.seed(seed)
    random.shuffle(test_indices)
    eval_indices = test_indices[:1024]
...
def merge_neta(pretrained_sd, task_vectors_sd, lambda_0=0.3, alpha=1.0):
    ...
    for g in range(13):
        ...
        for i, tv in enumerate(task_vectors_sd):
            ...
            # Apply alpha-relaxation: w_k_relaxed = (1 - alpha) + alpha * w_k
...
def merge_neta_no_group0(pretrained_sd, task_vectors_sd, lambda_0=0.3):
    visual_keys = [k for k in pretrained_sd.keys() if 'visual' in k]
    non_visual_keys = [k for k in pretrained_sd.keys() if k not in visual_keys]
        ...
        if 'visual.proj' in k:
            ...
            for i in range(12):
                if f'resblocks.{i}.' in k:
                    ...
                    break
            if not found:
                # Treat this specific key as its own group (Independent Normalization)
                groups[k] = [k]
    ...
    for g_id, g_keys in groups.items():
        if not g_keys:
            continue
        norms = []
        for tv in task_vectors_sd:
...
def merge_ties(pretrained_sd, task_vectors_sd, lambda_0=0.3, reset_thresh=20):
    remove_keys = []
    
    # Flatten on the fly to conserve memory
    flat_ptm = state_dict_to_vector(pretrained_sd, remove_keys)
    flat_tvs = []
    for tv in task_vectors_sd:
    ...
    # Free temporary lists before heavy ties_merging operation
    del flat_tvs
    gc.collect()
    
    merged_tv = ties_merging(flat_tvs_tensor, reset_thresh=reset_thresh, merge_func="dis-sum")
    
    # Assemble back
    merged_flat = flat_ptm + lambda_0 * merged_tv
    merged_sd = vector_to_state_dict(merged_flat, pretrained_sd, remove_keys)
...
if __name__ == "__main__":
    ...
    all_results = {
        ...
        'ties_merging': {d: [] for d in datasets_list},
        'neta_alpha_1.0': {d: [] for d in datasets_list},
        'neta_alpha_0.5': {d: [] for d in datasets_list},
        'neta_no_group0': {d: [] for d in datasets_list},
    ...
    for seed in seeds:
        ...
        # Extract evaluation and calibration subsets (1024 test images)
        eval_subsets = {}
        calib_subsets = {}
        for dataset in datasets_list:
        ...
        del ta_sd, ta_sd_clean, seed_model
        gc.collect()

        # ---------------- 2. TIES-Merging Baseline (Flaw 2) ----------------
        print("\nEvaluating TIES-Merging Baseline...")
        ties_sd = merge_ties(pretrained_sd, task_vectors_sd, lambda_0=0.3, reset_thresh=20)
        ties_sd_clean = {k.replace('model.', ''): v for k, v in ties_sd.items()}
        
        seed_model = ModelWrapper(copy.deepcopy(clip_model)).to(device)
        seed_model.model.load_state_dict(ties_sd_clean, strict=False)
        
        ties_res = evaluate_model(seed_model, eval_subsets, heads, device)
        for dataset in datasets_list:
            all_results['ties_merging'][dataset].append(ties_res[dataset])
            print(f"TIES-Merging - {dataset}: {ties_res[dataset] * 100:.2f}%")
            
        del ties_sd, ties_sd_clean, seed_model
        gc.collect()
            
        # ---------------- 3. NETA (alpha=1.0) ----------------
        print("\nEvaluating NETA (alpha=1.0)...")
        neta_1_sd = merge_neta(pretrained_sd, task_vectors_sd, lambda_0=0.3, alpha=1.0)
        neta_1_sd_clean = {k.replace('model.', ''): v for k, v in neta_1_sd.items()}
        
        seed_model = ModelWrapper(copy.deepcopy(clip_model)).to(device)
        seed_model.model.load_state_dict(neta_1_sd_clean, strict=False)
        
        neta_1_res = evaluate_model(seed_model, eval_subsets, heads, device)
        for dataset in datasets_list:
            all_results['neta_alpha_1.0'][dataset].append(neta_1_res[dataset])
            print(f"NETA (alpha=1.0) - {dataset}: {neta_1_res[dataset] * 100:.2f}%")
            
        del neta_1_sd, neta_1_sd_clean, seed_model
        gc.collect()

        # ---------------- 4. NETA (alpha=0.5 - Flaw 3) ----------------
        print("\nEvaluating NETA (alpha=0.5)...")
        neta_05_sd = merge_neta(pretrained_sd, task_vectors_sd, lambda_0=0.3, alpha=0.5)
        neta_05_sd_clean = {k.replace('model.', ''): v for k, v in neta_05_sd.items()}
        
        seed_model = ModelWrapper(copy.deepcopy(clip_model)).to(device)
        seed_model.model.load_state_dict(neta_05_sd_clean, strict=False)
        
        neta_05_res = evaluate_model(seed_model, eval_subsets, heads, device)
        for dataset in datasets_list:
            all_results['neta_alpha_0.5'][dataset].append(neta_05_res[dataset])
            print(f"NETA (alpha=0.5) - {dataset}: {neta_05_res[dataset] * 100:.2f}%")
            
        del neta_05_sd, neta_05_sd_clean, seed_model
        gc.collect()

        # ---------------- 5. NETA Independent Normalization (No Group 0 - Flaw 3) ----------------
        print("\nEvaluating NETA with Independent Normalization (No Group 0)...")
        neta_ng0_sd = merge_neta_no_group0(pretrained_sd, task_vectors_sd, lambda_0=0.3)
        neta_ng0_sd_clean = {k.replace('model.', ''): v for k, v in neta_ng0_sd.items()}
        
        seed_model = ModelWrapper(copy.deepcopy(clip_model)).to(device)
        seed_model.model.load_state_dict(neta_ng0_sd_clean, strict=False)
        
        neta_ng0_res = evaluate_model(seed_model, eval_subsets, heads, device)
        for dataset in datasets_list:
            all_results['neta_no_group0'][dataset].append(neta_ng0_res[dataset])
            print(f"NETA (No Group 0) - {dataset}: {neta_ng0_res[dataset] * 100:.2f}%")
            
        del neta_ng0_sd, neta_ng0_sd_clean, seed_model
        gc.collect()
            
        # ---------------- 6. Task-Wise AdaMerging (Continuous TTA) ----------------
        ...
        del seed_clip, seed_model, opt_model, optimizer, final_img_enc
        gc.collect()
            
        # ---------------- 7. Layer-Wise AdaMerging (Continuous TTA) ----------------
    ...
    print("Comprehensive evaluation script complete!")
