...
def eval_single_dataset(image_encoder, head, dataset_name, args):
    ...
    # Using small batch to speed up evaluation under CPU constraints
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            # Limit evaluation to 2 batches (256 images) for fast and highly representative estimates, matching train_experts.py
            if i > 1:
                break
...
def run_adam_optimization(adamerging_model, exam_datasets, args, beta=0.0, gamma=0.0, use_ccn=False, use_snew=False, steps=50):
    ...
    # 1. First Pass: Compute Initial Scale Weights for SNEW if enabled
    scale_weights = {dataset_name: 1.0 for dataset_name in exam_datasets}
    class_capacities = {dataset_name: 10.0 for dataset_name in exam_datasets} # all datasets here have 10 classes
    ...
    # Pre-load calibration batches to avoid loading from disk at each step
    calib_batches = {}
    for dataset_name in exam_datasets:
        dataset = get_dataset(dataset_name, adamerging_model.model.val_preprocess, location=args.data_location, batch_size=16)
        dataloader = get_dataloader_shuffle(dataset)
        for batch in dataloader:
            batch = maybe_dictionarize(batch)
            calib_batches[dataset_name] = batch['images'].to(args.device)
            break
    ...
    if use_snew:
        ...
        with torch.no_grad():
            for dataset_name in exam_datasets:
                ...
                # Class-Capacity Normalize if CCN is also active
                if use_ccn:
...
def run_es_optimization(adamerging_model, exam_datasets, args, beta=0.0, gamma=0.0, use_ccn=False, use_snew=False, steps=50):
    # Black-box derivative-free 1+1 Evolution Strategy
    # Optimized for CPU execution speed and simplicity
    ...
    # Define a loss evaluation function
...
def run_experiment_pipeline():
    ...
    if not os.path.exists(pretrained_checkpoint):
        print("ERROR: Pre-trained checkpoint does not exist. Please run train_experts.py first!")
        return

    print("Loading pre-trained model and constructing ModelWrapper...")
    ...
    for dataset_name in args.eval_datasets:
        ...
        if not os.path.exists(finetuned_path):
            print(f"ERROR: Expert checkpoint for {dataset_name} does not exist!")
            return
    ...
    for seed in seeds:
        ...
        # static weights: lambda_raw set to 0.3, no optimization
    ...
    # 2. Unconstrained AdaMerging (Adam GD)
    print("\nEvaluating Baseline: Unconstrained AdaMerging (Adam GD)")
    ...
    for seed in seeds:
        ...
        run_adam_optimization(model_adam, args.eval_datasets, args, beta=0.0, gamma=0.0, steps=25)
        results = evaluate_model(model_adam, args.eval_datasets, args)
        for k, v in results.items():
            unconstrained_adam_accs[k].append(v)
            
    all_metrics["adam_opt"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in unconstrained_adam_accs.items()
    }

    # 3. Unconstrained AdaMerging (1+1 ES)
    print("\nEvaluating Baseline: Unconstrained AdaMerging (1+1 ES)")
    ...
    for seed in seeds:
        ...
        run_es_optimization(model_es, args.eval_datasets, args, beta=0.0, gamma=0.0, steps=25)
        results = evaluate_model(model_es, args.eval_datasets, args)
        for k, v in results.items():
            unconstrained_es_accs[k].append(v)
            
    all_metrics["es_opt"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in unconstrained_es_accs.items()
    }

    # 4. Spatially Averaged AdaMerging (Spatial Mean)
    print("\nEvaluating Baseline: Spatially Averaged AdaMerging (Spatial Mean)")
    ...
    for seed in seeds:
        ...
        run_es_optimization(model_sm, args.eval_datasets, args, beta=0.0, gamma=0.0, steps=25)
        results = evaluate_model(model_sm, args.eval_datasets, args)
        for k, v in results.items():
            spatial_mean_accs[k].append(v)
            
    all_metrics["spatial_mean_es"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in spatial_mean_accs.items()
    }

    # 5. Diagnostic Treatment: Shuffled Adam GD
    print("\nEvaluating Diagnostic Treatment: Shuffled Adam GD")
    ...
    for seed in seeds:
        ...
        run_adam_optimization(model_sh_adam, args.eval_datasets, args, beta=0.0, gamma=0.0, steps=25)
        
        # Shuffle parameters across named parameter dimension
        with torch.no_grad():
    ...
    all_metrics["shuffled_adam"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in shuffled_adam_accs.items()
    }

    # 6. Diagnostic Treatment: Shuffled ES
    print("\nEvaluating Diagnostic Treatment: Shuffled ES")
    ...
    for seed in seeds:
        ...
        run_es_optimization(model_sh_es, args.eval_datasets, args, beta=0.0, gamma=0.0, steps=25)
    ...
    all_metrics["shuffled_es"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in shuffled_es_accs.items()
    }

    # 7. RegCalMerge (Our complete model: ESR with beta=1.0, gamma=1.0, CCN and SNEW)
    ...
    for seed in seeds:
        ...
        run_adam_optimization(model_rcm, args.eval_datasets, args, beta=1.0, gamma=1.0, use_ccn=True, use_snew=True, steps=25)
        results = evaluate_model(model_rcm, args.eval_datasets, args)
        for k, v in results.items():
            rcm_accs[k].append(v)
            
    all_metrics["regcalmerge_opt"] = {
        k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in rcm_accs.items()
    }

    # 8. Ablation Study: Dense Grid Sweep over Beta and Gamma
    print("\nRunning Ablation Study Grid Sweep (beta x gamma)...")
    ablation_results = {}
    beta_sweep = [0.0, 1.0, 2.0]
    gamma_sweep = [0.0, 1.0, 2.0]
    ...
    for b in beta_sweep:
        for g in gamma_sweep:
            ...
            for seed in seeds[:1]:
                ...
                run_adam_optimization(model_grid, args.eval_datasets, args, beta=b, gamma=g, use_ccn=True, use_snew=True, steps=20)
                results = evaluate_model(model_grid, args.eval_datasets, args)
                for k, v in results.items():
                    grid_accs[k].append(v)
            
            # Save stats for this grid point
            ablation_results[grid_key] = {
                k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in grid_accs.items()
            }
            avg_acc = np.mean([np.mean(v) for v in grid_accs.values()])
            print(f"Grid Configuration: beta={b}, gamma={g} -> Average Acc: {100*avg_acc:.2f}%")
    ...
    print(f"\nAll experiments complete! Metrics saved to {metrics_path}")
