...
def test_time_optimize(active_model, base_dict, task_vectors, expert_heads, mode, quantize, bits, calib_loaders, steps=40, lr=1e-3):
    print(f"DEBUG: test_time_optimize bits={bits}")
    prior = 0.3
...
def evaluate_model(active_model, base_dict, task_vectors, lambdas_raw, expert_heads, mode, quantize, bits, test_loaders):
    print(f"DEBUG: evaluate_model bits={bits}")
    active_model.eval()
    task_accs = {}
    with torch.no_grad():
        update_model_weights(active_model, base_dict, task_vectors, lambdas_raw, mode, quantize, bits)
...
def main():
    ...
    for calib_size in sizes_to_test:
        print(f"\nEvaluating calibration size N={calib_size} per task (Total: {calib_size * 4})...")
        calib_datasets = {task: get_cached_dataset(task, split='test', size=calib_size, seed=777) for task in TASKS}
        calib_loaders = {task: DataLoader(calib_datasets[task], batch_size=calib_size, shuffle=False) for task in TASKS}
        
        results[calib_size] = {}
        for bits in [8, 4]:
            lambdas = test_time_optimize(
                active_model, base_dict, task_vectors, expert_heads, 'clipping', 
                quantize=True, bits=bits, calib_loaders=calib_loaders
            )
            accs = evaluate_model(
            ...
            results[calib_size][bits] = {
                'accuracies': accs,
                'average': float(avg_acc)
            }
            
    # Save results to json for safety
    with open("calibration_size_sensitivity.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nCalibration sensitivity sweep finished and saved!")
