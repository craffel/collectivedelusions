...
DEVICE = 'cpu' # Run on CPU for login node safety and simplicity
...
def update_model_weights(active_model, base_dict, task_vectors, lambdas_raw, mode, quantize, bits):
    if mode == 'clipping':
        ...
        lambdas = torch.softmax(lambdas_raw, dim=-1) # Sum-to-one across tasks
    else:
...
def test_time_optimize(active_model, base_dict, task_vectors, expert_heads, mode, quantize, bits, calib_loaders, steps=40, lr=1e-3):
    if mode == 'clipping':
        ...
        # Softmax inputs initialized to identical values so it starts at 0.25 uniformly
...
def main():
    ...
    print(f"\n--- Comparing Clipping vs Softmax for Seed {seed}, SAM Radius {rho} ---")
    expert_heads = []
    expert_dicts = []
    for task in TASKS:
    ...
    for bits in [8, 4]:
        ...
        # 1. Independent clipping [0, 1]
        lambdas_clip = test_time_optimize(active_model, base_dict, task_vectors, expert_heads, 'clipping', quantize=True, bits=bits, calib_loaders=calib_loaders)
        accs_clip = evaluate_model(active_model, base_dict, task_vectors, lambdas_clip, expert_heads, 'clipping', quantize=True, bits=bits, test_loaders=test_loaders)
        avg_clip = np.mean(list(accs_clip.values()))
        print(f"  [Clipping] Task Accuracies: {accs_clip}")
        print(f"  [Clipping] Average Multi-Task Accuracy: {avg_clip:.2f}%")
        
        # 2. Softmax (Convex combination)
        lambdas_softmax = test_time_optimize(active_model, base_dict, task_vectors, expert_heads, 'softmax', quantize=True, bits=bits, calib_loaders=calib_loaders)
        accs_softmax = evaluate_model(active_model, base_dict, task_vectors, lambdas_softmax, expert_heads, 'softmax', quantize=True, bits=bits, test_loaders=test_loaders)
        avg_softmax = np.mean(list(accs_softmax.values()))
        print(f"  [Softmax ] Task Accuracies: {accs_softmax}")
        print(f"  [Softmax ] Average Multi-Task Accuracy: {avg_softmax:.2f}%")
        
        # Print weight scale/norms comparison
        lambdas_clip_clamped = torch.clamp(lambdas_clip, min=0.0, max=1.0)
        lambdas_softmax_normalized = torch.softmax(lambdas_softmax, dim=-1)
        print(f"  [Clipping] Mean coefficient sum per layer: {lambdas_clip_clamped.sum(dim=-1).mean().item():.3f}")
        print(f"  [Softmax ] Mean coefficient sum per layer: {lambdas_softmax_normalized.sum(dim=-1).mean().item():.3f}")
