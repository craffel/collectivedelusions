...
class DynamicMergedModel(nn.Module):
    ...
    def forward(self, images):
        with torch.no_grad():
            self.sensory_extractor.eval()
            pooled = self.sensory_extractor(images)
            
        h = F.linear(pooled, self.P_proj)
        self.current_h = h
...
def main():
    ...
    print(f"Running optimal Cosine Annealing scheduler (LR 1e-3) sweep on {device}...")
    ...
    # Optimal Cosine Annealing (Base LR 1e-3)
    set_seed(42)
    ...
    print(f"Optimal Cosine Annealing (Base LR 1e-3, 256 samples) | Accuracy: {acc:.2f}%")
