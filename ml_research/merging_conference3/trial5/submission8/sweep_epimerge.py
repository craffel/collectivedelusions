...
class DynamicMergedModel(nn.Module):
    ...
    def forward(self, images):
        ...
        # 2. Extract global representational state h
        mean_tokens = x.mean(dim=1) # [B, 192]
        h = F.linear(mean_tokens, self.P_proj) # [B, latent_dim]
        self.current_h = h
        
        # 3. Process remaining layers
...
def main():
    ...
    print(f"Sweeping EpiMerge hyperparameters on {device}...")
    ...
    # Load test loader (shuffled)
    test_images = []
    test_labels = []
    test_task_indices = []
    for task_idx, ds in enumerate(datasets_list):
        test_ds = get_dataset(ds, split='val')
        indices = list(range(len(test_ds)))[:500]
        for idx in indices:
    ...
    step_counts = [100, 150, 200, 250, 300]
    ...
    for lr in learning_rates:
        for steps in step_counts:
            ...
            optimizer = torch.optim.Adam(epimerge_model.parameters(), lr=lr)
            ...
            print(f"LR: {lr} | Steps: {steps} | Acc: {acc:.2f}%")
            if acc > best_acc:
                best_acc = acc
                best_config = (lr, steps)
                
    print(f"\nSweep complete! Best Config: LR={best_config[0]}, Steps={best_config[1]} | Best Acc: {best_acc:.2f}%")
