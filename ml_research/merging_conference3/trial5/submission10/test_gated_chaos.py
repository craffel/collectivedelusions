...
# --- 1. Original ChaosMerge Model ---
class OriginalChaosMergeModel(torch.nn.Module):
    ...
    def forward_coefficients(self, x_features, average_batch=True):
        ...
        for l in range(self.L):
            ...
            s = torch.sigmoid(logits + perturbation)
            all_states.append(s)
        all_states = torch.stack(all_states, dim=0) # L x B x K
        R = self.R_raw.unsqueeze(1) # L x 1 x K
        alpha = R * all_states # L x B x K
        if average_batch:
            return torch.mean(alpha, dim=1) # L x K
        return alpha # L x B x K

# --- 2. Gated ChaosMerge Model ---
class GatedChaosMergeModel(torch.nn.Module):
    def __init__(self, d=4, K=4, L=14):
        ...
        # New gating parameter
        # Sigmoid will map self.gate_raw to range [0, 1]. Initialized to 0.0 (sigmoid(0.0) = 0.5) or slightly lower.
        # Let's initialize to -1.0 so that gate is around 0.27, allowing strong skip connection (0.73) from the previous state.
        self.gate_raw = torch.nn.Parameter(torch.ones(L) * -1.0)
...
def main():
    ...
    # --------------------------------------------------------------------
    # Experiment A: Original ChaosMerge (Task-Averaged Coefficients)
    # --------------------------------------------------------------------
    print("\n--- Running Experiment A: Original ChaosMerge (Task-Averaged) ---")
    orig_model = OriginalChaosMergeModel().to(device)
    optimizer = torch.optim.Adam(orig_model.parameters(), lr=0.01)
    criterion = torch.nn.CrossEntropyLoss()
    
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_bar = orig_model.forward_coefficients(calib_mean_feats, average_batch=True)
    ...
    orig_accs = []
    with torch.no_grad():
        alpha_bar_test = orig_model.forward_coefficients(test_mean_feats, average_batch=True)
    for idx, name in enumerate(task_names):
        acc = evaluate_merged_model_with_coeffs(base_model, task_vectors, alpha_bar_test, task_heads, test_loaders[name], idx, device=device)
        orig_accs.append(acc)
    print(f"Original ChaosMerge: {orig_accs} | Avg: {np.mean(orig_accs)*100:.2f}%")


    # --------------------------------------------------------------------
    # Experiment B: Gated ChaosMerge (Task-Averaged Coefficients)
    # --------------------------------------------------------------------
    print("\n--- Running Experiment B: Gated ChaosMerge (Task-Averaged) ---")
    gated_model = GatedChaosMergeModel().to(device)
    optimizer = torch.optim.Adam(gated_model.parameters(), lr=0.01)
    
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_bar = gated_model.forward_coefficients(calib_mean_feats, average_batch=True)
        merged_state = merge_weights(pretrained_state, task_vectors, alpha_bar, device=device)
        for idx, name in enumerate(task_names):
            x_cal, y_cal = next(iter(calib_loaders[name]))
            x_cal, y_cal = x_cal.to(device), y_cal.to(device)
    ...
    gated_accs = []
    with torch.no_grad():
        alpha_bar_test = gated_model.forward_coefficients(test_mean_feats, average_batch=True)
    for idx, name in enumerate(task_names):
        acc = evaluate_merged_model_with_coeffs(base_model, task_vectors, alpha_bar_test, task_heads, test_loaders[name], idx, device=device)
        gated_accs.append(acc)
    print(f"Gated ChaosMerge (Task-Averaged): {gated_accs} | Avg: {np.mean(gated_accs)*100:.2f}%")


    # --------------------------------------------------------------------
    # Experiment C: Gated ChaosMerge (Task-Specific Coefficients)
    # --------------------------------------------------------------------
    print("\n--- Running Experiment C: Gated ChaosMerge (Task-Specific) ---")
    # Here, during evaluation and optimization, we do NOT take the average across tasks/samples.
    # We map task features directly to task coefficients.
    gated_spec_model = GatedChaosMergeModel().to(device)
    optimizer = torch.optim.Adam(gated_spec_model.parameters(), lr=0.01)
    
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        # Shape: L x K x K (L layers, K tasks, K experts)
        alpha_spec = gated_spec_model.forward_coefficients(calib_mean_feats, average_batch=False)
        
        for idx, name in enumerate(task_names):
            x_cal, y_cal = next(iter(calib_loaders[name]))
            x_cal, y_cal = x_cal.to(device), y_cal.to(device)
            
            # Use task-specific alpha: shape L x K
    ...
    gated_spec_accs = []
    with torch.no_grad():
        alpha_spec_test = gated_spec_model.forward_coefficients(test_mean_feats, average_batch=False)
    for idx, name in enumerate(task_names):
        ...
        gated_spec_accs.append(acc)
    print(f"Gated ChaosMerge (Task-Specific): {gated_spec_accs} | Avg: {np.mean(gated_spec_accs)*100:.2f}%")
