...
# --- 1. Linear Router Model (Task-Specific support) ---
class LinearRouterModel(torch.nn.Module):
    ...
    def forward_coefficients(self, x_features, average_batch=True):
        ...
        out = out.permute(1, 0, 2)  # L x B x K
        if average_batch:
            return torch.mean(out, dim=1)  # L x K
        return out  # L x B x K

# --- 2. QWS-Merge Model (Task-Specific support) ---
class QWSMergeModel(torch.nn.Module):
    ...
    def forward_coefficients(self, x_features, average_batch=True):
        ...
        for l in range(self.L):
            proj = torch.matmul(psi, self.Theta[l].t()) + self.theta[l]
            alpha_l = self.R_raw[l] * torch.cos(proj)
            all_alphas.append(alpha_l)
        all_alphas = torch.stack(all_alphas, dim=0)  # L x B x K
        if average_batch:
            return torch.mean(all_alphas, dim=1)
        return all_alphas

# --- 3. Gated ChaosMerge Model ---
...
def main():
    ...
    for task_idx, name in enumerate(task_names):
        ...
        calib_idx = list(range(train_size, min(len(train_ds), train_size + calib_size)))
    ...
    # --------------------------------------------------------------------
    # Evaluation 1: Linear Router (Task-Specific Routing)
    # --------------------------------------------------------------------
    print("\n--- Running Linear Router (Task-Specific) ---")
    lr_model = LinearRouterModel().to(device)
    optimizer = torch.optim.Adam(lr_model.parameters(), lr=0.01)
    criterion = torch.nn.CrossEntropyLoss()
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_spec = lr_model.forward_coefficients(calib_mean_feats, average_batch=False)
    ...
    lr_accs = []
    with torch.no_grad():
        alpha_spec_test = lr_model.forward_coefficients(test_mean_feats, average_batch=False)
    for idx, name in enumerate(task_names):
        ...
        lr_accs.append(acc)
    print(f"Linear Router (Task-Specific): {lr_accs} | Avg: {np.mean(lr_accs)*100:.2f}%")

    # --------------------------------------------------------------------
    # Evaluation 2: QWS-Merge (Task-Specific Routing)
    # --------------------------------------------------------------------
    print("\n--- Running QWS-Merge (Task-Specific) ---")
    qws_model = QWSMergeModel().to(device)
    optimizer = torch.optim.Adam(qws_model.parameters(), lr=0.01)
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_spec = qws_model.forward_coefficients(calib_mean_feats, average_batch=False)
        for idx, name in enumerate(task_names):
            x_cal, y_cal = next(iter(calib_loaders[name]))
            x_cal, y_cal = x_cal.to(device), y_cal.to(device)
            task_alpha = alpha_spec[:, idx, :]
    ...
    qws_accs = []
    with torch.no_grad():
        alpha_spec_test = qws_model.forward_coefficients(test_mean_feats, average_batch=False)
    for idx, name in enumerate(task_names):
        task_alpha_test = alpha_spec_test[:, idx, :]
        acc = evaluate_merged_model_with_coeffs(base_model, task_vectors, task_alpha_test, task_heads, test_loaders[name], idx, device=device)
        qws_accs.append(acc)
    print(f"QWS-Merge (Task-Specific): {qws_accs} | Avg: {np.mean(qws_accs)*100:.2f}%")

    # --------------------------------------------------------------------
    # Evaluation 3: ChaosMerge (Task-Specific Routing)
    # --------------------------------------------------------------------
    print("\n--- Running ChaosMerge (Task-Specific) ---")
    ...
    print(f"ChaosMerge (Task-Specific): {chaos_accs} | Avg: {np.mean(chaos_accs)*100:.2f}%")
