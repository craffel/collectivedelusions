...
class AnnealedChaosMergeModel(torch.nn.Module):
    ...
    def forward_coefficients(self, x_features, step_ratio=0.0, average_batch=True):
        """
        step_ratio: float from 0.0 to 1.0 representing progression through training.
                    0.0 means fully chaotic Logistic Map (f_chaotic)
                    1.0 means fully contractive Tanh Gated Map (f_contractive)
        """
        ...
        for l in range(self.L):
            ...
            # Linear interpolation (annealing) between Chaotic and Contractive Map
            # f_chaotic: Logistic Map 4u(1-u)
            # f_contractive: Tanh Gated tanh(4(u-0.5))*0.5 + 0.5
            
            # Using inline definitions for maps
            f_chaotic = 4.0 * s * (1.0 - s)
            f_contractive = torch.tanh(4.0 * (s - 0.5)) * 0.5 + 0.5
            
            f_s = (1.0 - step_ratio) * f_chaotic + step_ratio * f_contractive
...
def train_and_evaluate_annealed(device, pretrained_state, task_vectors, task_heads, test_loaders, calib_loaders, calib_mean_feats, test_mean_feats, base_model, steps=50):
    print(f"\nTraining Annealed Chaos-to-Order Merging ({steps} steps)...")
    model = AnnealedChaosMergeModel().to(device)
    ...
    for step in range(steps):
        step_ratio = float(step) / max(1, steps - 1)
        optimizer.zero_grad()
        total_loss = 0.
        # Pass step_ratio to interpolate maps dynamically
        alpha_spec = model.forward_coefficients(calib_mean_feats, step_ratio=step_ratio, average_batch=False)
        ...
        optimizer.step()
        if (step+1) % 10 == 0:
            print(f"  Step {step+1}/{steps} - Loss: {total_loss.item():.4f} - Step Ratio: {step_ratio:.2f}")
        
    accs = []
    print(f"Evaluating Annealed Chaos-to-Order Model (at final step_ratio=1.0)...")
    with torch.no_grad():
        # Evaluate with step_ratio = 1.0 (converged, contractive regime)
        alpha_spec_test = model.forward_coefficients(test_mean_feats, step_ratio=1.0, average_batch=False)
    for idx, name in enumerate(task_names):
        ...
        accs.append(acc)
    print(f"Results for Annealed Chaos-to-Order Model: {accs} | Average: {np.mean(accs)*100:.2f}%")
    return accs
...
def main():
    ...
    train_size = 2000
    calib_size = 64
    test_size = 32 # Keep testing batch small for high efficiency
    ...
    train_and_evaluate_annealed(device, pretrained_state, task_vectors, task_heads, test_loaders, calib_loaders, calib_mean_feats, test_mean_feats, base_model, steps=50)
