    ...
    def forward_coefficients(self, x_features, average_batch=True):
        ...
        for l in range(self.L):
            ...
            if self.map_type == 'logistic':
                ...
                def f(u):
                    return u
            elif self.map_type == 'tanh_gated':
                # Non-chaotic, bounded
                def f(u):
                    return torch.tanh(4.0 * (u - 0.5)) * 0.5 + 0.5
            elif self.map_type == 'sigmoid_gated':
                # Non-chaotic, bounded
                def f(u):
                    return torch.sigmoid(4.0 * (u - 0.5))
            else:
...
def train_and_evaluate(map_type, device, pretrained_state, task_vectors, task_heads, test_loaders, calib_loaders, calib_mean_feats, test_mean_feats, base_model, steps=10):
    print(f"\nTraining Gated ChaosMerge (G-CML) with {map_type.capitalize()} Map ({steps} steps)...")
    ...
    accs = []
    print(f"Evaluating {map_type.capitalize()} Map...")
    with torch.no_grad():
    ...
    for idx, name in enumerate(task_names):
        ...
        accs.append(acc)
    print(f"Results for {map_type.capitalize()} Map: {accs} | Average: {np.mean(accs)*100:.2f}%")
    return accs
...
def main():
    ...
    # Run non-chaotic map ablations for 50 steps
    maps = ['logistic', 'identity', 'tanh_gated', 'sigmoid_gated']
    results = {}
    for m in maps:
    ...
    print("\n--- Non-Chaotic Ablation Study Table (10 steps) ---")
    print(f"{'Map Type':<15} | {'MNIST':<8} | {'Fashion':<8} | {'CIFAR10':<8} | {'SVHN':<8} | {'Average':<8}")
    print("-" * 75)
    for m in maps:
        row = results[m]
        print(f"{m.capitalize():<15} | {row[0]*100:<7.2f}% | {row[1]*100:<7.2f}% | {row[2]*100:<7.2f}% | {row[3]*100:<7.2f}% | {np.mean(row)*100:<7.2f}%")
