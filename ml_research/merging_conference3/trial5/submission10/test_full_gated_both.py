...
# --- Gated ChaosMerge Model ---
class GatedChaosMergeModel(torch.nn.Module):
    def __init__(self, d=4, K=4, L=14, init_val=-1.0):
        super().__init__()
        self.d = d
        self.K = K
        self.L = L
...
def main():
    ...
    # --------------------------------------------------------------------
    # Configuration 1: Gated ChaosMerge (Task-Averaged, gate_init = -1.0)
    # --------------------------------------------------------------------
    print("\n--- Config 1: Gated ChaosMerge (Task-Averaged, gate=-1.0) ---")
    m1 = GatedChaosMergeModel(init_val=-1.0).to(device)
    optimizer = torch.optim.Adam(m1.parameters(), lr=0.01)
    criterion = torch.nn.CrossEntropyLoss()
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_bar = m1.forward_coefficients(calib_mean_feats, average_batch=True)
    ...
    accs1 = []
    with torch.no_grad():
        alpha_bar_test = m1.forward_coefficients(test_mean_feats, average_batch=True)
    for idx, name in enumerate(task_names):
        ...
        accs1.append(acc)
    print(f"Results: {accs1} | Avg: {np.mean(accs1)*100:.2f}%")

    # --------------------------------------------------------------------
    # Configuration 2: Gated ChaosMerge (Task-Specific, gate_init = -1.0)
    # --------------------------------------------------------------------
    print("\n--- Config 2: Gated ChaosMerge (Task-Specific, gate=-1.0) ---")
    m2 = GatedChaosMergeModel(init_val=-1.0).to(device)
    optimizer = torch.optim.Adam(m2.parameters(), lr=0.01)
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_spec = m2.forward_coefficients(calib_mean_feats, average_batch=False)
        for idx, name in enumerate(task_names):
            x_cal, y_cal = next(iter(calib_loaders[name]))
            x_cal, y_cal = x_cal.to(device), y_cal.to(device)
    ...
    accs2 = []
    with torch.no_grad():
        alpha_spec_test = m2.forward_coefficients(test_mean_feats, average_batch=False)
    for idx, name in enumerate(task_names):
        ...
        accs2.append(acc)
    print(f"Results: {accs2} | Avg: {np.mean(accs2)*100:.2f}%")

    # --------------------------------------------------------------------
    # Configuration 3: Gated ChaosMerge (Task-Specific, gate_init = -2.0)
    # --------------------------------------------------------------------
    print("\n--- Config 3: Gated ChaosMerge (Task-Specific, gate=-2.0) ---")
    m3 = GatedChaosMergeModel(init_val=-2.0).to(device)
    optimizer = torch.optim.Adam(m3.parameters(), lr=0.01)
    for step in range(50):
        optimizer.zero_grad()
        total_loss = 0.
        alpha_spec = m3.forward_coefficients(calib_mean_feats, average_batch=False)
        for idx, name in enumerate(task_names):
            x_cal, y_cal = next(iter(calib_loaders[name]))
            x_cal, y_cal = x_cal.to(device), y_cal.to(device)
            task_alpha = alpha_spec[:, idx, :]
    ...
    accs3 = []
    with torch.no_grad():
        alpha_spec_test = m3.forward_coefficients(test_mean_feats, average_batch=False)
    for idx, name in enumerate(task_names):
        task_alpha_test = alpha_spec_test[:, idx, :]
        acc = evaluate_merged_model_with_coeffs(base_model, task_vectors, task_alpha_test, task_heads, test_loaders[name], idx, device=device)
        accs3.append(acc)
    print(f"Results: {accs3} | Avg: {np.mean(accs3)*100:.2f}%")
