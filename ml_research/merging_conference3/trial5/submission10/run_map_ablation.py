...
def train_and_evaluate(map_type, device, pretrained_state, task_vectors, task_heads, test_loaders, calib_loaders, calib_mean_feats, test_mean_feats, base_model):
    print(f"\nTraining Gated ChaosMerge (G-CML) with {map_type.capitalize()} Map...")
...
def main():
    ...
    # Run map ablations
    maps = ['logistic', 'tent', 'sine']
    results = {}
    for m in maps:
        results[m] = train_and_evaluate(m, device, pretrained_state, task_vectors, task_heads, test_loaders, calib_loaders, calib_mean_feats, test_mean_feats, base_model)
        
    print("\n--- Map Ablation Study Table ---")
    print(f"{'Map Type':<12} | {'MNIST':<8} | {'Fashion':<8} | {'CIFAR10':<8} | {'SVHN':<8} | {'Average':<8}")
    print("-" * 65)
    for m in maps:
        row = results[m]
        print(f"{m.capitalize():<12} | {row[0]*100:<7.2f}% | {row[1]*100:<7.2f}% | {row[2]*100:<7.2f}% | {row[3]*100:<7.2f}% | {np.mean(row)*100:<7.2f}%")
