...
def main():
        ...
        with torch.no_grad():
            for task in tasks:
                ...
                for imgs, labels in loader:
                    ...
                    router_in = base_model.patch_embed(imgs).mean(dim=1)
    ...
    for seed in seeds:
        ...
        print(f"Linear Router - SVHN: {lr_accs['SVHN']:.4f}, Joint Mean: {lr_mean:.4f}")
        ...
        print(f"RLR - SVHN: {rlr_accs['SVHN']:.4f}, Joint Mean: {rlr_mean:.4f}")
    ...
    print(f"Linear Router SVHN: {np.mean(lr_svhns):.4f} +/- {np.std(lr_svhns):.4f} (Min: {np.min(lr_svhns):.4f})")
    print(f"Linear Router Mean: {np.mean(lr_means):.4f} +/- {np.std(lr_means):.4f}")
    print(f"RLR SVHN: {np.mean(rlr_svhns):.4f} +/- {np.std(rlr_svhns):.4f} (Min: {np.min(rlr_svhns):.4f})")
    print(f"RLR Mean: {np.mean(rlr_means):.4f} +/- {np.std(rlr_means):.4f}")
