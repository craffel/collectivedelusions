...
def get_optimal_profile_stepwise(k, l):
    # Step-wise block architecture: early (0-3), middle (4-9), late (10-11)
    if k == 0:
        if l <= 3: return 0.2
        elif l <= 9: return 0.5
        else: return 0.8
...
print("\n--- STEP-WISE LANDSPACE: COUPLED COVARIANCE (10 Seeds Avg Acc) ---")
for k, v in coupled_res.items():
    print(f"{k:<10}: {np.mean(v):.4f}%")

print("\n--- STEP-WISE LANDSPACE: DECOUPLED EUCLIDEAN (10 Seeds Avg Acc) ---")
