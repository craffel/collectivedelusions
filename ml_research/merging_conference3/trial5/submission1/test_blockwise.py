...
def run_experiment_for_seed(seed):
    ...
    # TV-Regularized (beta=0.2, lr=0.02)
    ...
    # RCR-Merge (Ours) (alpha_gnb=0.5, lr=0.08, anchor=0.001)
...
print("\n--- STAGE-WISE MODULAR TRANSITION LANDSCAPE: COUPLED COVARIANCE (30 Seeds Avg Acc) ---")
for k, v in coupled_res.items():
    print(f"{k:<10}: {np.mean(v):.4f}% +/- {np.std(v):.4f}%")

print("\n--- STAGE-WISE MODULAR TRANSITION LANDSCAPE: DECOUPLED EUCLIDEAN (30 Seeds Avg Acc) ---")
