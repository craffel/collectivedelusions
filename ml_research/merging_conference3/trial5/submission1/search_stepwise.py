...
def run_experiment_for_seed(seed, alpha_gnb, gamma_anchor, lr_rcr, use_euclidean=False):
    ...
    def evaluate_accuracy(coeffs, use_euclidean):
        d = coeffs - alpha_opt
    ...
    return evaluate_accuracy(coeffs_rcr, use_euclidean)

# Run a grid search on 10 seeds
for use_euc in [False, True]:
    print(f"\n--- GRID SEARCH ON STEPWISE LANDSCAPE (EUCLIDEAN={use_euc}) ---")
    best_acc = 0
    best_params = None
        ...
        for gamma in [0.001, 0.005, 0.01, 0.02, 0.05, 0.1]:
            for lr in [0.01, 0.02, 0.05, 0.1]:
                ...
                for seed in range(1, 11):
                    acc = np.mean(run_experiment_for_seed(seed, alpha, gamma, lr, use_euclidean=use_euc))
                    accs.append(acc)
                mean_acc = np.mean(accs)
                if mean_acc > best_acc:
                    best_acc = mean_acc
                    best_params = (alpha, gamma, lr)
    print(f"Best: {best_acc:.4f}% with Alpha={best_params[0]}, Gamma={best_params[1]}, LR={best_params[2]}")
