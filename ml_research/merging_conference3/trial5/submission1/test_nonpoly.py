...
def get_optimal_profile_nonpoly(k, l_norm):
    # Add a high-frequency, non-polynomial sinusoidal ripple representing localized layer specializations
    ripple = 0.12 * np.sin(3.5 * np.pi * l_norm)
    if k == 0:
        return 0.3 + 0.4 * l_norm + ripple
    elif k == 1:
        return 0.7 - 0.4 * l_norm + ripple
    elif k == 2:
        return 0.2 + 1.2 * l_norm - 1.2 * (l_norm ** 2) + ripple
    elif k == 3:
        return 0.55 + ripple
    else:
        return 0.5 + ripple
...
def run_experiment_for_seed(seed):
    ...
    for k in range(K):
        for l in range(L):
            l_norm = l / (L - 1)
            opt_val = get_optimal_profile_nonpoly(k, l_norm)
...
print("\n--- NON-POLYNOMIAL LANDSPACE: COUPLED COVARIANCE (10 Seeds Avg Acc) ---")
for k, v in coupled_res.items():
    print(f"{k:<10}: {np.mean(v):.4f}%")

print("\n--- NON-POLYNOMIAL LANDSPACE: DECOUPLED EUCLIDEAN (10 Seeds Avg Acc) ---")
