...
def get_optimal_profile_blockwise(k, l):
    # Distinct stage blocks: early (0-3), middle (4-7), late (8-11)
    if k == 0:
        if l <= 3: return 0.1
...
def run_experiment_for_seed(seed, alpha_gnb, gamma_anchor, lr_rcr):
    ...
    return {
        'coupled': evaluate_accuracy(coeffs_rcr, use_euclidean=False),
        'euclidean': evaluate_accuracy(coeffs_rcr, use_euclidean=True)
    }

# Search
for alpha in [0.05, 0.1, 0.2, 0.5, 1.0, 1.5, 2.0]:
    for gamma in [0.001, 0.005, 0.01, 0.02]:
        for lr in [0.02, 0.05, 0.08]:
            coupled_accs = []
            euc_accs = []
            for seed in range(1, 11):
                res = run_experiment_for_seed(seed, alpha, gamma, lr)
                coupled_accs.append(np.mean(res['coupled']))
                euc_accs.append(np.mean(res['euclidean']))
            print(f"Alpha={alpha:<5} Gamma={gamma:<5} LR={lr:<5} => Coupled: {np.mean(coupled_accs):.4f}%  Euclidean: {np.mean(euc_accs):.4f}%")
