...
for l_pac in [0.04, 0.06, 0.08, 0.10, 0.12, 0.15, 0.20]:
    for damp in [0.1, 0.3, 0.5, 1.0]:
        det_accs = []
        ens_accs = []
        for seed in SEEDS:
...
print("\n--- Computing Baselines for Seeds 10, 11, 12 ---")
uniform_accs = []
unconstrained_accs = []
rbpm_accs = []
...
for seed in SEEDS:
    ...
    for step in range(50):
        ...
        for k in range(NUM_TASKS):
            logits = forward_functional(cal_feats[k], W_m, B_m)
            ce_loss += torch.clamp(criterion(logits, cal_labels[k]), max=5.0)
        ce_loss /= NUM_TASKS
        reg_loss = 0.0
        for k in range(NUM_TASKS):
    ...
    with torch.no_grad():
        ...
        for l in range(NUM_LAYERS):
            ...
            W_opt.append(W_base[l] + torch.sum(a_l[:, None, None] * V_W[l], dim=0))
            B_opt.append(B_base[l] + torch.sum(a_l[:, None] * V_B[l], dim=0))
        rbpm_accs.append(np.mean(evaluate_merged_model_functional(W_opt, B_opt, test_feats, test_labels)))
...
print(f"RBPM (L1) Mean: {np.mean(rbpm_accs)*100:.3f}%")
