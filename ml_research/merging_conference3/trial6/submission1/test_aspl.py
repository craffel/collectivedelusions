# Empirical Validation of Activation-Space Projection Layers (ASPL)
import math
import random
import numpy as np
import torch
...
# ----------------------------------------------------------------------
# 1. Compute task-specific clean activations and PCA bases U_k^{(l)}
# ----------------------------------------------------------------------
print("Computing clean activations and projection bases...")
clean_bases = {l: {} for l in range(L)} # l = 1 to L-1 (intermediate layer outputs)

# For each task, propagate clean training samples and perform PCA
for k in range(K):
    # Filter training data for task k
    mask = (Y_task_train == k)
    X_k_train = X_train[mask]
    
    # We propagate this training batch clean-ly
    H = X_k_train
    # Save input activations at each layer
    for l in range(L - 1):
        # Clean layer weight for task k
        W_clean = W_base_layers[l] + V_task_layers[k][l]
        H_next = torch.matmul(H, W_clean.t())
        
        # Save output of layer l as activations for layer l+1
        # H_next shape: [N_k, D]
        # Let's compute PCA on H_next to find principal components
        # Center the activations
        H_mean = H_next.mean(dim=0)
        H_centered = H_next - H_mean
        
        # Use full SVD/lowrank to find the principal components of dimension D
        _, _, V_comp = torch.pca_lowrank(H_centered, q=D) # V_comp shape: [D, D]
        clean_bases[l][k] = V_comp # store full basis, slice as needed for d
        
        H = H_next

def run_aspl_ehpb(d_proj=16):
    # Set seeds for reproducibility
    random.seed(42)
    np.random.seed(42)
    torch.manual_seed(42)
    ...
    # Prepare projection bases sliced to dimension d_proj
    # clean_bases_sliced[l][k] has shape [D, d_proj]
    proj_bases = {l: {} for l in range(L - 1)}
    if d_proj > 0:
        for l in range(L - 1):
            for k in range(K):
                # Slice top d_proj components
                proj_bases[l][k] = clean_bases[l][k][:, :d_proj]
    ...
    for epoch in range(15):
        ...
        for b in range(Psi_cal.shape[0]):
            ...
            for l in range(L - 1):
                c_b_l = coeffs[l][b]
                # Softmax the predicted coeffs or keep as linear? Let's softmax for projections
                alpha_b_l = torch.softmax(c_b_l, dim=-1)
                
                U_bl = torch.sum(c_b_l.view(K, 1, 1) * keys_stacked[l], dim=0)
                W_bl = W_base_layers[l] + W_holo_layers[l] * U_bl
                h_b = torch.matmul(h_b, W_bl.t())
                
                # Apply ASPL projection on the pre-activation h_b
                if d_proj > 0:
                    # Construct P_b = sum_{k} alpha_{k} U_k U_k^T
                    # h_b shape: [1, D]
                    # We can compute projected h_b as: sum_k alpha_k * (h_b @ U_k @ U_k^T)
                    h_b_proj = torch.zeros_like(h_b)
                    for k in range(K):
                        U_k = proj_bases[l][k] # Shape: [D, d_proj]
                        # project h_b onto task k subspace: h_b @ U_k @ U_k^T
                        h_b_proj += alpha_b_l[k] * torch.matmul(torch.matmul(h_b, U_k), U_k.t())
                    h_b = h_b_proj
...
# Sweep d_proj values
d_values = [0, 2, 4, 8, 16, 32, 64, 128]
print("\nStarting ASPL Projection Dimension Sweep:")
for d in d_values:
    t0 = time.time()
    accs, mean_acc = run_aspl_ehpb(d_proj=d)
    dt = time.time() - t0
    print(f"  d = {d:<3} | MNIST: {accs[0]:.1f}% | Fashion: {accs[1]:.1f}% | CIFAR: {accs[2]:.1f}% | SVHN: {accs[3]:.1f}% | Joint Mean: {mean_acc:.2f}% | Time: {dt:.2f}s")
