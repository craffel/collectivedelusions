# Empirical Validation of Continuous Cleanup Networks (CCN)
import math
import random
import numpy as np
import torch
...
def run_ccn_ehpb(use_ccn=True, lr_ccn=0.01, epochs_ccn=50, num_samples_ccn=200):
    # Set seeds for reproducibility
    random.seed(42)
    np.random.seed(42)
    torch.manual_seed(42)
    ...
    # Train Router (same fast 15 steps on calibration set)
    ...
    # ----------------------------------------------------------------------
    # 2. Sequential training of CCN linear maps for intermediate layers
    # ----------------------------------------------------------------------
    ccns = {}
    
    if use_ccn:
        print(f"Training Continuous Cleanup Networks using {num_samples_ccn} training samples...")
        
        # Use a subset of training data for CCN training (to avoid overfitting and for speed)
        # We need both clean and noisy propagation
        with torch.no_grad():
            coeffs_train = router(get_psi(X_train[:num_samples_ccn]))
            
        # We will sequentially train CCN at each layer l from 0 to L-2
        # Initialize inputs to layer 0 (which are the original, clean, noise-free input features)
        H_noisy = X_train[:num_samples_ccn].clone() # Shape: [N, D]
        H_clean = X_train[:num_samples_ccn].clone() # Shape: [N, D]
        
        for l in range(L - 1):
            # 1. Compute clean layer targets
            # Since each sample has a specific predicted routing coefficient, we construct
            # the clean merged weight for each sample
            H_clean_next_list = []
            H_noisy_next_list = []
            ...
            H_clean_next = torch.cat(H_clean_next_list, dim=0) # Shape: [N, D]
            H_noisy_next = torch.cat(H_noisy_next_list, dim=0) # Shape: [N, D]
            
            # 2. Train CCN mapping H_noisy_next -> H_clean_next
            # We use a simple linear projection first
            ...
            # 3. For the next layer, update H_noisy with the denoised outputs
            with torch.no_grad():
                H_noisy = ccn_layer(H_noisy_next)
                H_clean = H_clean_next
                
            print(f"  Layer {l:2d} CCN trained | Initial noisy MSE: {ccn_criterion(H_noisy_next, H_clean_next).item():.6f} -> Cleaned MSE: {ccn_criterion(H_noisy, H_clean).item():.6f}")

    # 3. Evaluate with CCNs
...
print("Starting CCN Evaluation:")
print("\n--- Running Baseline EHPB (No CCN) ---")
accs_base, mean_base = run_ccn_ehpb(use_ccn=False)
print(f"Baseline EHPB Accuracies: {[round(a, 1) for a in accs_base]} | Joint Mean: {mean_base:.2f}%")

print("\n--- Running EHPB with Continuous Cleanup Networks (CCN) ---")
accs_ccn, mean_ccn = run_ccn_ehpb(use_ccn=True, lr_ccn=0.01, epochs_ccn=100, num_samples_ccn=400)
print(f"EHPB with CCN Accuracies: {[round(a, 1) for a in accs_ccn]} | Joint Mean: {mean_ccn:.2f}%")
