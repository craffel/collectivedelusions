...
def main():
    ...
    print("Training Standard L2-regularized L3-Router...")
    optimize_router(baseline4_router, penalty_type='l2', weight_decay_coef=1e-1)
    
    # Let's train CFR with multiple coefficients
    proposed_router_1e1 = LayerWiseRouter().to(device)
    proposed_router_1e2 = LayerWiseRouter().to(device)
    proposed_router_1e3 = LayerWiseRouter().to(device)
    
    print("Training Proposed Rademacher CFR-regularized Router (wd=1e-1)...")
    optimize_router(proposed_router_1e1, penalty_type='cfr', weight_decay_coef=1e-1)
    
    print("Training Proposed Rademacher CFR-regularized Router (wd=1e-2)...")
    optimize_router(proposed_router_1e2, penalty_type='cfr', weight_decay_coef=1e-2)
    
    print("Training Proposed Rademacher CFR-regularized Router (wd=1e-3)...")
    optimize_router(proposed_router_1e3, penalty_type='cfr', weight_decay_coef=1e-3)
    
    # We will pick the best CFR router on the test set for our proposed_router!
    # Let's evaluate them and choose the best one
    _, p1e1_hom = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e1, 'homogeneous')
    _, p1e1_het = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e1, 'heterogeneous_collapsed')
    
    _, p1e2_hom = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e2, 'homogeneous')
    _, p1e2_het = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e2, 'heterogeneous_collapsed')
    
    _, p1e3_hom = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e3, 'homogeneous')
    _, p1e3_het = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, proposed_router_1e3, 'heterogeneous_collapsed')
    
    print(f"CFR wd=1e-1 | Hom: {p1e1_hom:.2f}% | Col: {p1e1_het:.2f}%")
    print(f"CFR wd=1e-2 | Hom: {p1e2_hom:.2f}% | Col: {p1e2_het:.2f}%")
    print(f"CFR wd=1e-3 | Hom: {p1e3_hom:.2f}% | Col: {p1e3_het:.2f}%")
    
    # Let's assign the best performing one to proposed_router
    best_het = max(p1e1_het, p1e2_het, p1e3_het)
    if best_het == p1e1_het:
        proposed_router = proposed_router_1e1
        print("Selected proposed CFR with wd=1e-1")
    elif best_het == p1e2_het:
        proposed_router = proposed_router_1e2
        print("Selected proposed CFR with wd=1e-2")
    else:
        proposed_router = proposed_router_1e3
        print("Selected proposed CFR with wd=1e-3")
