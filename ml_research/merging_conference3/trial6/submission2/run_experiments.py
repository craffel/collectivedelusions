...
def main():
    ...
    baseline5_router = LayerWiseRouter().to(device) # Static Layer-Wise Optimized baseline (w=0)
    with torch.no_grad():
        baseline5_router.w.zero_()
    baseline5_router.w.requires_grad = False
    ...
    print("Training Proposed Rademacher CFR-regularized Router...")
    optimize_router(proposed_router, penalty_type='cfr', weight_decay_coef=1e-2)
    
    print("Training Static Layer-Wise (Optimized) Router...")
    optimize_router(baseline5_router, penalty_type='none')
    ...
    b5_hom, b5_hom_avg = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, baseline5_router, 'homogeneous')
    ...
    b5_hets, b5_hets_avg = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, baseline5_router, 'heterogeneous_sample')
    ...
    b5_hetc, b5_hetc_avg = evaluate_on_stream(merged_model, base_vit, test_loaders, mean_feat, P_matrix, baseline5_router, 'heterogeneous_collapsed')
...
| **Static Layer-Wise (Optimized)** | {b5_hom_avg:.2f}% | {b5_hets_avg:.2f}% | {b5_hetc_avg:.2f}% | {b5_hetc_avg - b5_hets_avg:.2f}% |
...
| **Static Layer-Wise (Opt)** | {b5_hom[0]:.2f}% | {b5_hom[1]:.2f}% | {b5_hom[2]:.2f}% | {b5_hom[3]:.2f}% | {b5_hom_avg:.2f}% |
...
| **Static Layer-Wise (Opt)** | {b5_hetc[0]:.2f}% | {b5_hetc[1]:.2f}% | {b5_hetc[2]:.2f}% | {b5_hetc[3]:.2f}% | {b5_hetc_avg:.2f}% |
...
"""
    ...
    methods = ["Static Uniform", "Global Router", "QWS SOTA", "L2 Reg L3", "Static Layer-Wise", "R2D-Merge (Ours)"]
    hom_scores = [u_hom_avg, b2_hom_avg, b3_hom_avg, b4_hom_avg, b5_hom_avg, prop_hom_avg]
    hets_scores = [u_hets_avg, b2_hets_avg, b3_hets_avg, b4_hets_avg, b5_hets_avg, prop_hets_avg]
    hetc_scores = [u_hetc_avg, b2_hetc_avg, b3_hetc_avg, b4_hetc_avg, b5_hetc_avg, prop_hetc_avg]
