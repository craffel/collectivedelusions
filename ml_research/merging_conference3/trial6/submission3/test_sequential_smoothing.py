...
class PhysicalBWS_Router(nn.Module):
    def __init__(self, L=3, G=3, d=4, K=4, activation='Sigmoid', lambda_max=0.3, init_bias=-2.0):
        super().__init__()
        self.L = L
        self.G = G
        self.d = d
...
# --- Router Calibration Training with Sequential Smoothing Regularization ---
def train_physical_router_smooth(router, experts, pcas, calib_data, lambda_smooth=0.0, epochs=120, lr=0.05, lambda_wd=1e-4):
    router.train()
    ...
    for epoch in range(epochs):
        ...
        # 2. Sequential Smoothing Regularization: penalize squared L2 difference between adjacent layers
        smooth_reg = torch.tensor(0.0, requires_grad=True)
        if lambda_smooth > 0.0 and router.G > 1:
            # router.W has shape [G, K, d]
            W_diff = router.W[1:] - router.W[:-1] # [G-1, K, d]
            smooth_reg = smooth_reg + torch.sum(W_diff**2)
            # router.B has shape [G, K]
            B_diff = router.B[1:] - router.B[:-1] # [G-1, K]
            smooth_reg = smooth_reg + torch.sum(B_diff**2)
            
        total_loss = loss + lambda_wd * l2_reg + lambda_smooth * smooth_reg
...
def evaluate_physical_router(router, experts, pcas, test_data, mode='Heterogeneous_B256'):
    router.eval()
    K = len(experts)
...
def main():
    ...
    smooth_scales = [0.0, 1e-4, 1e-3, 1e-2, 1e-1, 1.0]
    
    results = {lam: {'homogeneous': [], 'heterogeneous': []} for lam in smooth_scales}
    
    print("=" * 60)
    print("RUNNING SEQUENTIAL SMOOTHING REGULARIZATION SWEEP")
    print("=" * 60)
    ...
    for seed in seeds:
        ...
        for lam in smooth_scales:
            # Unshared physical BWS M=1 (L=3, G=3)
            router = PhysicalBWS_Router(L=3, G=3, d=4, K=4, activation='Sigmoid', init_bias=-2.0)
            train_physical_router_smooth(router, experts, pcas, calib_data, lambda_smooth=lam, lr=0.05, lambda_wd=1e-4)
            
            _, homo_mean = evaluate_physical_router(router, experts, pcas, test_data, mode='Homogeneous_B256')
            _, hetero_mean = evaluate_physical_router(router, experts, pcas, test_data, mode='Heterogeneous_B256')
            
            results[lam]['homogeneous'].append(homo_mean)
            results[lam]['heterogeneous'].append(hetero_mean)
            
            print(f"  lambda_smooth = {lam:g} | Homo: {homo_mean:.2f}% | Hetero: {hetero_mean:.2f}%")
            
    print("\n" + "=" * 60)
    print("SUMMARY STATISTICS FOR SEQUENTIAL SMOOTHING SWEEP")
    print("=" * 60)
    
    summary_out = {}
    for lam in smooth_scales:
        homo_mean = np.mean(results[lam]['homogeneous'])
        homo_std = np.std(results[lam]['homogeneous'])
        hetero_mean = np.mean(results[lam]['heterogeneous'])
        hetero_std = np.std(results[lam]['heterogeneous'])
        
        print(f"lambda_smooth = {lam:g} | Homogeneous: {homo_mean:.2f} \u00b1 {homo_std:.2f}% | Heterogeneous: {hetero_mean:.2f} \u00b1 {hetero_std:.2f}%")
    ...
    with open('physical_smoothing_results.json', 'w') as f:
        json.dump(summary_out, f, indent=4)
    print("\nSaved sequential smoothing sweep results to 'physical_smoothing_results.json'")
