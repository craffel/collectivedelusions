...
def run_test(seed, use_entropy_gating=True):
    ...
    for l in range(4, L + 1):
        ...
        if use_entropy_gating:
            # Hard-gate off updates when routing entropy exceeds 0.95 to shield the state
            delta[H > 0.95] = 0.0
...
accs_no_gate, sims_no_gate = [], []
accs_gate, sims_gate = [], []
for seed in range(10):
    acc, sim = run_test(seed, use_entropy_gating=False)
    accs_no_gate.append(acc)
    sims_no_gate.append(sim)
    
    acc, sim = run_test(seed, use_entropy_gating=True)
    accs_gate.append(acc)
    sims_gate.append(sim)

print(f"No Gating: Joint Mean Acc: {np.mean(accs_no_gate)*100:.2f}% ± {np.std(accs_no_gate)*100:.2f}%, Similarity: {np.mean(sims_no_gate):.4f} ± {np.std(sims_no_gate):.4f}")
print(f"With Gating: Joint Mean Acc: {np.mean(accs_gate)*100:.2f}% ± {np.std(accs_gate)*100:.2f}%, Similarity: {np.mean(sims_gate):.4f} ± {np.std(sims_gate):.4f}")
