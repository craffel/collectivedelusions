...
def run_evaluation_on_seed(seed, rbf_gamma):
    ...
    def get_kernel_coords(z):
        ...
        for k in range(K):
            ...
            k_x = compute_rbf_kernel(z_norm, cal_pts)
            alpha = kpca_eigenvectors[k]
            coords[:, k] = np.abs(np.dot(k_x, alpha))
        return coords
        
    # Measure Kernel PCA inference time on test set
    t0_kpca = time.time()
    ...
    # ----------------------------------------------------
    # We train the MLP on sub_distorted using InfoNCE (standard basis anchors)
    ...
    # InfoNCE loss with standard basis anchors can be implemented using cross-entropy with temperature
    tau = 0.1
...
best_gamma = 2.0
linear_accs, kernel_accs, cph_accs = [], [], []
kpca_lats, cph_lats = [], []
...
print("\n" + "="*60)
print("COMPREHENSIVE PROJECTION METHOD COMPARISON")
print("="*60)
