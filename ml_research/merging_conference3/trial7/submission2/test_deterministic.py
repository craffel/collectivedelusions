...
print("Running Deterministic Evaluator...")
for seed in seeds:
    ...
    # PFSR
    acc_pfsr = evaluate_method_online("PFSR_MBH", C_prototypes, None, test_f, test_l, test_t, batch_size=256, seed=seed)
    pfsr_results.append(acc_pfsr)
    
    # Online Estimated Covariance Shrinkage & Alignment with alpha=0.2
    U_est_list = []
    ...
    acc_online = evaluate_method_online("FIOSR-Online", C_prototypes, df_online, test_f, test_l, test_t, batch_size=256, U_est=U_est_list, seed=seed)
    fiosr_online_results.append(acc_online)
    
    print(f"Seed {seed:<2} | PFSR+MBH: {acc_pfsr:.2f}% | FIOSR-Online (Ours, alpha=0.2): {acc_online:.2f}%")

print("\n=== DETERMINISTIC PAIRWISE RESULTS ===")
print(f"PFSR + MBH (Flat Cosine):               {np.mean(pfsr_results):.4f}% +/- {np.std(pfsr_results):.4f}%")
print(f"FIOSR-Online (Ours, alpha=0.2):         {np.mean(fiosr_online_results):.4f}% +/- {np.std(fiosr_online_results):.4f}%")
