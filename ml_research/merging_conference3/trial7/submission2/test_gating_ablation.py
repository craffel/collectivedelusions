...
def get_mbh_coefficients_gated(method_name, feat, C_prototypes, Fisher_M, top_m=None):
    B_size = len(feat)
    ...
    # Apply Top-M expert gating
    if top_m is not None and top_m < K:
        u_gated = torch.full_like(u_calibrated, -1e9)
        top_vals, top_indices = torch.topk(u_calibrated, top_m, dim=-1)
        u_gated.scatter_(-1, top_indices, top_vals)
        u_calibrated = u_gated
...
def evaluate_method_gated(method_name, C_prototypes, Fisher_M, test_features, test_labels, test_tasks, batch_size, top_m):
    total_correct = 0
    ...
    for k in range(K):
        ...
        for i in range(0, num_task_samples, batch_size):
            ...
            alpha = get_mbh_coefficients_gated(method_name, batch_feat, C_prototypes, Fisher_M, top_m=top_m)
...
top_m_values = [1, 2, 3, 4]

print("Running Top-M Gating Ablation over 10 seeds...")
gating_results = {m: [] for m in top_m_values}
pfsr_gating_results = {m: [] for m in top_m_values}
...
for seed in seeds:
    ...
    for m in top_m_values:
        acc_fiosr = evaluate_method_gated("FIOSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=256, top_m=m)
        gating_results[m].append(acc_fiosr)
        
        acc_pfsr = evaluate_method_gated("PFSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=256, top_m=m)
        pfsr_gating_results[m].append(acc_pfsr)

print("\n=== TOP-M GATING RESULTS (HOMOGENEOUS) ===")
print("FIOSR (Ours):")
for m in top_m_values:
    means = np.mean(gating_results[m])
    stds = np.std(gating_results[m])
    print(f"  M = {m}: {means:.4f}% +/- {stds:.4f}%")

print("PFSR (Baseline):")
for m in top_m_values:
    means = np.mean(pfsr_gating_results[m])
    stds = np.std(pfsr_gating_results[m])
    print(f"  M = {m}: {means:.4f}% +/- {stds:.4f}%")
