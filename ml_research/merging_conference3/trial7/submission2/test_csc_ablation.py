...
def get_mbh_coefficients(method_name, feat, C_prototypes, Fisher_M, use_csc):
    B_size = len(feat)
...
def evaluate_method(method_name, C_prototypes, Fisher_M, test_features, test_labels, test_tasks, batch_size, is_homogeneous, use_csc):
    total_correct = 0
    ...
    if is_homogeneous:
        ...
        for k in range(K):
            ...
            for i in range(0, num_task_samples, batch_size):
                ...
                alpha = get_mbh_coefficients(method_name, batch_feat, C_prototypes, Fisher_M, use_csc)
...
print("Running CSC Ablation Experiment over 10 seeds...")
for seed in seeds:
    ...
    # Eval with CSC
    acc_csc = evaluate_method("FIOSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=256, is_homogeneous=True, use_csc=True)
    results_with_csc.append(acc_csc)
    
    # Eval without CSC
    acc_no_csc = evaluate_method("FIOSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=256, is_homogeneous=True, use_csc=False)
    results_no_csc.append(acc_no_csc)

print("\nRESULTS (HOMOGENEOUS):")
print(f"FIOSR with CSC: {np.mean(results_with_csc):.4f}% +/- {np.std(results_with_csc):.4f}%")
print(f"FIOSR without CSC: {np.mean(results_no_csc):.4f}% +/- {np.std(results_no_csc):.4f}%")

# Let's also do a heterogeneous evaluation with B=1
results_with_csc_b1 = []
results_no_csc_b1 = []
...
for seed in seeds:
    ...
    acc_csc_b1 = evaluate_method("FIOSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=1, is_homogeneous=False, use_csc=True)
    results_with_csc_b1.append(acc_csc_b1)
    
    acc_no_csc_b1 = evaluate_method("FIOSR", C_prototypes, Fisher_M, test_f, test_l, test_t, batch_size=1, is_homogeneous=False, use_csc=False)
    results_no_csc_b1.append(acc_no_csc_b1)

print("\nRESULTS (HETEROGENEOUS, B=1):")
print(f"FIOSR with CSC: {np.mean(results_with_csc_b1):.4f}% +/- {np.std(results_with_csc_b1):.4f}%")
print(f"FIOSR without CSC: {np.mean(results_no_csc_b1):.4f}% +/- {np.std(results_no_csc_b1):.4f}%")
