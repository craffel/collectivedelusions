...
def run_experiment_on_seed(seed):
    ...
    # --- Projection Dimension Ablation Sweep on this seed ---
    ablation_results = {}
...
def main():
    ...
    print(f"Running physical PyTorch experiment across {len(seeds)} random seeds...")
    ...
    for s_idx, s in enumerate(seeds):
        ...
        print(f"Seed {s_idx+1}/{len(seeds)} (Seed: {s}) completed in {time.time()-t0:.2f}s.")
    ...
    print("="*85)
    print(f"{'Method':<35} | {'Mean Joint Accuracy (%)':<30} | {'Std Dev (%)':<12}")
    print("="*85)
    for m, (mean, std) in aggregated_main.items():
        print(f"{m:<35} | {mean:.2f}%                        | {std:.2f}%")
    print("="*85)
    
    print("\n" + "="*110)
    print("PROJECTION DIMENSION ABLATION SWEEP WITH STATISTICAL SIGNIFICANCE (10 SEEDS)")
    print("="*110)
    print(f"{'Proj Dim':<10} | {'Unregularized':<15} | {'L2 Regularized':<15} | {'TSAR':<15} | {'SR3-F (Ours)':<15} | {'SR3-S (Ours)':<15} | {'SR3-H (Ours)':<15}")
    print("="*110)
    for pd in pds:
        ...
        print(f"{pd:<10} | {unreg:<15} | {l2:<15} | {tsar:<15} | {sr3_f:<15} | {sr3_s:<15} | {sr3_h:<15}")
    print("="*110)
