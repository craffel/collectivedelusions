...
for l in range(L):
    for k in range(K):
        Gamma_F[l, k] = torch.sum(V_matrices[l, k] ** 2)
        svals = torch.linalg.svdvals(V_matrices[l, k])
        Gamma_S[l, k] = svals[0] ** 2
...
def compute_test_distances(alpha, router=None):
    ...
    noise_factor = 0.05
    mean_V_k_F = torch.mean(torch.sqrt(Gamma_F), dim=0)
...
def calibrate_router(reg_type="none", lambda_reg=0.1, lr=0.1, epochs=600):
    ...
    for epoch in range(epochs):
        ...
        elif reg_type == "sr3_semi_linear":
            loss_reg = lambda_reg * torch.sum(torch.sqrt(Gamma_F) * (W_squared + B_squared))
        elif reg_type == "sr3_s_semi_linear":
            loss_reg = lambda_reg * torch.sum(torch.sqrt(Gamma_S) * (W_squared + B_squared))
...
print("Running fine sweeps...")
run_sweep("l2", [1e-5, 3e-5, 5e-5, 8e-5, 1e-4, 2e-4])
run_sweep("sr3_semi_linear", [1e-6, 5e-6, 1e-5, 2e-5, 4e-5, 6e-5, 8e-5, 1e-4])
run_sweep("sr3_s_semi_linear", [1e-5, 3e-5, 5e-5, 8e-5, 1e-4, 2e-4, 3e-4, 5e-4])
