...
# We will copy run_experiments.py and implement layer-wise top-M masking inside run_hypermerge.
# Let's write the complete code with this minor modification.
...
def run_regime(subspaces, c_hyperbolic=0.1, tau=0.05, mask_layers=True):
    ...
    def run_hypermerge(X, hca_centroids, c=0.1, tau=0.05, M=2):
        ...
        for l in range(L):
            ...
            for b in range(B):
                ...
                # Check if we should mask layer activations
                if mask_layers:
                    val, idx = torch.topk(alpha[b], M)
                    alpha_b_m = alpha[b].clone()
                    mask = torch.zeros(K)
                ...
                for k in range(K):
                    if alpha_b_m[k] > 0:
                        weight = alpha_b_m[k] * gamma_list[k]
    ...
    acc_sable_e = compute_acc(run_sable_early(homo_samples, Eucl_centroids, tau), homo_labels)
    acc_sps = compute_acc(run_sps_zca(homo_samples, Eucl_centroids, tau), homo_labels)
    acc_hm = compute_acc(run_hypermerge(homo_samples, HCA_centroids, c_hyperbolic, tau), homo_labels)
    
    return acc_sable_e, acc_sps, acc_hm

# Run evaluations for overlapping subspaces
subspaces_overlap = {
    0: (0, 96),
    1: (32, 128),
    2: (64, 160),
...
print("Running with mask_layers=True (LAYER-WISE TOP-M MASKING):")
for c in [0.01, 0.05, 0.1, 0.2, 0.3, 0.5]:
    acc_sable, acc_sps, acc_hm = run_regime(subspaces_overlap, c_hyperbolic=c, tau=0.05, mask_layers=True)
    print(f"OVERLAP c={c:.2f} -> SABLE: {acc_sable*100:.2f}%, SPS-ZCA: {acc_sps*100:.2f}%, HyperMerge: {acc_hm*100:.2f}%")

print("\nRunning ORTHOGONAL with mask_layers=True:")
for c in [0.01, 0.05, 0.1, 0.2, 0.3, 0.5]:
    acc_sable, acc_sps, acc_hm = run_regime(subspaces_orth, c_hyperbolic=c, tau=0.05, mask_layers=True)
    print(f"ORTH c={c:.2f} -> SABLE: {acc_sable*100:.2f}%, SPS-ZCA: {acc_sps*100:.2f}%, HyperMerge: {acc_hm*100:.2f}%")
