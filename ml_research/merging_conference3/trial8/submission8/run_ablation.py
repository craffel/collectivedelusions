...
class ExperimentRunner:
    ...
    def run_evaluation(self, N_calib, noise_var, n_components, use_shrunk=True, seed=42):
        ...
        for task in TASKS:
            ...
            if use_shrunk:
                gmm = ShrunkGMM(n_components=n_components, target_type='global_diagonal', random_state=seed, reg_covar=1e-5)
            else:
                gmm = GaussianMixture(n_components=n_components, covariance_type='diag', random_state=seed, reg_covar=1e-5)
            gmm.fit(X_calib)
            models[task] = gmm
...
def main():
    ...
    print("Ablation Study: M=1 vs M=2 under Global Diagonal Shrinkage (N=64, sigma^2=0.05)")
    print("-" * 60)
    for model_name, use_shrunk, n_comps in [
        ("Unregularized M=1", False, 1),
        ("Unregularized M=2", False, 2),
        ("SRC-DE (Ours) M=1", True, 1),
        ("SRC-DE (Ours) M=2", True, 2)
    ]:
        aucs = []
        for s in seeds:
            auc = runner.run_evaluation(N_calib=64, noise_var=0.05, n_components=n_comps, use_shrunk=use_shrunk, seed=s)
            aucs.append(auc)
        print(f"  {model_name:25s} | AUC = {np.mean(aucs):.4f} +- {np.std(aucs):.4f}")
