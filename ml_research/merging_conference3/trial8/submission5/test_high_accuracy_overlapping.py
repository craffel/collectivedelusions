...
def generate_sandbox_data(seed, overlap_bleed=0.08):
    ...
    # Noise scales reduced to simulate highly optimized experts:
    noise_scales = [0.01, 0.03, 0.05, 0.12]
...
def run_simulation_seed_high_accuracy(seed, use_base_backbone=True):
    ...
    def evaluate_test():
        ...
        task_accs = {}
        for method_name, logits_tensor in [("ceil", logits_ceil), ("unif", logits_unif), ("pear", logits_pear), ("sable", logits_sable)]:
            task_accs[method_name] = {}
            for k in range(K):
                task_mask = (Y_test_task == k)
                pred = logits_tensor[task_mask].argmax(dim=-1)
                correct = (pred == Y_test_class[task_mask]).float().mean().item() * 100
                task_accs[method_name][k] = correct
...
def main():
    ...
    for s in seeds:
        ...
        res = run_simulation_seed_high_accuracy(s)
        all_results.append(res)
        
    print("\n================== AVERAGED HIGH-ACCURACY RESULTS ==================\n")
    baselines = ["ceil", "unif", "pear", "sable"]
    ...
    for b in baselines:
        ...
        task_str_list = []
        for k in range(K):
            t_accs = [res["task_accs"][b][k] for res in all_results]
            t_mean = np.mean(t_accs)
            t_std = np.std(t_accs)
            task_str_list.append(f"{task_names[k]}: {t_mean:.2f}% ± {t_std:.2f}%")
            
        print(f"{b:<10}: Mean Accuracy = {mean_acc:.2f}% ± {std_acc:.2f}%")
        print(f"            Task breakdown: {', '.join(task_str_list)}")
