...
print("="*80)
print("REAL-WORLD 4-TASK LORA ENSEMBLING WITH PEAR & ELFT (EARLY-LAYER FREEZING)")
print("="*80)
...
# 5. Train Task-Specific LoRA Adapters and Heads UNDER ELFT
# Under ELFT, we freeze Blocks 0 and 1 (leaving their LoRAs as zero) and only train Blocks 2 to 11.
criterion = nn.CrossEntropyLoss()

for name in task_names:
    print(f"\nTraining {name} Task-Specific Adapter (Blocks 2--11 only, ELFT) and Head...")
    
    # Gather parameters to optimize: only LoRA adapters for Block 2 to Block 11
    params = []
    for l in range(2, 12):
        params.append(wrapped_qkvs[l].lora_A[name])
        params.append(wrapped_qkvs[l].lora_B[name])
...
# 6. Evaluation on Heterogeneous Test Set under ELFT
print("\nPreparing 4-Task Heterogeneous Test Set...")
test_batches = {}
for name in task_names:
...
evaluate_method_accuracy("Static Uniform + ELFT", get_uniform_alphas, dynamic_heads=True)

# --- Method B: SABLE SOTA (Late Adaptation) ---
# SABLE results remain exactly the same as leaving all blocks unadapted, but we can evaluate it under ELFT-trained heads.
with torch.no_grad():
    results["SABLE SOTA + ELFT"] = {}
    total_correct = 0
    ...
    for k, name in enumerate(task_names):
        ...
        results["SABLE SOTA + ELFT"][name] = acc
        total_correct += correct
    results["SABLE SOTA + ELFT"]["Joint Mean"] = total_correct / (K * N_test) * 100
    reset_alphas()

# --- Method C: PEAR + ELFT (Ours) ---
def get_pear_alphas():
    return pear_alphas
evaluate_method_accuracy("PEAR (Ours) + ELFT", get_pear_alphas, dynamic_heads=True)

# --- Method D: Expert Ceiling + ELFT (Perfect Routing) ---
def get_ceiling_alphas():
    ...
    return alphas
evaluate_method_accuracy("Expert Ceiling + ELFT", get_ceiling_alphas, dynamic_heads=False)

# Format and display table
print("\n" + "="*90)
print(f"{'REAL-WORLD LORA ENSEMBLING RESULTS WITH ELFT (EARLY-LAYER FREEZING)':^90}")
print("="*90)
print(f"{'Method':25} | " + " | ".join([f"{n:12}" for n in task_names]) + " | Joint Mean")
print("-"*90)
for method in ["Static Uniform + ELFT", "SABLE SOTA + ELFT", "PEAR (Ours) + ELFT", "Expert Ceiling + ELFT"]:
    res = results[method]
    row = f"{method:25} | " + " | ".join([f"{res[n]:11.2f}%" for n in task_names]) + f" | {res['Joint Mean']:10.2f}%"
    print(row)
print("="*90)

# Save results to a file for referencing in our paper
with open("real_world_lora_results_elft.txt", "w") as f:
    for m in ["Static Uniform + ELFT", "SABLE SOTA + ELFT", "PEAR (Ours) + ELFT", "Expert Ceiling + ELFT"]:
        f.write(f"--- {m} ---\n")
        f.write(f"Joint Mean: {results[m]['Joint Mean']:.2f}%\n")
        for n in task_names:
            f.write(f"{n}: {results[m][n]:.2f}%\n")

print("\nSuccessfully saved ELFT real-world results to 'real_world_lora_results_elft.txt'")
