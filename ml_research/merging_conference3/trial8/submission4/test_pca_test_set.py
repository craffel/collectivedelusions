...
# Compute PCA norms on test set
test_pca_norms = torch.zeros(1000, K)
for b in range(K):
    # e_b = || z_test @ V_b ||_2
    test_pca_norms[:, b] = (z_test @ V_pca_cent[b]).norm(dim=1)

# Predict routing on test set with tau = 1.0 (raw Softmax or argmax)
preds_test = torch.argmax(test_pca_norms, dim=1)
acc_test = (preds_test == test_y).float().mean().item() * 100.0
print(f"Routing accuracy on unseen test set: {acc_test:.2f}%")

# Break down routing accuracy by task
for k in range(K):
    mask = (test_y == k)
    task_acc = (preds_test[mask] == k).float().mean().item() * 100.0
    print(f"  Task {k} (MNIST/F-MNIST/CIFAR/SVHN) routing accuracy: {task_acc:.2f}%")
