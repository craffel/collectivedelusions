...
print(f"ID Mean LH (192D): {id_lh.mean().item():.3f} | Std: {id_lh.std().item():.3f}")
print(f"OOD Mean LH (192D): {ood_lh.mean().item():.3f} | Std: {ood_lh.std().item():.3f}")
...
# Let's find min and max LH to determine sweep bounds
min_lh = all_lh.min().item()
max_lh = all_lh.max().item()
print(f"LH Range: [{min_lh:.1f}, {max_lh:.1f}]")

thresholds = np.linspace(min_lh, max_lh, 21)
print("\n192D GMM Threshold Sweep:")
