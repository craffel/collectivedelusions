...
class TestTimeModelMergerVariants(TestTimeModelMerger):
    def evaluate_stream_variant(self, stream, variant_name, lr_base=0.1, beta1=0.9, beta2=0.99, eps=1e-8):
        ...
        for step, (images, labels, task_id) in enumerate(stream):
            ...
            if variant_name == 'AdaSNR_Standard':
                # The baseline AdaSNR implementation
            ...
            elif variant_name == 'AdaSNR_TC':
                # Task-conditioned SNR (separate tracking for each task)
            ...
            elif variant_name == 'AdaSNR_Wide':
                # Wider range SNR using proper standard deviation estimation
                m = beta1 * m + (1.0 - beta1) * grad
                v = beta2 * v + (1.0 - beta2) * grad.pow(2)
                m_hat = m / (1.0 - beta1 ** t_step)
                v_hat = v / (1.0 - beta2 ** t_step)
                ...
                # Dynamic range scaling via sigmoid
                scale = 2.0 * torch.sigmoid(snr) - 1.0 # Goes from 0.0 to 1.0
                # Prevent freeze
                alpha0 = 0.1
                scale = alpha0 + (1.0 - alpha0) * scale
                lambdas_raw.data -= lr_base * scale * grad
                
            elif variant_name == 'AdaSNR_TC_Wide':
                # Task-conditioned AND Wide Range SNR
                task_steps[task_id] += 1
                curr_t = task_steps[task_id]
...
def main():
    ...
    print(f"Running variants on device: {device}")
    
    torch.backends.cudnn.enabled = False
    alt_stream, seq_stream = get_test_streams(batch_size=64)
    merger = TestTimeModelMergerVariants(device)
    
    variants = ['AdaSNR_Standard', 'AdaSNR_TC', 'AdaSNR_Wide', 'AdaSNR_TC_Wide']
    lr_grid = [0.05, 0.1, 0.2, 0.5, 1.0, 2.0]
    ...
    for var in variants:
        ...
        for lr in lr_grid:
            ...
            print(f"  LR: {lr:<4} | Alt: {avg_alt:.2f}% | Seq: {avg_seq:.2f}%")
            results[var]['alt'].append((avg_alt, lr))
            results[var]['seq'].append((avg_seq, lr))
            
    print("\n" + "="*80)
    print("BEST RESULTS FOR EACH VARIANT")
    print("="*80)
    for var in variants:
        best_alt = max(results[var]['alt'], key=lambda x: x[0])
        best_seq = max(results[var]['seq'], key=lambda x: x[0])
        print(f"Variant: {var:<18} | Alt Best: {best_alt[0]:.2f}% (LR={best_alt[1]}) | Seq Best: {best_seq[0]:.2f}% (LR={best_seq[1]})")
    print("="*80)
