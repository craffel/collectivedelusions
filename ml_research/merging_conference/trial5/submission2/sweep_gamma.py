...
def main():
    ...
    # Setup evaluation streams (exactly as in run_experiments.py)
    batch_size = 32
    num_batches = 150
    ...
    # Evaluation function for a specific gamma (with alpha=1.0)
    def evaluate_fpca_gamma(stream, gamma, corruption='gaussian_noise'):
        alpha = 1.0
        ...
        for b_idx, (batch_data, true_task) in enumerate(stream):
            ...
            loss_total = loss_ent + 0.1 * loss_contra
    ...
    # Running gamma sweep
    gammas = [0.0, 0.5, 0.7, 0.8, 0.85, 0.9, 0.95]
    results_alt = {}
    results_seq = {}
    
    print("\n--- Running Gamma Sweep (Gaussian Noise Corruption) ---")
    for gamma in gammas:
        print(f"Evaluating gamma={gamma}...")
        acc_alt = evaluate_fpca_gamma(stream_alternating, gamma, 'gaussian_noise')
        acc_seq = evaluate_fpca_gamma(stream_sequential, gamma, 'gaussian_noise')
        results_alt[gamma] = acc_alt
        results_seq[gamma] = acc_seq
        print(f"Gamma {gamma:.2f} -> Alternating Stream: {acc_alt:.2f}%, Sequential Stream: {acc_seq:.2f}%")
        
    print("\n--- Final Results Summary (Gaussian Noise) ---")
    print("| Gamma (Confidence Threshold) | Alternating Stream Accuracy | Sequential Stream Accuracy |")
    print("| :---: | :---: | :---: |")
    for gamma in gammas:
        print(f"| {gamma:.2f} | {results_alt[gamma]:.2f}% | {results_seq[gamma]:.2f}% |")
