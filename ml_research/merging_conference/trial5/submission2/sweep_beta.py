...
def main():
    ...
    # Evaluation function for a specific beta (with alpha=1.0, gamma=0.85)
    def evaluate_fpca_beta(stream, beta, corruption='gaussian_noise'):
        alpha = 1.0
        gamma = 0.85
    ...
    print("\n--- Running Beta Sweep (Gaussian Noise Corruption) ---")
    for beta in betas:
        print(f"Evaluating beta={beta}...")
        acc_alt = evaluate_fpca_beta(stream_alternating, beta, 'gaussian_noise')
        acc_seq = evaluate_fpca_beta(stream_sequential, beta, 'gaussian_noise')
        results_alt[beta] = acc_alt
        results_seq[beta] = acc_seq
        print(f"Beta {beta:.2f} -> Alternating Stream: {acc_alt:.2f}%, Sequential Stream: {acc_seq:.2f}%")
        
    print("\n--- Final Results Summary (Gaussian Noise) ---")
    print("| Beta (Contrastive Loss Weight) | Alternating Stream Accuracy | Sequential Stream Accuracy |")
    print("| :---: | :---: | :---: |")
    for beta in betas:
        print(f"| {beta:.2f} | {results_alt[beta]:.2f}% | {results_seq[beta]:.2f}% |")
