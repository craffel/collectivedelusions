...
def main():
    ...
    # Evaluation function for a specific eta
    def evaluate_fpca_eta(stream, eta, corruption='gaussian_noise'):
    ...
    print("\n--- Running Eta (Base Learning Rate) Sweep (Gaussian Noise Corruption) ---")
    for eta in etas:
        print(f"Evaluating eta={eta}...")
        acc_alt = evaluate_fpca_eta(stream_alternating, eta, 'gaussian_noise')
        acc_seq = evaluate_fpca_eta(stream_sequential, eta, 'gaussian_noise')
        results_alt[eta] = acc_alt
        results_seq[eta] = acc_seq
        print(f"Eta {eta} -> Alternating Stream: {acc_alt:.2f}%, Sequential Stream: {acc_seq:.2f}%")
        
    print("\n--- Final Results Summary (Gaussian Noise) ---")
    print("| Base Learning Rate (eta) | Alternating Stream Accuracy | Sequential Stream Accuracy |")
    print("| :---: | :---: | :---: |")
    for eta in etas:
        print(f"| {eta} | {results_alt[eta]:.2f}% | {results_seq[eta]:.2f}% |")
