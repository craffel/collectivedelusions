...
def main():
    ...
    # Evaluation function for a specific alpha
    def evaluate_fpca_alpha(stream, alpha, corruption='gaussian_noise'):
    ...
    print("\n--- Running Alpha Sweep (Gaussian Noise Corruption) ---")
    for alpha in alphas:
        print(f"Evaluating alpha={alpha}...")
        acc_alt = evaluate_fpca_alpha(stream_alternating, alpha, 'gaussian_noise')
        acc_seq = evaluate_fpca_alpha(stream_sequential, alpha, 'gaussian_noise')
        results_alt[alpha] = acc_alt
        results_seq[alpha] = acc_seq
        print(f"Alpha {alpha:.2f} -> Alternating Stream: {acc_alt:.2f}%, Sequential Stream: {acc_seq:.2f}%")
        
    print("\n--- Final Results Summary (Gaussian Noise) ---")
    print("| Alpha (Damping Factor) | Alternating Stream Accuracy | Sequential Stream Accuracy |")
    print("| :---: | :---: | :---: |")
    for alpha in alphas:
        print(f"| {alpha:.2f} | {results_alt[alpha]:.2f}% | {results_seq[alpha]:.2f}% |")
