...
def main():
    ...
    # Evaluation function
    def evaluate_fpca(stream, prototypes_experts, joint_fisher_scalar, corruption='gaussian_noise'):
        alpha = 1.0
        gamma = 0.85
        beta = 0.1
    ...
    n_cals = [50, 100, 200, 500]
    results_alt = {}
    results_seq = {}
    
    print("\n--- Running Calibration Sample Size Sweep (Gaussian Noise Corruption) ---")
    
    for n_cal in n_cals:
        print(f"\nEvaluating with N_cal={n_cal}...")
        
        # Slicing calibration subsets
        mnist_calib = Subset(mnist_train_full, list(range(10000, 10000 + n_cal)))
        fmnist_calib = Subset(fmnist_train_full, list(range(10000, 10000 + n_cal)))
        kmnist_calib = Subset(kmnist_train_full, list(range(10000, 10000 + n_cal)))
        ...
        acc_alt = evaluate_fpca(stream_alternating, prototypes_experts, joint_fisher_scalar, 'gaussian_noise')
        acc_seq = evaluate_fpca(stream_sequential, prototypes_experts, joint_fisher_scalar, 'gaussian_noise')
        results_alt[n_cal] = acc_alt
        results_seq[n_cal] = acc_seq
        print(f"N_cal {n_cal} -> Alternating Stream: {acc_alt:.2f}%, Sequential Stream: {acc_seq:.2f}%")
        
    print("\n--- Final Results Summary (Gaussian Noise) ---")
    print("| Calibration Samples (N_cal) | Alternating Stream Accuracy | Sequential Stream Accuracy |")
    print("| :---: | :---: | :---: |")
    for n_cal in n_cals:
        print(f"| {n_cal} | {results_alt[n_cal]:.2f}% | {results_seq[n_cal]:.2f}% |")
