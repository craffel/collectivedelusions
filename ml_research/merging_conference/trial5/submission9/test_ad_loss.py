...
def evaluate_method_ad_loss(method_name, test_batches, experts_sds, base_sd, expert_encoders, expert_heads, joint_fisher, device, lr=0.10, alpha_lfwa=0.5):
    ...
    for t, (imgs, lbls, active_task_idx) in enumerate(test_batches):
        ...
        # Dynamic Dampening based on running average loss (unsupervised AD-Merge)
        current_lr = lr
        if "AD-Merge" in method_name and len(loss_history) >= 10:
            avg_loss_10 = sum(loss_history[-10:]) / 10.0
            if avg_loss_10 > 0.40:
                # High loss indicating alternating/chaotic or unstable stream
                current_lr = 0.0
                # Reset and freeze lambdas at uniform (static)
                with torch.no_grad():
                    merged_model.raw_lambdas.zero_()
