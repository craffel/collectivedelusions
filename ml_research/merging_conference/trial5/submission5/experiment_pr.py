...
def run_evaluation_with_pr():
    ...
    methods = ["cp_cadr", "cp_cadr_with_pr"]
    ...
    # EMA rate for prototype rectification
    alpha_proto = 0.1
    
    for method in methods:
        for stream_name, batches in streams.items():
            for corr in corruptions:
                print(f"Evaluating: {method.upper()} | Stream: {stream_name.upper()} | Corruption: {corr.upper()}")
                ...
                # Copy prototypes so we can modify them dynamically if using PR
                ...
                for batch_idx, (raw_imgs, labels, true_task_idx) in enumerate(batches):
                    ...
                    # Apply Prototype Rectification (PR) if enabled AFTER backward & optimizer step
                    if method == "cp_cadr_with_pr" and confidence_mask.sum() > 0:
                        with torch.no_grad():
                            hc_embeddings = embeddings_norm[confidence_mask] # [N_hc, 128]
                            hc_pred_classes = pred_classes[confidence_mask] # [N_hc]
                            ...
                            for unique_c in torch.unique(hc_pred_classes):
                                ...
                                new_p = (1.0 - alpha_proto) * old_p + alpha_proto * class_emb_mean
    ...
    print("\n" + "="*80)
    print("PROTOTYPE RECTIFICATION EXPERIMENTAL RESULTS")
    print("="*80)
