...
def create_bib():
    bib_content = """@inproceedings{wortsman22,
...
\textbf{Test-Time Model Merging (TTMM):} TTMM combines the parameter efficiency of model merging with the dynamic adaptation of TTA \cite{yang24}. Instead of updating millions of model weights, TTMM optimizes only a few blending coefficients (and layer-wise offsets) on-the-fly for each test batch \cite{hubotter26}. Early work in this direction includes CP-AM \cite{cpam26}, which utilizes contrastive prototypes with an angular margin to create noise-robust feature routing spaces, and FDF-DPA \cite{fdfdpa26}, which proposes fully data-free dynamic prototype adaptation with Kronecker-trace guided feature anchoring. BK-CoMerge \cite{weimar25} unifies confidence-based soft-routing with Batch Normalization statistics fusion and parameter sensitivity preconditioning. However, these methods either depend on specialized pre-training of the expert models (such as angular margin loss in CP-AM) or rely on high-confidence classification head outputs which are highly vulnerable to overconfidence and the entropy feedback trap under severe noise (e.g., FDF-DPA and BK-CoMerge). Our proposed SSCPA is fully post-hoc and is completely immune to the feedback trap, utilizing self-supervised dual-view contrastive feature alignment at test-time.
