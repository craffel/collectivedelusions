...
def run_evaluation_with_params(rho, eta, beta_kl, gamma_c=0.02, Nstep=5):
...
# Run sweep
print("Sweeping hyperparameters...")
rhos = [0.01, 0.02, 0.05, 0.1, 0.2]
etas = [0.005, 0.01, 0.02, 0.05]
beta_kls = [0.05, 0.1, 0.2, 0.5, 1.0]
...
for rho in rhos:
    for eta in etas:
        for beta in beta_kls:
            res = run_evaluation_with_params(rho, eta, beta)
            overall = res[5]
            if overall > best_overall:
                best_overall = overall
                best_params = (rho, eta, beta)
                print(f"NEW BEST! rho={rho}, eta={eta}, beta_kl={beta} => {res[0]:.2f}% / {res[1]:.2f}% / {res[2]:.2f}% / {res[3]:.2f}% / {res[4]:.2f}% | Overall: {overall:.4f}%")
