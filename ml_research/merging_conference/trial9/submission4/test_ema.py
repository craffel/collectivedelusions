...
def evaluate_config_ema(alpha, use_sum=False, use_pseudo_ce=False, damping=0.01):
    ...
    # EMAs for standard and CosFace sensitivities separately
    sens_ema_std = {}
    sens_ema_cos = {}
    ...
    for b_idx, (x, y, seg_name) in enumerate(stream_batches):
        ...
        for (name, _), grad in zip(init_state.items(), grads_sens):
            if grad is not None:
                ...
                raw_sens[name] = sens_val
                total_sens += sens_val
            else:
                raw_sens[name] = 1e-4
                total_sens += 1e-4
                
        # Select correct EMA dict
        ema_dict = sens_ema_std if is_sparse else sens_ema_cos
        
        # Update EMA
        on_the_fly_sens = {}
        total_ema_sens = 0.0
        for name in raw_sens.keys():
            if name not in ema_dict:
                ema_dict[name] = raw_sens[name]
            else:
                ema_dict[name] = alpha * ema_dict[name] + (1.0 - alpha) * raw_sens[name]
            total_ema_sens += ema_dict[name]
            
        # Normalize sensitivities
        for name in raw_sens.keys():
            on_the_fly_sens[name] = ema_dict[name] / (total_ema_sens + 1e-8)
...
alphas = [0.0, 0.3, 0.5, 0.7, 0.8, 0.9, 0.95]
print("Starting EMA Momentum Sweep...")
for alpha in alphas:
    accs = evaluate_config_ema(alpha)
    ...
    print(f"Alpha={alpha:<4} | MNIST_C: {c_mnist:.2f}% | MNIST_N: {n_mnist:.2f}% | Fash_C: {c_fashion:.2f}% | Fash_N: {n_fashion:.2f}% | KMNIST: {n_kmnist:.2f}% | OVERALL: {overall:.2f}%")
