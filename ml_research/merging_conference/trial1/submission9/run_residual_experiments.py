...
class ResidualMLP(nn.Module):
    ...
    def forward(self, x):
        for layer in self.layers[:-1]:
            # Residual connection
            x = x + 0.1 * torch.relu(layer(x))
        x = x + 0.1 * self.layers[-1](x)
        return x
...
def solve_weighted_procrustes(W_target, W0, F, lr=0.03, num_iters=250):
    ...
    for i in range(num_iters):
        ...
        # Riemannian Gradient Clipping to prevent any numerical instability
...
def run_deep_experiment(seed, d=64, num_layers=6, num_tasks=5, rotation_scale=0.1, drift_scale=0.05):
    np.random.seed(seed)
    ...
    # 2. Generate Target Task-Specific Expert MLPs
    experts = []
    ...
    for t_idx, expert in enumerate(experts):
        ...
        for l_idx in range(num_layers):
            ...
            # Clamp minimum to prevent dividing by zero
            expert_fishers[l_idx] = torch.clamp(expert_fishers[l_idx], min=1e-8)
            expert_fishers[l_idx] = torch.sqrt(expert_fishers[l_idx])
            expert_fishers[l_idx] /= expert_fishers[l_idx].mean() # Normalize
    ...
    for t_idx, expert in enumerate(experts):
        ...
        for l_idx in range(num_layers):
            ...
            # Drift is larger in flat directions
            epsilon = 0.05
    ...
    merged_models = {
        ...
        "ties": ResidualMLP(d, num_layers),
        "orthomerge": ResidualMLP(d, num_layers),
        "fomm": ResidualMLP(d, num_layers)
    }
...
if __name__ == "__main__":
    ...
    num_layers = int(sys.argv[1]) if len(sys.argv) > 1 else 6
    rotation_scale = float(sys.argv[2]) if len(sys.argv) > 2 else 0.15
    drift_scale = float(sys.argv[3]) if len(sys.argv) > 3 else 0.04
    
    print(f"=== RUNNING DEEP RESIDUAL MODEL MERGING SIMULATION ===")
    print(f"Layers={num_layers}, Rotation={rotation_scale}, Drift={drift_scale}")
    seeds = [42, 100, 2026, 7, 999]
    all_losses = {key: [] for key in ["pretrained", "ta", "ties", "orthomerge", "fomm"]}
    all_norms = {key: [] for key in ["pretrained", "ta", "ties", "orthomerge", "fomm"]}
    ...
    print("\n=== FINAL DEEP RESIDUAL SIMULATION SUMMARY ===")
    summary = {}
    for key in all_losses.keys():
        summary[key] = {
    ...
    out_name = f"results_residual_{num_layers}_{rotation_scale}_{drift_scale}.json"
