...
def run_sat_symerge(rho=0.08, sam_type='global'):
    torch.manual_seed(42)
    np.random.seed(42)
...
# Run fine-grained rho sweep under seed 42
print("\n--- Running fine-grained rho sweep under seed 42 ---")
rhos = [0.0, 0.001, 0.002, 0.005, 0.01, 0.015, 0.02, 0.03, 0.04, 0.06]
for r in rhos:
    s_type = 'none' if r == 0.0 else 'tensor_wise'
    lambdas, heads = run_sat_symerge(rho=r, sam_type=s_type)
    ...
    print(f"rho={r:.4f} | sam_type={s_type:<11} | Clean: {clean_res['avg']:.2f}% | Noise: {noise_res['avg']:.2f}% | Blur: {blur_res['avg']:.2f}% | Contrast: {contrast_res['avg']:.2f}% | Rotation: {rotation_res['avg']:.2f}% | OOD Avg: {ood_avg:.2f}%")
