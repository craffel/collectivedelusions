...
def main():
    ...
    # We will evaluate 3 configurations:
    # 1. OrthoMerge (Adam)
    # 2. D-SMM (rho=0.10)
    # 3. D-SMM (rho=0.20)
    
    configs = [
        {"name": "OrthoMerge (Standard)", "mode": "orthomerge", "rho": 0.0},
        {"name": "D-SMM (rho=0.10)", "mode": "smm_decoupled", "rho": 0.10},
        {"name": "D-SMM (rho=0.20)", "mode": "smm_decoupled", "rho": 0.20}
    ]
    
    stored_runs = {}
    
    for cfg in configs:
        print(f"\nTraining model for flatness check: {cfg['name']}")
        
        # Task 1
        model1 = torchvision.models.resnet18(weights=torchvision.models.ResNet18_Weights.DEFAULT)
        apply_oft(model1, block_size=block_size)
        ...
        apply_oft(model2, block_size=block_size)
        model2.fc = nn.Linear(model2.fc.in_features, 5)
        model2 = model2.to(device)
        
        if cfg["mode"] == "orthomerge":
            oft_params1 = [p for n, p in model1.named_parameters() if 'q_params' in n or 'fc' in n]
            optimizer1 = torch.optim.Adam(oft_params1, lr=lr)
            train_joint(model1, train_loader1, optimizer1, epochs, device)
            
            oft_params2 = [p for n, p in model2.named_parameters() if 'q_params' in n or 'fc' in n]
            optimizer2 = torch.optim.Adam(oft_params2, lr=lr)
            train_joint(model2, train_loader2, optimizer2, epochs, device, is_task2=True)
            ...
            sam_q1 = SAM(q_params1, torch.optim.Adam, rho=cfg["rho"], lr=lr)
            optimizer_fc1 = torch.optim.Adam(fc_params1, lr=lr)
            train_decoupled(model1, train_loader1, sam_q1, optimizer_fc1, epochs, device)
            ...
            sam_q2 = SAM(q_params2, torch.optim.Adam, rho=cfg["rho"], lr=lr)
            optimizer_fc2 = torch.optim.Adam(fc_params2, lr=lr)
            train_decoupled(model2, train_loader2, sam_q2, optimizer_fc2, epochs, device, is_task2=True)
        ...
        merged_sd = merge_models(m1_sd, m2_sd)
        
        stored_runs[cfg["name"]] = {
            "merged_sd": merged_sd,
            "fc1_state": fc1_state,
            "fc2_state": fc2_state
        }
        
    # Noise scales to evaluate flatness
    noise_sigmas = [0.0, 0.01, 0.02, 0.05, 0.10, 0.15, 0.20]
    num_trials = 3
    
    flatness_results = {}
    
    # Base model structure for evaluation
    eval_model = torchvision.models.resnet18()
    apply_oft(eval_model, block_size=block_size)
    eval_model.fc = nn.Linear(eval_model.fc.in_features, 5)
    eval_model = eval_model.to(device)
    
    for name, run in stored_runs.items():
        print(f"\n--- Measuring flatness for: {name} ---")
        flatness_results[name] = {}
        
        for sigma in noise_sigmas:
            print(f"Evaluating with noise sigma: {sigma}")
            acc_list = []
            
            for trial in range(num_trials):
                # Copy merged state dict
                perturbed_sd = copy.deepcopy(run["merged_sd"])
                
                # Apply noise only to q_params if sigma > 0
                if sigma > 0.0:
                    for key in perturbed_sd.keys():
                        if 'q_params' in key:
                            perturbed_sd[key] = perturbed_sd[key] + torch.randn_like(perturbed_sd[key]) * sigma
                
                # Evaluate
                eval_model.load_state_dict(perturbed_sd, strict=False)
                
                eval_model.fc.load_state_dict(run["fc1_state"])
                acc1 = evaluate(eval_model, test_loader1, device)
                
                eval_model.fc.load_state_dict(run["fc2_state"])
                acc2 = evaluate(eval_model, test_loader2, device, is_task2=True)
                
                avg_acc = (acc1 + acc2) / 2.0
                acc_list.append(avg_acc)
                
            mean_acc = sum(acc_list) / len(acc_list)
            print(f"Mean Accuracy at sigma {sigma}: {mean_acc:.2f}%")
            flatness_results[name][str(sigma)] = mean_acc
            
    # Save the flatness results
    with open("flatness_results.json", "w") as f:
        json.dump(flatness_results, f, indent=4)
    print("\nFlatness results saved to flatness_results.json successfully!")
