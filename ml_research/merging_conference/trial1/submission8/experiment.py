...
def generate_synthetic_data(num_samples=1000, input_dim=128, num_classes=8, task_id=1):
    # Create random features
    X = torch.randn(num_samples, input_dim)
    # Define a distinct transformation matrix for each task to simulate different domains
    torch.manual_seed(100 + task_id)
    W_task = torch.randn(input_dim, num_classes)
    logits = torch.matmul(X, W_task) + 0.1 * torch.randn(num_samples, num_classes)
    y = torch.argmax(logits, dim=1)
    return X, y

# Generate datasets for Task 1 and Task 2 with 8 classes
X1, y1 = generate_synthetic_data(1000, 128, 8, task_id=1)
X2, y2 = generate_synthetic_data(1000, 128, 8, task_id=2)

train_loader1 = DataLoader(TensorDataset(X1[:800], y1[:800]), batch_size=32, shuffle=True)
val_loader1 = DataLoader(TensorDataset(X1[800:], y1[800:]), batch_size=32, shuffle=False)

train_loader2 = DataLoader(TensorDataset(X2[:800], y2[:800]), batch_size=32, shuffle=True)
val_loader2 = DataLoader(TensorDataset(X2[800:], y2[800:]), batch_size=32, shuffle=False)
...
# Instantiate pre-trained base model
...
# --- 4. Models wrapped with OFT Layers ---
...
# --- 6. Fine-Tuning Baselines ---

# 6a. Euclidean Fine-Tuning for Task Arithmetic
def train_euclidean(train_loader, val_loader, base_model, lr=0.01, epochs=15):
...
print("Training Task 1 (Euclidean)...")
...
# 6b. Orthogonal Fine-Tuning (OFT) (OrthoMerge Baseline)
...
# 6c. Sharpness-Aware Orthogonal (SA-Ortho) Fine-Tuning (Our Proposed Method)
def train_sa_oft(train_loader, val_loader, base_model, lr=0.01, epochs=15, rho=0.05):
    model = OFTMLP(base_model, block_size=8)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()
    ...
    for epoch in range(epochs):
        model.train()
        for inputs, targets in train_loader:
            # First pass: compute gradients at current point
            optimizer.zero_grad()
            outputs = model(inputs)
            loss = criterion(outputs, targets)
            loss.backward()
            
            # Save original parameters and accumulate their gradients
            orig_params = {}
            grads = {}
            ...
            # Compute perturbation epsilon = rho * g / ||g||_2
            # Calculate overall norm of gradients across all parameters
            ...
            # Restore original parameters and use the gradient computed at the perturbed point
            for name, p in model.named_parameters():
...
sa_model1, acc1_sa = train_sa_oft(train_loader1, val_loader1, base_model, rho=0.05)
print(f"Task 1 SA-Ortho Acc: {acc1_sa:.4f}")

print("Training Task 2 (SA-Ortho)...")
sa_model2, acc2_sa = train_sa_oft(train_loader2, val_loader2, base_model, rho=0.05)
print(f"Task 2 SA-Ortho Acc: {acc2_sa:.4f}")


# --- 7. Model Merging and Evaluation ---

# Helper to load weights and evaluate
def evaluate_merged_euclid(model1, model2, base_model, alpha, val_loader1, val_loader2):
    # Euclidean Task Arithmetic Merging
...
def evaluate_merged_oft(model1, model2, base_model, alpha, val_loader1, val_loader2):
    # Orthogonal Model Merging on Riemannian Manifold (averaging Q)
...
# Grid evaluation across alpha values [0.0, 0.1, ..., 1.0]
...
print("\n--- Results Summary (Average Multi-task Accuracy) ---")
...
# --- 8. Save Results and Create Plot ---
...
print("\nPlot saved successfully to results.png")
