...
def run_evaluation(device, corruption='clean'):
    print(f"\n==================================================")
    print(f"Running TTA Evaluation under corruption: {corruption}")
    print(f"==================================================")
    ...
    for task in tasks:
        # We take a subset of 1000 images per task to make evaluation fast but statistically sound
    ...
    for m in methods:
        ...
        # Ensure 100% deterministic, fair comparison by seeding before each method
        torch.manual_seed(42)
        np.random.seed(42)
        
        # Initialize TTA parameters
        # Merging coefficients (tensor-wise uniform initialization)
        keys = list(base_encoder_sd.keys())
        lambdas = torch.full((len(keys), 3), 0.33, dtype=torch.float32, device=device, requires_grad=True)
        
        # Adapted heads (initialized to expert heads)
        adapted_heads = {}
        for task in tasks:
        ...
        # Initialize running Fisher Information
        running_fisher = {}
        ...
        spor_beta = 0.1 # SPOR/RGP base weight
        
        # Metrics trackers
        task_correct = {task: 0 for task in tasks}
        task_total = {task: 0 for task in tasks}
        
        # Interleave batches to simulate multi-task stream
        ...
        for batch_idx in range(max_batches):
            for task_idx, task in enumerate(tasks):
                ...
                # Apply corruption to test stream input
                x_corr = get_corrupted_batch(x, corruption)
                
                # Generate expert pseudo-labels on clean data
                with torch.no_grad():
                    expert_features = expert_encoders[task](x)
                    expert_logits = expert_heads[task](expert_features)
                    expert_probs = F.softmax(expert_logits, dim=1)
                
                # Define helper to reconstruct merged model and get prediction
                def forward_merged(x_input):
                    # Construct merged encoder
                    merged_sd = copy.deepcopy(base_encoder_sd)
                    
                    # Apply Softmax normalization (C-TMM) for our methods
                    if m in ["SATA-SBF (Ours)", "SATA-RGP (Enhanced Ours)"]:
                    ...
                    # Apply state dict to temporary encoder
                ...
                # Evaluation tracking on current batch (before adaptation update on this batch)
                with torch.no_grad():
                ...
                # TTA optimization step
                if m == "Static Merged":
                    continue
                
                optimizer.zero_grad()
                
                # Step 1: Compute self-labeling loss
                ...
                # Compute dynamic regularizer weight based on prediction entropy
                entropy = -torch.sum(probs_merged * torch.log(probs_merged + 1e-8), dim=1).mean()
                log_C = 2.302585  # ln(10)
                ...
                if m == "Standard TTA":
                    ...
                    grad_norm = torch.sqrt(sum(p.grad.norm()**2 for p in active_params if p.grad is not None))
                    if grad_norm > 1e-8:
                        for p in active_params:
                            if p.grad is not None:
                                p.data += rho * p.grad.data / grad_norm
                    ...
                    if m == "SATA-SBF (Ours)":
                        reg_perturbed = compute_spor(adapted_heads[task], expert_heads[task])
                    else:
                        reg_perturbed = compute_rgp(adapted_heads[task], expert_heads[task])
                        
                    total_loss_perturbed = loss_perturbed + (spor_beta_dynamic * reg_perturbed)
        ...
        # Calculate task-wise and average accuracies
        avg_acc = 0.0
        for task in tasks:
            acc = 100.0 * task_correct[task] / task_total[task]
            method_accuracies[m][task] = acc
            avg_acc += acc
            print(f"Task: {task:8s} | Acc: {acc:.2f}%")
        avg_acc /= len(tasks)
...
if __name__ == "__main__":
    ...
    for corr in corruptions:
        all_results[corr] = run_evaluation(device, corruption=corr)
        
    # Format and display final results
    print("\n" + "="*80)
    print("SUMMARY OF AVERAGE MULTI-TASK ACCURACIES")
    print("="*80)
    ...
    for m in methods:
        row = {"Method": m}
        for corr in corruptions:
            row[corr] = f"{all_results[corr][m]['average']:.2f}%"
    ...
    # Save to CSV
    df.to_csv("tta_merging_results.csv", index=False)
    print("\nSaved results to tta_merging_results.csv")
    
    # Save a beautiful bar plot
    plt.figure(figsize=(12, 6))
    x = np.arange(len(corruptions))
    width = 0.12
    
    for i, m in enumerate(methods):
        accs = [all_results[corr][m]['average'] for corr in corruptions]
        plt.bar(x + (i - len(methods)/2 + 0.5) * width, accs, width, label=m)
        
    plt.ylabel('Average Multi-Task Accuracy (%)')
    plt.title('Comparison of TTA Model Merging Methods under Corruptions')
    plt.xticks(x, [c.capitalize() for c in corruptions])
    plt.ylim(30, 100)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.legend(loc='lower left')
    plt.tight_layout()
    plt.savefig("tta_merging_results.png", dpi=150)
    print("Saved evaluation plot to tta_merging_results.png")
