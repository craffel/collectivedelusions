...
def run_tta_rho(loaders, expert_cifar_state, expert_svhn_state, method="cas_merge", rho=0.05, initial_fill=0.0, beta=1.0, corrupt=True):
    torch.manual_seed(42)
    np.random.seed(42)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(42)
...
def main():
    ...
    rhos = [0.0, 0.005, 0.01, 0.02, 0.05, 0.10]
    
    print("--- Evaluating Rho Sweep for SAT-TTA (SAM without CALP) under Fill 0.0 ---")
    for rho in rhos:
        res = run_tta_rho(loaders, expert_cifar_state, expert_svhn_state, method="sat_tta", rho=rho, initial_fill=0.0, beta=0.0, corrupt=True)
        print(f"Rho: {rho:<5} | SAT-TTA Avg: {res['average']:.2f}% (CIFAR: {res['cifar10']:.2f}%, SVHN: {res['svhn']:.2f}%)")
        
    print("\n--- Evaluating Rho Sweep for CAS-Merge (Ours with Relative CALP beta=1.0) under Fill 0.0 ---")
    for rho in rhos:
        res = run_tta_rho(loaders, expert_cifar_state, expert_svhn_state, method="cas_merge", rho=rho, initial_fill=0.0, beta=1.0, corrupt=True)
        print(f"Rho: {rho:<5} | CAS-Merge Avg: {res['average']:.2f}% (CIFAR: {res['cifar10']:.2f}%, SVHN: {res['svhn']:.2f}%)")
