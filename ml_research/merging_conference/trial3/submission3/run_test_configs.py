...
def run_tta_config(loaders, expert_cifar_state, expert_svhn_state, method="cas_merge", initial_fill=0.0, calp_type="relative", beta=1.0, corrupt=True):
    torch.manual_seed(42)
    np.random.seed(42)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(42)
                        ...
                        for m_name, m in merger.base_model.named_modules():
                            if isinstance(m, LoRALinear) and m.current_activations is not None:
                                ...
                                if calp_type == "sum":
                                    ...
                                    loss_calp += torch.mean((diff ** 2) * (var.unsqueeze(0) + 1e-5))
                                elif calp_type == "relative":
...
def main():
    ...
    print("Evaluating Static & SyMerge baselines with initial_fill...")
    for fill in [0.0, 0.5]:
        res_static = run_tta_config(loaders, expert_cifar_state, expert_svhn_state, method="static", initial_fill=fill, corrupt=True)
        res_symerge = run_tta_config(loaders, expert_cifar_state, expert_svhn_state, method="symerge", initial_fill=fill, corrupt=True)
        print(f"Fill: {fill} | Static Avg: {res_static['average']:.2f}% (CIFAR: {res_static['cifar10']:.2f}%, SVHN: {res_static['svhn']:.2f}%)")
        print(f"Fill: {fill} | SyMerge Avg: {res_symerge['average']:.2f}% (CIFAR: {res_symerge['cifar10']:.2f}%, SVHN: {res_symerge['svhn']:.2f}%)")
        
    print("\nEvaluating CAS-Merge with different configs...")
    configs = [
        {"fill": 0.0, "type": "relative", "beta": 0.1},
        {"fill": 0.0, "type": "relative", "beta": 0.5},
        {"fill": 0.0, "type": "relative", "beta": 1.0},
        {"fill": 0.0, "type": "relative", "beta": 5.0},
        {"fill": 0.0, "type": "relative", "beta": 10.0},
        {"fill": 0.0, "type": "mean", "beta": 100.0},
        {"fill": 0.0, "type": "mean", "beta": 500.0},
        {"fill": 0.0, "type": "mean", "beta": 1000.0},
        {"fill": 0.0, "type": "sum", "beta": 0.5},
        {"fill": 0.0, "type": "sum", "beta": 1.0},
        
        {"fill": 0.5, "type": "relative", "beta": 0.5},
        {"fill": 0.5, "type": "relative", "beta": 1.0},
        {"fill": 0.5, "type": "relative", "beta": 5.0},
    ]
    
    for config in configs:
        res = run_tta_config(loaders, expert_cifar_state, expert_svhn_state, method="cas_merge", initial_fill=config["fill"], calp_type=config["type"], beta=config["beta"], corrupt=True)
        print(f"Fill: {config['fill']} | Type: {config['type']:<8} | Beta: {config['beta']:<5} | CAS-Merge Avg: {res['average']:.2f}% (CIFAR: {res['cifar10']:.2f}%, SVHN: {res['svhn']:.2f}%)")
