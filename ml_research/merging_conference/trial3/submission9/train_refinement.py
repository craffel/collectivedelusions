...
# Define the shared encoder architecture
        ...
        for images, labels in test_loader:
            ...
            if corruption is not None:
                if corruption == "noise":
                    ...
                    # Simple average pooling as a proxy for blur
...
def optimize_procrustes(W_0, W_k, F_k, steps=150, lr=0.05, weighted=True, device='cpu'):
    # W_0, W_k, F_k are tensors of shape [out_dim, in_dim, ...]
    # We first reshape them to 2D matrices [out_dim, d]
    ...
    if F_k is not None:
        ...
        # Apply Soft-Bounded log1p normalization to stabilize weighted Procrustes loss
        F_k_2d = torch.log1p(F_k_2d / (F_k_2d.mean() + 1e-8))
    else:
        F_k_2d = torch.ones_like(W_k_2d).to(device)
        
    # Initialize the unconstrained parameter matrix B as zero
    B = nn.Parameter(torch.zeros((out_dim, out_dim), device=device))
    optimizer = torch.optim.Adam([B], lr=lr)
    
    for _ in range(steps):
        optimizer.zero_grad()
        # Create skew-symmetric matrix A
        A = B - B.t()
        # Compute orthogonal rotation matrix R
    ...
    with torch.no_grad():
        ...
        # Reshape residual E back to original shape
...
def merge_models_procrustes(base_encoder, experts, fishers, device, weighted=True, steps=150, lr=0.05, lambda_resid=None):
    ...
    # Store Lie algebra matrices A and residuals E for each expert and each parameter
    ...
    for name in base_state:
        # We only apply Procrustes rotation to weights of Conv2d and Linear layers that are 2D/4D
        ...
        if is_weight and is_multidim:
            ...
            # For biases or 1D tensors, there is no rotation; residual is the full task vector
    ...
    # Now merge by averaging the Lie algebra matrices A and scaling the residuals E
    for name in base_state:
        if A_all[name][0] is not None:
            # Average the Lie algebra matrices
            A_merged = torch.zeros_like(A_all[name][0])
            for A_k in A_all[name]:
                A_merged += A_k
            A_merged /= num_experts
            
            # Sum the residuals and scale
            ...
            # Sum and scale residuals (task vectors) for 1D parameters (biases)
            E_merged = torch.zeros_like(E_all[name][0])
            for E_k in E_all[name]:
                E_merged += E_k
            E_merged *= actual_lambda_resid
...
def merge_models_procrustes_weighted_fusion(base_encoder, experts, fishers, device, weighted=True, steps=150, lr=0.05, lambda_val=0.5, scale_rotation=True):
    ...
    # Now merge by weighting/averaging Lie algebra matrices A and residuals E
    for name in base_state:
        # Check if we have Fisher information
        if weighted and fishers is not None:
            # Compute layer-wise Fisher importances for weighting the Lie algebra
            ...
            # Normalize layer means to sum to 1.0 (with small eps to avoid division by zero)
            sum_means = layer_means.sum() + 1e-8
            layer_weights = layer_means / sum_means  # shape [num_experts]
            
            # Compute parameter-wise Fisher weights for the residuals
            # To handle different shapes, we stack along a new dimension
            stacked_fishers = torch.stack(layer_fishers, dim=0) # [num_experts, out, in, ...]
            # Add eps to sum
            sum_fishers = stacked_fishers.sum(dim=0, keepdim=True) + 1e-8
            param_weights = stacked_fishers / sum_fishers # [num_experts, out, in, ...]
        else:
            layer_weights = torch.ones(num_experts) / num_experts
            param_weights = torch.ones((num_experts,) + base_state[name].shape) / num_experts
            
        if A_all[name][0] is not None:
            # Merge Lie algebra matrices with layer-wise Fisher weights and scaling
            # Note: when weights are uniform, layer_weights[k] * num_experts = 1.0, so this reduces to
            # A_merged = lambda_val * sum(A_k)
            A_merged = torch.zeros_like(A_all[name][0])
            for k in range(num_experts):
                # Scale weight by num_experts to maintain magnitude comparable to simple sum
                w_k = layer_weights[k] * num_experts
                A_merged += w_k * A_all[name][k]
                
            if scale_rotation:
                # Scale the rotation by lambda_val
                A_merged *= lambda_val
            else:
                # Standard averaging (no lambda scaling of rotation)
                A_merged /= num_experts
            
            # Merge residuals with parameter-wise Fisher weights
            E_merged = torch.zeros_like(E_all[name][0])
            for k in range(num_experts):
                # Shape of param_weights[k] matches layer shape. E_all[name][k] also matches layer shape.
                # Scale weight by num_experts to maintain magnitude comparable to simple sum
                w_k_p = param_weights[k] * num_experts
                E_merged += w_k_p * E_all[name][k]
            # Scale merged residuals by lambda_val
            E_merged *= lambda_val
            ...
            # Sum and scale residuals (task vectors) for 1D parameters (biases) using parameter-wise Fisher
            E_merged = torch.zeros_like(E_all[name][0])
            for k in range(num_experts):
                w_k_p = param_weights[k] * num_experts
                E_merged += w_k_p * E_all[name][k]
...
def main():
    ...
    # Evaluate experts (individual baseline)
    ...
    # Define environments (Clean and Corrupted)
    environments = [
    ...
    # We will test several merging strategies
    merging_strategies = {}
    
    # 1. Task Arithmetic with different lambdas
    ...
    # 1.1 DARE with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
    ...
    # 1.2 TIES-Merging with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
    ...
    # 2. Unweighted OrthoMerge with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
    ...
    # 3. Fisher-Weighted Procrustes Alignment (FWPA) [Ours] with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
    ...
    # 4. Fisher-Weighted Procrustes Alignment with Fisher-Weighted Fusion (FWPA-FWF) [Ours] with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
    ...
    # 5. Ablation: FWPA-FWF (No Scale Rotation) with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
        fw_fwf_no_scale_encoder = merge_models_procrustes_weighted_fusion(base_encoder, experts_encoders, fishers=fishers, device=device, weighted=True, lambda_val=lamb, scale_rotation=False)
        merging_strategies[f"Ablation: FWPA-FWF (No Scale Rotation) (λ={lamb})"] = fw_fwf_no_scale_encoder

    # 6. Ablation: OrthoMerge-FWF (No Fisher Weight) with different lambdas
    for lamb in [0.33, 0.4, 0.5]:
        fw_fwf_no_weight_encoder = merge_models_procrustes_weighted_fusion(base_encoder, experts_encoders, fishers=fishers, device=device, weighted=False, lambda_val=lamb, scale_rotation=True)
        merging_strategies[f"Ablation: OrthoMerge-FWF (No Fisher Weight) (λ={lamb})"] = fw_fwf_no_weight_encoder
    
    # Results dictionary
    # format: {strategy: {env: {dataset: accuracy}}}
    results = {}
    ...
    with open("metrics_refinement.json", "w") as f:
        json.dump(results, f, indent=4)
    print("\nResults successfully saved to metrics_refinement.json.")
...
def generate_plot(results, environments):
    ...
    plt.title("Multi-Task Merged Model Accuracy Across Environments", fontsize=14, fontweight='bold')
    plt.xlabel("Environment (Clean & Corrupted)", fontsize=12)
    plt.ylabel("Average Multi-Task Accuracy (%)", fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend(fontsize=10, loc='lower left')
    plt.tight_layout()
    plt.savefig("performance_comparison_refinement.png", dpi=300)
    print("Performance comparison plot saved as performance_comparison_refinement.png.")
