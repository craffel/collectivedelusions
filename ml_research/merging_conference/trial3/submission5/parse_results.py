...
tracker_file = "experiments_tracker.txt"
...
for line in lines:
    ...
    if os.path.exists(out_file):
        ...
        if "Final Results" in content:
            # Parse the line: "Final Results | CIFAR10: XX.XX% | SVHN: XX.XX% | Average: XX.XX%"
            # Or similar final output lines
            final_line = ""
            ...
            # Parse metrics
            # Final Results | CIFAR10: 97.28% | SVHN: 71.62% | Average: 84.45%
            try:
...
print(f"{'Job Name':<25} | {'CIFAR-10 (%)':<12} | {'SVHN (%)':<10} | {'Average (%)':<12} | {'Status':<10}")
print("-" * 73)
for r in sorted(results, key=lambda x: x["name"]):
    print(f"{r['name']:<25} | {r['cifar']:<12} | {r['svhn']:<10} | {r['avg']:<12} | {r['status']:<10}")
