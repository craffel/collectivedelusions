...
tracker_file = "experiments_tracker_blur.txt"
...
print("\n### Blur TTA Experimental Results Summary ###\n")
print(f"{'Job Name':<30} | {'CIFAR-10 (%)':<12} | {'SVHN (%)':<10} | {'Average (%)':<12} | {'Status':<10}")
print("-" * 78)
for r in sorted(results, key=lambda x: x["name"]):
    print(f"{r['name']:<30} | {r['cifar']:<12} | {r['svhn']:<10} | {r['avg']:<12} | {r['status']:<10}")

if incomplete:
    print(f"\nIncomplete/Running jobs ({len(incomplete)}): {', '.join(incomplete)}")
else:
    print("\nAll Blur TTA experiments completed successfully!")
