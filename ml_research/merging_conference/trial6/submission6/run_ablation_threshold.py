...
def run_ablation(threshold_val, use_corruption=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for b_idx, (inputs, labels, task_name) in enumerate(test_batches):
        ...
        is_novel_detected = max_cohesion < threshold_val
                ...
                if loss is not None:
                    ...
                    with torch.no_grad():
                        for i, (name, param) in enumerate(lambda_dict.items()):
                            grad = grads[i]
                            if grad is not None:
                                # Fixed alpha = 1.0 for threshold sweep
...
if __name__ == "__main__":
    ...
    print("=== Running Threshold Sweep over \tau_N (Clean scenario) ===")
    for t in thresholds:
        c_acc, s_acc, f_acc, o_acc, ndr, fpr = run_ablation(t, use_corruption=False)
        print(f"Threshold: {t:.2f} | CIFAR10: {c_acc:.2f}% | SVHN: {s_acc:.2f}% | F-MNIST (Novel): {f_acc:.2f}% | Overall: {o_acc:.2f}% | NDR: {ndr:.2f}% | FPR: {fpr:.2f}%")
        results_clean.append((c_acc, s_acc, f_acc, o_acc, ndr, fpr))
        
    print("\n=== Running Threshold Sweep over \tau_N (Corrupted scenario) ===")
    for t in thresholds:
    ...
    np.savez("ablation_threshold_results.npz", thresholds=thresholds, clean=results_clean, corrupted=results_corrupted)
    print("\nThreshold sweep completed! Saved to ablation_threshold_results.npz")
    ...
    color1 = 'tab:blue'
    axes[0].set_xlabel("Novelty Detection Threshold ($\\tau_N$)", fontsize=12)
    axes[0].set_ylabel("Detection Rate / FPR (%)", color=color1, fontsize=12)
    line1 = axes[0].plot(thresholds, results_clean[:, 4], marker='o', linestyle='-', color='blue', label='Novelty Detection Rate (NDR)')
    line2 = axes[0].plot(thresholds, results_clean[:, 5], marker='x', linestyle='--', color='red', label='False Positive Rate (FPR)')
    ...
    line3 = ax2.plot(thresholds, results_clean[:, 2], marker='s', linestyle='-.', color='darkorange', label='Novel Acc (FashionMNIST)')
    line4 = ax2.plot(thresholds, results_clean[:, 3], marker='^', linestyle=':', color='green', label='Overall Acc')
    ax2.tick_params(axis='y', labelcolor=color2)
    
    lines = line1 + line2 + line3 + line4
    labels = [l.get_label() for l in lines]
    axes[0].legend(lines, labels, loc='lower left')
    axes[0].set_title("Clean Test Stream Sensitivity", fontsize=14, fontweight='bold')
    
    # Right: Corrupted
    axes[1].set_xlabel("Novelty Detection Threshold ($\\tau_N$)", fontsize=12)
    axes[1].set_ylabel("Detection Rate / FPR (%)", color=color1, fontsize=12)
    line1_c = axes[1].plot(thresholds, results_corrupted[:, 4], marker='o', linestyle='-', color='blue', label='Novelty Detection Rate (NDR)')
    line2_c = axes[1].plot(thresholds, results_corrupted[:, 5], marker='x', linestyle='--', color='red', label='False Positive Rate (FPR)')
    axes[1].tick_params(axis='y', labelcolor=color1)
    axes[1].grid(True, linestyle='--', alpha=0.5)
    
    ax2_c = axes[1].twinx()
    ax2_c.set_ylabel("Accuracy (%)", color=color2, fontsize=12)
    line3_c = ax2_c.plot(thresholds, results_corrupted[:, 2], marker='s', linestyle='-.', color='darkorange', label='Novel Acc (FashionMNIST)')
    line4_c = ax2_c.plot(thresholds, results_corrupted[:, 3], marker='^', linestyle=':', color='green', label='Overall Acc')
    ...
    axes[1].set_title("Corrupted Test Stream Sensitivity", fontsize=14, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig("ablation_threshold_plot.png", dpi=300)
    print("Ablation plot saved to ablation_threshold_plot.png")
