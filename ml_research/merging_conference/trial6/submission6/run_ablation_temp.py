...
def run_ablation(temp_val, use_corruption=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    # Evaluation with specific temperature
    ...
    # Fix standard threshold to 0.50
    tau_N = 0.50
    ...
    for b_idx, (inputs, labels, task_name) in enumerate(test_batches):
        ...
        is_novel_detected = max_cohesion < tau_N
        ...
        if not is_novel_detected:
            ...
            for step in range(num_steps):
                ...
                logits = torch.func.functional_call(base_model, merged_params, (inputs, False))
                # Pass specific temperature value here
                loss = compute_contrastive_loss(features_centered, logits, prototypes_dict, tau_p=0.40, temperature=temp_val)
    ...
    return cifar_acc, svhn_acc, fmnist_acc, overall_acc, ndr, fpr
...
if __name__ == "__main__":
    ...
    print("=== Running Temperature Sweep over \\tau_{temp} (Clean scenario) ===")
    for t in temperatures:
        c_acc, s_acc, f_acc, o_acc, ndr, fpr = run_ablation(t, use_corruption=False)
        print(f"Temperature: {t:.2f} | CIFAR10: {c_acc:.2f}% | SVHN: {s_acc:.2f}% | F-MNIST (Novel): {f_acc:.2f}% | Overall: {o_acc:.2f}% | NDR: {ndr:.2f}% | FPR: {fpr:.2f}%")
        results_clean.append((c_acc, s_acc, f_acc, o_acc, ndr, fpr))
        
    print("\n=== Running Temperature Sweep over \\tau_{temp} (Corrupted scenario) ===")
    for t in temperatures:
        c_acc, s_acc, f_acc, o_acc, ndr, fpr = run_ablation(t, use_corruption=True)
        print(f"Temperature: {t:.2f} | CIFAR10: {c_acc:.2f}% | SVHN: {s_acc:.2f}% | F-MNIST (Novel): {f_acc:.2f}% | Overall: {o_acc:.2f}% | NDR: {ndr:.2f}% | FPR: {fpr:.2f}%")
        results_corrupted.append((c_acc, s_acc, f_acc, o_acc, ndr, fpr))
        
    results_clean = np.array(results_clean)
    results_corrupted = np.array(results_corrupted)
    
    np.savez("ablation_temp_results.npz", temperatures=temperatures, clean=results_clean, corrupted=results_corrupted)
    print("\nTemperature sweep completed! Saved to ablation_temp_results.npz")
    
    # Generate Plot
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    
    # Left: Clean (NDR & FPR on left axis, accuracy on right axis)
    color1 = 'tab:blue'
    axes[0].set_xlabel("Contrastive Temperature ($\\tau_{temp}$)", fontsize=12)
    axes[0].set_ylabel("Detection Rate / FPR (%)", color=color1, fontsize=12)
    line1 = axes[0].plot(temperatures, results_clean[:, 4], marker='o', linestyle='-', color='blue', label='Novelty Detection Rate (NDR)')
    line2 = axes[0].plot(temperatures, results_clean[:, 5], marker='x', linestyle='--', color='red', label='False Positive Rate (FPR)')
    ...
    line3 = ax2.plot(temperatures, results_clean[:, 2], marker='s', linestyle='-.', color='darkorange', label='Novel Acc (FashionMNIST)')
    line4 = ax2.plot(temperatures, results_clean[:, 3], marker='^', linestyle=':', color='green', label='Overall Acc')
    ax2.tick_params(axis='y', labelcolor=color2)
    
    lines = line1 + line2 + line3 + line4
    labels = [l.get_label() for l in lines]
    axes[0].legend(lines, labels, loc='lower left')
    axes[0].set_title("Clean Test Stream Temperature Sensitivity", fontsize=14, fontweight='bold')
    
    # Right: Corrupted
    axes[1].set_xlabel("Contrastive Temperature ($\\tau_{temp}$)", fontsize=12)
    axes[1].set_ylabel("Detection Rate / FPR (%)", color=color1, fontsize=12)
    line1_c = axes[1].plot(temperatures, results_corrupted[:, 4], marker='o', linestyle='-', color='blue', label='Novelty Detection Rate (NDR)')
    line2_c = axes[1].plot(temperatures, results_corrupted[:, 5], marker='x', linestyle='--', color='red', label='False Positive Rate (FPR)')
    axes[1].tick_params(axis='y', labelcolor=color1)
    axes[1].grid(True, linestyle='--', alpha=0.5)
    
    ax2_c = axes[1].twinx()
    ax2_c.set_ylabel("Accuracy (%)", color=color2, fontsize=12)
    line3_c = ax2_c.plot(temperatures, results_corrupted[:, 2], marker='s', linestyle='-.', color='darkorange', label='Novel Acc (FashionMNIST)')
    line4_c = ax2_c.plot(temperatures, results_corrupted[:, 3], marker='^', linestyle=':', color='green', label='Overall Acc')
    ax2_c.tick_params(axis='y', labelcolor=color2)
    
    lines_c = line1_c + line2_c + line3_c + line4_c
    labels_c = [l.get_label() for l in lines_c]
    axes[1].legend(lines_c, labels_c, loc='lower left')
    axes[1].set_title("Corrupted Test Stream Temperature Sensitivity", fontsize=14, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig("ablation_temp_plot.png", dpi=300)
    print("Ablation plot saved to ablation_temp_plot.png")
