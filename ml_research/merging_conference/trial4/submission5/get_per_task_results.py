...
def main():
    ...
    print(f"{'Method':<20} | {'MNIST':<10} | {'Fashion':<10} | {'KMNIST':<10} | {'Overall':<10}")
    print("-" * 65)
    
    for method in methods:
        correct, totals = run_experiment(method, stream, experts, original_heads, anchors=anchors)
        ...
        mnist_acc = mnist_correct / mnist_total * 100
        fashion_acc = fashion_correct / fashion_total * 100
        kmnist_acc = kmnist_correct / kmnist_total * 100
        overall_acc = overall_correct / overall_total * 100
        
        print(f"{method:<20} | {mnist_acc:.2f}%    | {fashion_acc:.2f}%    | {kmnist_acc:.2f}%    | {overall_acc:.2f}%")
