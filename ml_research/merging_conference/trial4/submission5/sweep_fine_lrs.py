...
def run_lr_experiment(stream, experts, original_heads, anchors, lr_alpha, lr_head):
    set_seed(2026)
    ...
    for step, (inputs, targets, true_task_id) in enumerate(stream):
        ...
        # Hyperparameters (Optimal CAbA-Merge)
        gamma = 5.0
        distill_weight = 5.0
...
def main():
    ...
    lr_alphas = [0.001, 0.002, 0.003, 0.004]
    lr_heads = [0.0006, 0.0008, 0.001, 0.0012, 0.0014]
    
    best_overall = 0.0
    best_config = None
    best_results = None
    
    print("Starting Fine Learning Rate Sweep...")
    for lr_alpha in lr_alphas:
        for lr_head in lr_heads:
            accs = run_lr_experiment(stream, experts, original_heads, anchors, lr_alpha, lr_head)
            print(f"lr_alpha: {lr_alpha}, lr_head: {lr_head} | MNIST: {accs[0]*100:.2f}% | Fashion: {accs[1]*100:.2f}% | KMNIST: {accs[2]*100:.2f}% | Overall: {accs['overall']*100:.2f}%")
            
            if accs["overall"] > best_overall:
                best_overall = accs["overall"]
                best_config = (lr_alpha, lr_head)
                best_results = accs
                
    print("\n=== FINE LR SWEEP COMPLETE ===")
    print(f"Best Config: lr_alpha={best_config[0]}, lr_head={best_config[1]}")
