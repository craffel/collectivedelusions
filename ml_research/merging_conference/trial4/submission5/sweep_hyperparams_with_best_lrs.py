...
def run_experiment(stream, experts, original_heads, anchors, gamma, distill_weight):
    set_seed(2026)
    ...
    params_to_opt = [
        {"params": [alphas], "lr": 0.002}, # Best lr_alpha
    ]
    for h in heads:
        params_to_opt.append({"params": h.parameters(), "lr": 0.001}) # Best lr_head
...
def main():
    ...
    gammas = [0.0, 1.0, 3.0, 5.0, 10.0, 15.0]
    distills = [0.0, 1.0, 3.0, 5.0, 10.0, 15.0]
    
    best_overall = 0.0
    best_config = None
    best_results = None
    
    print("Starting Hyperparameter Sensitivity Sweep with lr_alpha=0.002, lr_head=0.001...")
    for gamma in gammas:
        for distill in distills:
            accs = run_experiment(stream, experts, original_heads, anchors, gamma, distill)
            print(f"gamma: {gamma}, distill: {distill} | MNIST: {accs[0]*100:.2f}% | Fashion: {accs[1]*100:.2f}% | KMNIST: {accs[2]*100:.2f}% | Overall: {accs['overall']*100:.2f}%")
            
            if accs["overall"] > best_overall:
                best_overall = accs["overall"]
                best_config = (gamma, distill)
                best_results = accs
                
    print("\n=== HYPERPARAMETER SWEEP COMPLETE ===")
    print(f"Best Config: gamma={best_config[0]}, distill={best_config[1]}")
