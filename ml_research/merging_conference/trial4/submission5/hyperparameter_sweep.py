...
def run_caba_sweep(stream, experts, original_heads, anchors, gamma, distill_weight, lr_alpha=0.005, lr_head=0.001):
    set_seed(2026)
...
def main():
    ...
    gammas = [0.0, 1.0, 5.0, 10.0, 15.0, 20.0]
    deltas = [0.0, 1.0, 3.0, 5.0, 10.0]
    ...
    for gamma in gammas:
        results[gamma] = {}
        for delta in deltas:
            acc = run_caba_sweep(stream, experts, original_heads, anchors, gamma, delta)
            results[gamma][delta] = acc
            print(f"gamma={gamma:<4} | delta={delta:<4} | acc={acc*100:.2f}%")
            
    # Print LaTeX Table
    print("\n--- LaTeX Table Output ---")
    print("\\begin{table}[h]")
    print("\\caption{Hyperparameter sensitivity analysis of CAbA-Merge overall accuracy (\\%) under different anchor-alignment weights $\\gamma$ and self-distillation weights $\\delta$ (Seed 2026).}")
    print("\\label{tab:sensitivity}")
    print("\\begin{center}")
    print("\\begin{tabular}{lccccc}")
    print("\\toprule")
    print(" & \\multicolumn{5}{c}{\\textbf{Self-Distillation Weight $\\delta$}} \\\\")
    print("\\cmidrule{2-6}")
    headers = [f"$\\delta = {d:.1f}$" for d in deltas]
    print("\\textbf{Alignment $\\gamma$} & " + " & ".join(headers) + " \\\\")
    print("\\midrule")
    for gamma in gammas:
        row = f"$\\gamma = {gamma:.1f}$"
        for delta in deltas:
            acc_val = results[gamma][delta] * 100
            if gamma == 10.0 and delta == 5.0:
                row += f" & \\textbf{{ {acc_val:.2f}\\% }}"
            else:
                row += f" & {acc_val:.2f}\\%"
        row += " \\\\"
        print(row)
