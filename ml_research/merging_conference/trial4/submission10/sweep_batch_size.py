...
def evaluate_batch_size(method_name, stream, base_encoder, expert_states, original_heads, fisher_priors, device,
                        lr_lambda=0.1, gamma_const=1.0, gamma_fwar=25.0):
    set_seed(42)
...
def main():
    ...
    # We will sweep batch sizes while keeping total samples per task constant (~1600 samples)
    sweep_configs = [
        {"batch_size": 8, "num_batches_per_task": 200},
        {"batch_size": 16, "num_batches_per_task": 100},
        {"batch_size": 32, "num_batches_per_task": 50},
        {"batch_size": 64, "num_batches_per_task": 25},
        {"batch_size": 128, "num_batches_per_task": 13}
    ]
    
    print("==============================================================")
    print("BATCH SIZE STABILITY SWEEP (SEQUENTIAL STREAM)")
    print("==============================================================")
    
    # Clean Stream
    print("\nCorruption: CLEAN")
    print(f"{'Batch Size':<12} | {'FW-CMS (TF)':<15} | {'FW-CMS+FWAR (TF)':<18}")
    print("-" * 55)
    for cfg in sweep_configs:
        bs = cfg["batch_size"]
        nb = cfg["num_batches_per_task"]
        stream_clean = build_test_stream(datasets, stream_type="sequential", batch_size=bs, num_batches_per_task=nb, seed=42)
        
        acc_no_fwar = evaluate_batch_size("fw_cms_tf", stream_clean, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_fwar=0.0)
        acc_with_fwar = evaluate_batch_size("fw_cms_tf_fwar", stream_clean, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_fwar=25.0)
        
        print(f"{bs:<12} | {acc_no_fwar:.2f}%          | {acc_with_fwar:.2f}%")
        
    # Gaussian Noise Stream
    ...
    for cfg in sweep_configs:
        ...
        acc_no_fwar = evaluate_batch_size("fw_cms_tf", stream_noise, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_fwar=0.0)
        acc_with_fwar = evaluate_batch_size("fw_cms_tf_fwar", stream_noise, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_fwar=25.0)
