...
def evaluate_gamma_const(method_name, stream, base_encoder, expert_states, original_heads, fisher_priors, device,
...
def main():
    ...
    for gc in [0.0, 0.1, 0.5, 1.0, 2.0, 5.0]:
        acc = evaluate_gamma_const("fw_cms_tf_fwar", stream_clean, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_const=gc)
        print(f"  gamma_const = {gc} -> Accuracy: {acc:.2f}%")
    ...
    for gc in [0.0, 0.1, 0.5, 1.0, 2.0, 5.0]:
        acc = evaluate_gamma_const("fw_cms_tf_fwar", stream_noise, base_encoder, expert_states, original_heads, fisher_priors, device, gamma_const=gc)
        print(f"  gamma_const = {gc} -> Accuracy: {acc:.2f}%")
