...
def test_regularized_task_specific(alpha, stream_name, corruption):
    set_seed(42)
...
for a in alphas:
    print(f"\nEvaluating with alpha = {a}")
    for corr in ["clean", "noise", "blur", "contrast"]:
        acc = test_regularized_task_specific(a, "sequential", corr)
        print(f"  Corruption: {corr:8s} | Acc: {acc:.2f}%")
