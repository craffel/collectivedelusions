...
def test_task_specific(stream_name, corruption):
    set_seed(42)
    ...
    # Optimizers for each task's logits
    optimizers = {
    ...
    for step, (x, y, task_idx) in enumerate(stream):
        ...
        # 1. Reconstruct merged encoder parameters using task-specific logits
        ...
        # 3. Adaptation step for this task's logits
    ...
    print(f"Stream: {stream_name:12s} | Corruption: {corruption:8s} | Task-Specific Merging Acc: {acc:.2f}%")
    return acc

for stream in ["sequential", "alternating"]:
    for corr in ["clean", "noise", "blur", "contrast"]:
        test_task_specific(stream, corr)
