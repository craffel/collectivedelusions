...
def run_td_atmm_with_lr(alpha, lr_logits, stream_name, corruption, device):
    set_seed(42)
...
def main():
    ...
    print("================================================================================")
    print("LR ABLATION FOR TD-ATMM (alpha = 0.05)")
    print("================================================================================")
    
    for lr in lrs:
        print(f"\nEvaluating learning rate eta_Lambda = {lr}")
        for s_name in ["sequential", "alternating"]:
            accs = []
            for corr in ["clean", "noise", "blur", "contrast"]:
                acc = run_td_atmm_with_lr(alpha, lr, s_name, corr, device)
                accs.append(acc)
            print(f"  Stream: {s_name:12s} | Clean: {accs[0]:.2f}% | Noise: {accs[1]:.2f}% | Blur: {accs[2]:.2f}% | Contrast: {accs[3]:.2f}% | Average: {np.mean(accs):.2f}%")
