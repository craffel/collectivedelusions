...
def test_threshold_l2_task_specific(threshold, alpha_adapt, alpha_freeze, stream_name, corruption):
    set_seed(42)
    ...
    for step, (x, y, task_idx) in enumerate(stream):
        ...
        # Threshold gating
        alpha = alpha_adapt if gamma_k >= threshold else alpha_freeze
        
        # Loss with gated L2 penalty on logits
...
for th in thresholds:
    ...
    for corr in ["clean", "noise", "blur", "contrast"]:
        acc_seq = test_threshold_l2_task_specific(th, 0.01, 10.0, "sequential", corr)
        acc_alt = test_threshold_l2_task_specific(th, 0.01, 10.0, "alternating", corr)
        print(f"  Corruption: {corr:8s} | Seq: {acc_seq:.2f}% | Alt: {acc_alt:.2f}%")
