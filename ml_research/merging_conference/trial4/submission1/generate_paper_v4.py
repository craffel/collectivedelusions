...
\subsection{Ablation Study of the Learning Rate $\eta_{\Lambda}$}
We systematically ablate the learning rate of the task-specific merging coefficients $\eta_{\Lambda} \in \{0.001, 0.005, 0.01, 0.05, 0.1\}$ with a fixed optimal anchoring weight $\alpha = 0.05$ (evaluated on CPU to maintain strict deterministic repeatability). The results are summarized in \cref{table:lr_ablation}.

At very low learning rates ($\eta_{\Lambda} = 0.001$), adaptation is extremely slow, and the performance remains close to the static baseline (e.g., 50.21\% average sequential accuracy vs. 49.86\% STATIC). As the learning rate is scaled up, the model's capacity to adapt on Clean and Blur domains increases dramatically. For instance, at $\eta_{\Lambda} = 0.1$, the Clean accuracy on sequential streams reaches an outstanding \textbf{92.98\%} and Blur accuracy reaches \textbf{70.05\%}, pushing the overall average to \textbf{56.35\%}. 
...
\begin{table}[t]
\caption{\textbf{Ablation of the merging learning rate $\eta_{\Lambda}$ (multi-task average accuracy \%) with fixed anchoring weight $\alpha = 0.05$ (computed on CPU).}}
\label{table:lr_ablation}
\vskip 0.15in
\begin{center}
\begin{small}
\begin{sc}
\begin{tabular}{lcccccr}
\toprule
$\eta_{\Lambda}$ & Stream & Clean & Noise & Blur & Contrast & Average \\
\midrule
0.001 & Seq & 79.26\% & 47.09\% & 57.11\% & \textbf{17.35\%} & 50.21\% \\
      & Alt & 79.27\% & 47.47\% & 57.14\% & \textbf{17.35\%} & 50.31\% \\
\midrule
0.005 & Seq & 83.14\% & 47.28\% & 59.46\% & 17.32\% & 51.80\% \\
      & Alt & 83.15\% & \textbf{47.96\%} & 59.44\% & 17.33\% & 51.97\% \\
\midrule
0.01  & Seq & 86.15\% & 47.65\% & 61.00\% & 17.31\% & 53.03\% \\
      & Alt & 86.16\% & 47.40\% & 61.01\% & 17.33\% & 52.97\% \\
\midrule
0.05  & Seq & 91.89\% & 45.49\% & 64.86\% & 17.26\% & 54.87\% \\
      & Alt & 91.78\% & 44.91\% & 64.81\% & 17.26\% & 54.69\% \\
\midrule
0.1   & Seq & \textbf{92.98\%} & 45.19\% & \textbf{70.05\%} & 17.20\% & \textbf{56.35\%} \\
      & Alt & \textbf{92.96\%} & 44.76\% & \textbf{69.90\%} & 17.22\% & \textbf{56.21\%} \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
