...
def run_evaluation(
    ...
    for step, ((inputs, targets), task_idx) in enumerate(batches):
        ...
        if method == 'static':
            ...
            # Evaluate using detected task Backbone + Head
            with torch.no_grad():
                merged_params = {}
                for param_key in base_backbone_params.keys():
            ...
            lambda_history.append([1.0 if idx == detected_k else 0.0 for idx in range(3)])
            continue

        elif method == 'eg_tvr_adaptive':
            # Entropy-Guided Task Vector Routing with Online Adaptation
            # 1. Detect task to initialize lambda_val
            with torch.no_grad():
                entropies = []
                for k in range(3):
                    temp_lambda = [0.0, 0.0, 0.0]
            ...
            # Initialize lambda to favor detected task
            # Let's initialize to 0.8 for detected, 0.1 for others
            init_lambda = [0.1, 0.1, 0.1]
            init_lambda[detected_k] = 0.8
            lambda_val = torch.tensor(init_lambda, device=device, requires_grad=True)
            
            # Step 1: Compute loss on active head
            # But wait, which head is active? The true task index is unknown at test-time.
            # In a real teacher-free setting, we must use the DETECTED head!
            # Since detection is extremely accurate, we can use the detected classification head.
            ...
            # Final Inference using detected head
            with torch.no_grad():
                merged_params = {}
                for k in base_backbone_params.keys():
                    merged_params[k] = base_backbone_params[k] + sum(
...
def main():
    ...
    print("\n--- EVALUATING ALL METHODS INCLUDING ENTROPY ROUTING ---")
