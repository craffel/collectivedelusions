...
def merge_and_copy_buffers(expert_backbones, dest_backbone, lambda_val):
    with torch.no_grad():
        for name, _ in dest_backbone.named_buffers():
            expert_bufs = []
            ...
            if 'num_batches_tracked' in name:
                ...
                merged_buf = sum(lambda_val[i] * expert_bufs[i] for i in range(len(expert_backbones)))
                
            parts = name.split('.')
            submodule = dest_backbone
            for part in parts[:-1]:
                submodule = getattr(submodule, part)
            dest_buf = getattr(submodule, parts[-1])
            dest_buf.copy_(merged_buf)
...
def run_evaluation(
    ...
    for step, ((inputs, targets), task_idx) in enumerate(batches):
        ...
        if method == 'static':
            ...
            # Evaluates in eval mode with merged BN buffers
            static_lambda = torch.tensor([1/3, 1/3, 1/3], device=device)
            merge_and_copy_buffers(expert_backbones, base_backbone, static_lambda)
            merged_params = {}
            for k in base_backbone_params.keys():
                merged_params[k] = base_backbone_params[k] + sum(
                    static_lambda[i] * task_vectors[i][k] for i in range(3)
            ...
            # Entropy-Guided Task Vector Routing (Zero-Shot, Static, Train mode)
            with torch.no_grad():
                entropies = []
                for k in range(3):
                    merged_params = {}
            ...
            # Entropy-Guided Task Vector Routing (Zero-Shot, Static, Eval mode with BN buffer copying)
            with torch.no_grad():
                entropies = []
                for k in range(3):
                    # Copy exact expert k's BN buffers
                    temp_lambda = [0.0, 0.0, 0.0]
                    temp_lambda[k] = 1.0
                    merge_and_copy_buffers(expert_backbones, base_backbone, temp_lambda)
            ...
            # Copy detected expert's BN buffers for final inference
            temp_lambda = [0.0, 0.0, 0.0]
            ...
            # Entropy-Guided Task Vector Routing with Online Adaptation (Train mode)
            with torch.no_grad():
                entropies = []
                for k in range(3):
                    merged_params = {}
            ...
            # Entropy-Guided Task Vector Routing with Online Adaptation (Eval mode with BN buffer copying)
            with torch.no_grad():
                entropies = []
                for k in range(3):
                    temp_lambda = [0.0, 0.0, 0.0]
            ...
            # Merge BN buffers based on current lambda
            merge_and_copy_buffers(expert_backbones, base_backbone, lambda_val.data)
            ...
            # Merge BN buffers again based on updated lambda
            merge_and_copy_buffers(expert_backbones, base_backbone, lambda_val.data)
...
def main():
    ...
    # Let's sweep a subset of batch sizes to be quick, or all of them if possible.
    # To be extremely fast, we can focus on [1, 2, 4, 8, 32].
    ...
    for bs in batch_sizes:
        ...
        for m in methods:
            # Run sequential and alternating
