...
def evaluate_tta(method_name, experts, base_params, task_vectors, test_loaders, fisher_dict=None, corruption_type='clean', num_cycles=100, ewc_lambda=1000.0, lr_head=1e-4, use_softmax=True):
    ...
    if method_name == 'static_merged':
        ...
        opt_lambdas = optim.Adam([raw_lambdas], lr=0.005)
        opt_heads = {
            name: optim.Adam(head.parameters(), lr=cur_lr_head) for name, head in adapted_heads.items()
        }
