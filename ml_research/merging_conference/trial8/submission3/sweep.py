...
def run_evaluation(multiplier, min_alpha, max_alpha):
    ...
    # Configure custom forward with hyperparams
    for name, m in base_model.named_modules():
        if isinstance(m, AdaKLBatchNorm2D):
            ...
            # Monkey-patch forward with our hyperparams
            def custom_forward(self, x_in):
                if self.use_adakl and self.w is not None:
                    ...
                    # Adaptive alpha
                    alpha = torch.clamp(multiplier * kl_val, min=min_alpha, max=max_alpha).item()
...
def main():
    print("--- SWEEPING ADAPTIVE ALPHA PARAMETERS ---")
    multipliers = [0.005, 0.008, 0.01, 0.012, 0.015]
    max_alphas = [0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.25]
    
    best_noisy_mnist = 0
    best_noisy_fashion = 0
    best_cfg = None
    
    for mult in multipliers:
        for max_a in max_alphas:
            res = run_evaluation(mult, 0.01, max_a)
            print(f"Mult: {mult:.2f}, MaxAlpha: {max_a:.2f} | CleanM: {res[0]:.2f}% | NoisyM: {res[1]:.2f}% | CleanF: {res[2]:.2f}% | NoisyF: {res[3]:.2f}% | KMNIST: {res[4]:.2f}%")
            
            # We want to maximize both Noisy MNIST and Noisy Fashion
            if res[1] >= best_noisy_mnist and res[3] >= best_noisy_fashion:
                best_noisy_mnist = res[1]
                best_noisy_fashion = res[3]
                best_cfg = (mult, max_a, res)
                
    print("\nBest Configuration:")
    print(f"  Multiplier: {best_cfg[0]}")
    print(f"  Max Alpha: {best_cfg[1]}")
    print(f"  Results: CleanM={best_cfg[2][0]:.2f}%, NoisyM={best_cfg[2][1]:.2f}%, CleanF={best_cfg[2][2]:.2f}%, NoisyF={best_cfg[2][3]:.2f}%, KMNIST={best_cfg[2][4]:.2f}%")
