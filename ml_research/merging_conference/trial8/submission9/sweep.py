...
if __name__ == '__main__':
    ...
    print(f"Executing sweep on device: {device}")
    ...
    # We will log the results of each combination
    sweep_log = []
    
    for lr in lrs:
        for steps in steps_list:
            for gamma in gammas:
                res, overall = run_bk_co_merge(
                    batches, model_0, model_1, base_model, device, prototypes_0, prototypes_1,
                    steps=steps, lr=lr, gamma_coherence=gamma, temporal_smoothing=False
                )
                
                # Check temporally-smoothed variant as well
                res_ts, overall_ts = run_bk_co_merge(
                    batches, model_0, model_1, base_model, device, prototypes_0, prototypes_1,
                    steps=steps, lr=lr, gamma_coherence=gamma, temporal_smoothing=True
                )
                
                print(f"BK-CoMerge: {lr},{steps},{gamma},{res['Clean MNIST']:.4f},{res['Noisy MNIST']:.4f},{res['Clean FashionMNIST']:.4f},{res['Noisy FashionMNIST']:.4f},{res['Novel KMNIST']:.4f},{overall:.4f}")
                ...
                sweep_log.append({
                    "method": "BK-CoMerge", "lr": lr, "steps": steps, "gamma": gamma, "res": res, "overall": overall
                })
                sweep_log.append({
                ...
                if overall > best_overall:
                    ...
                    best_config = {"method": "BK-CoMerge", "lr": lr, "steps": steps, "gamma": gamma}
                    best_results = res
    ...
    # Save sweep results to sweep_results.txt
    ...
    print("Sweep results saved to sweep_results.txt!")
