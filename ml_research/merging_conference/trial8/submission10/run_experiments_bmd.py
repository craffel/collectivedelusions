...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for x, _ in mnist_cal_loader:
            ...
            # Compute batch-mean calibration distance
            batch_means = [mnist_feats[i:i+128].mean(dim=0) for i in range(0, 500-128, 50)]
            cal_dist_mnist = torch.mean(torch.stack([torch.norm(bm - proto_mnist) for bm in batch_means])).item()
        ...
        for x, _ in kmnist_cal_loader:
            ...
            cal_dist_kmnist = torch.mean(torch.stack([torch.norm(bm - proto_kmnist) for bm in batch_means])).item()
            
    print(f"MNIST Prototype Norm: {torch.norm(proto_mnist).item():.4f} | Batch-Mean Cal Distance: {cal_dist_mnist:.4f}")
    print(f"KMNIST Prototype Norm: {torch.norm(proto_kmnist).item():.4f} | Batch-Mean Cal Distance: {cal_dist_kmnist:.4f}")
    ...
    # Phase 1: Noisy MNIST (Gaussian Noise std=1.2) - More challenging noise to prove adaptation!
    for _ in range(10):
        x, y = next(mnist_iter)
    ...
    dist_threshold = 2.5  # Adjusted distance threshold for batch-mean space
    temperature = 0.15   
    ...
    for method in methods:
        ...
        for t, (x, y_true, domain) in enumerate(zip(stream_batches, stream_labels, stream_domains)):
            with torch.no_grad():
                ...
                # Compute batch-mean distances in shared space
                dist_mnist = torch.norm(batch_mean_feat - curr_proto_mnist).item()
                dist_kmnist = torch.norm(batch_mean_feat - curr_proto_kmnist).item()
                
                # Normalized distances
                norm_dist_mnist = dist_mnist / curr_cal_dist_mnist
                norm_dist_kmnist = dist_kmnist / curr_cal_dist_kmnist
                
                # Forward pass through experts to get predictions
                ...
                # Calculate expert confidences (mean max probability)
                ...
                # Route predictions (blend outputs)
                ...
                # Active expert and OOD detection
                active_expert = torch.argmax(routing_weights).item()
                min_norm_dist = min(norm_dist_mnist, norm_dist_kmnist)
                
                is_ood = False
                # Flag as OOD if overall entropy is high OR normalized distance is too large
                ...
                if method == 'Ungated EMA-Proto':
                    ...
                    # Gate updates based on both OOD status and expert confidence alignment
    ...
    for method in methods:
        ...
        row['Overall'] = overall_avg
        row_str += f"{overall_avg:.2f}%"
        print(row_str)
        summary_data[method] = row
    print("="*80)
