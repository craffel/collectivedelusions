...
def run_experiment_for_seed(seed, mnist_train, kmnist_train, mnist_test, kmnist_test, fmnist_test):
    ...
    for method in methods:
        print(f"--- Simulating Stream using: {method} ---")
...
if __name__ == "__main__":
    ...
    all_runs = []
    for seed in seeds:
        seed_res = run_experiment_for_seed(seed, mnist_train, kmnist_train, mnist_test, kmnist_test, fmnist_test)
        all_runs.append(seed_res)
        
    # Aggregate results across seeds
    aggregated = {m: {d: [] for d in domains} for m in methods}
    for run in all_runs:
        for m in methods:
            for d in domains:
                # Store the mean accuracy for this seed/domain
                aggregated[m][d].append(np.mean(run[m][d]))
                
    # Compute overall accuracies for each seed and aggregate
    aggregated_overall = {m: [] for m in methods}
    for run in all_runs:
        for m in methods:
            all_accs = []
            for d in domains:
    ...
    summary_data = {}
    print("\n" + "="*110)
    print("ROBUST MULTI-SEED EXPERIMENTAL SUMMARY (Mean ± Std % across 5 seeds)")
    print("="*110)
    print(f"{'Method':<25} | {'MNIST':<12} | {'Noisy M':<12} | {'KMNIST':<12} | {'Noisy K':<12} | {'F-MNIST':<12} | {'Overall':<12}")
    print("-"*110)
    ...
    print("Saved multi-seed results to results.json")
