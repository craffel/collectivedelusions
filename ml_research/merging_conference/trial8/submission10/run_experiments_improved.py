...
if __name__ == "__main__":
    ...
    # Phase 1: Noisy MNIST (Gaussian Noise std=0.6)
    for _ in range(10):
        x, y = next(mnist_iter)
    ...
    # Phase 3: Noisy KMNIST (Gaussian Noise std=0.6)
    for _ in range(10):
        x, y = next(kmnist_iter)
        noisy_x = x + 0.6 * torch.randn_like(x)
        stream_batches.append(noisy_x)
    ...
    alpha = 0.15 # Slightly smaller alpha for more stable EMA transitions
    entropy_threshold = 1.45
    dist_threshold = 1.5  # Threshold on normalized distance
    temperature = 0.15   # Sharp temperature for routing!
    ...
    for method in methods:
        ...
        for t, (x, y_true, domain) in enumerate(zip(stream_batches, stream_labels, stream_domains)):
            with torch.no_grad():
                ...
                # Normalized distances using the unified global distance
                ...
                # Confidence-Calibrated Prototype Update gating
