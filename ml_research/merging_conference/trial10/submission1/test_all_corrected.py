...
def main():
    ...
    print(f"Running test_all_corrected on device: {device}")
    ...
    # Evaluate Method A: Fixed TTA + Reset (L2)
    # (Method A doesn't use offsets, it just resets to prior, then does one step of standard TTA using SGD)
    # Since it does standard TTA via SGD directly on parameters of merged model, gradients flow perfectly!
    ...
    # Evaluate Method B: CL W-Fisher + SCTS (L2) with corrected backprop
        ...
        with torch.no_grad():
            ...
            for name in state_m:
                if "weight" in name or "bias" in name:
                    state_m[name] = (1.0 - lam) * state0[name] + lam * state1[name]
    ...
    # Evaluate Method C: CL W-Fisher + A-SCTS (Angular) with corrected backprop
    ...
    # Evaluate Method D: CP-AM with corrected backprop
    ...
    # Evaluate Method E: Corrected BK-AHR
    ...
    print("\n================ FINAL SYSTEM CORRECTED RESULTS ================\n")
