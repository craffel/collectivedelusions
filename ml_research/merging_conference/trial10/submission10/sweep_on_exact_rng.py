...
# Pretrain standard & CosFace experts EXACTLY as in run_experiments.py (to preserve RNG state)
...
# Train CosFace joint base and experts exactly as in run_experiments.py to match RNG state!
...
# Build Test Stream EXACTLY as in run_experiments.py
test_batches = []
# Clean MNIST (0-9)
for b in range(10):
    imgs, lbls = [], []
...
def eval_config(rho, damping, lr_base):
    overall_accs = {
        "Clean MNIST": [],
        "Noisy MNIST": [],
        "Clean Fashion": [],
...
# Fast sweep over standard expert config with the exact run_experiments.py RNG alignment
rhos = [0.0, 0.005, 0.01, 0.02, 0.05]
dampings = [1e-4, 1e-3, 0.01]
lrs = [0.05, 0.1, 0.2]

configs = list(itertools.product(rhos, dampings, lrs))
print(f"Starting sweep over {len(configs)} configurations on exact RNG alignment...")

results = []
for idx, config in enumerate(configs):
    rho, damping, lr = config
    res = eval_config(rho, damping, lr)
    acc = res["Overall"]
    results.append((rho, damping, lr, acc, res))
    print(f"[{idx+1}/{len(configs)}] Rho: {rho}, Damping: {damping}, LR: {lr} -> Acc: {acc:.4f}%")

results.sort(key=lambda x: x[3], reverse=True)
print("\n--- TOP 5 ---")
for rank, res in enumerate(results[:5]):
    print(f"Rank {rank+1}: Rho={res[0]}, Damping={res[1]}, LR={res[2]} -> Acc={res[3]:.4f}%")
    print(f"  Details: {res[4]}")
