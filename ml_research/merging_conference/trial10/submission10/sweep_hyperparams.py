...
def get_routing_prior(X_t, expert0, expert1, proto0, proto1):
    ...
    routing_type = 0 if h_batch >= 0.50 else 1
...
print("Loading data and pre-training base models for sweep...")
...
# Evaluation Worker
def evaluate_config(params_config):
    rho, damping, lr_base = params_config
    ...
    mean_overall = np.mean(overall_accs) * 100
    return (rho, damping, lr_base, mean_overall)

if __name__ == '__main__':
    # Smart grid search with smaller grid for faster execution
    rhos = [0.02, 0.05, 0.1]
    dampings = [0.05, 0.1, 0.2]
    lrs = [0.005, 0.01, 0.02]
    
    configs = list(itertools.product(rhos, dampings, lrs))
    print(f"Starting sequential sweep over {len(configs)} configurations...")
    
    results = []
    for idx, config in enumerate(configs):
        res = evaluate_config(config)
        results.append(res)
        print(f"[{idx+1}/{len(configs)}] Rho: {res[0]:.3f}, Damping: {res[1]:.3f}, LR Base: {res[2]:.3f} -> Overall Acc: {res[3]:.4f}%")
        
    results.sort(key=lambda x: x[3], reverse=True)
    
    print("\n--- TOP 10 CONFIGURATIONS ---")
    print(f"{'Rank':<5} | {'Rho':<8} | {'Damping':<8} | {'LR Base':<8} | {'Overall Acc':<12}")
    print("-" * 50)
    for rank, res in enumerate(results[:10]):
        print(f"{rank+1:<5} | {res[0]:<8} | {res[1]:<8} | {res[2]:<8} | {res[3]:<12.4f}%")
        
    # Write top configuration to file
    best = results[0]
    with open("best_hyperparams.txt", "w") as f:
        f.write(f"best_rho={best[0]}\nbest_damping={best[1]}\nbest_lr={best[2]}\naccuracy={best[3]}\n")
    print("\nBest configuration saved to best_hyperparams.txt")
