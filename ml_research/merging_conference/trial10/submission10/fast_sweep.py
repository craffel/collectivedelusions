...
def eval_config(rho, damping, lr_base):
    ...
    return np.mean(overall_accs) * 100

if __name__ == "__main__":
    rhos = [0.005, 0.01, 0.015]
    dampings = [0.0003, 0.0005, 0.0007]
    lrs = [0.12, 0.14]
    
    # We will test a smart grid of these values
    configs = list(itertools.product(rhos, dampings, lrs))
    print(f"Starting sequential fast sweep over {len(configs)} configs...")
    
    best_acc = 0.0
    best_conf = None
    for idx, (rho, damping, lr) in enumerate(configs):
        acc = eval_config(rho, damping, lr)
        if acc > best_acc:
            best_acc = acc
            best_conf = (rho, damping, lr)
            print(f"[*] New Best! Rho: {rho}, Damping: {damping}, LR Base: {lr} -> Acc: {acc:.4f}%")
        else:
            if acc >= 68.30:
                print(f"    Rho: {rho}, Damping: {damping}, LR Base: {lr} -> Acc: {acc:.4f}%")
                
    print(f"\nSweep Finished! Best config: Rho={best_conf[0]}, Damping={best_conf[1]}, LR={best_conf[2]} with Acc={best_acc:.4f}%")
