...
def train_model(model, dataloader, epochs, lr, weight_decay):
...
def merge_parameters(expert0_sd, expert1_sd, w_global, delta):
    merged_params = {}
    groups = {
...
def merge_buffers(expert0_sd, expert1_sd, w_global):
    ...
    for mean_name, var_name in bn_names:
        mu0 = expert0_sd[mean_name]
        var0 = expert0_sd[var_name]
        mu1 = expert1_sd[mean_name]
        var1 = expert1_sd[var_name]
        ...
        merged_buffers[track_name] = expert0_sd[track_name]
...
# Get Routing Prior
def get_routing_prior(X_t, expert0, expert1, proto0, proto1):
    X_pos = (X_t + 1.0) / 2.0
...
def estimate_on_the_fly_sensitivity(expert0_sd, expert1_sd, w_global_init, inputs, skeleton_model):
    delta_zero = torch.zeros(4)
    params = merge_parameters(expert0_sd, expert1_sd, w_global_init, delta_zero)
    buffers = merge_buffers(expert0_sd, expert1_sd, w_global_init)
...
# Extract state dicts to avoid pickling/sharing models directly
expert0_sd = {k: v.cpu() for k, v in expert0_std.state_dict().items()}
expert1_sd = {k: v.cpu() for k, v in expert1_std.state_dict().items()}
...
def eval_single_config(args):
    rho, damping, lr_base = args
    overall_accs = []
    
    # We create expert instances locally per process to ensure isolation
    expert0 = SimpleCNN(use_cosface=False)
    expert0.load_state_dict(expert0_sd)
    expert1 = SimpleCNN(use_cosface=False)
    expert1.load_state_dict(expert1_sd)
    ...
    for b_idx, (inputs, targets, seg_name) in enumerate(test_batches):
        ...
        F_sens = estimate_on_the_fly_sensitivity(expert0_sd, expert1_sd, w_global, inputs, skeleton)
        ...
        for step in range(5):
            ...
            p_params = merge_parameters(expert0_sd, expert1_sd, w_global_var, delta_var)
            buffers = merge_buffers(expert0_sd, expert1_sd, w_global_var)
            ...
            if rho > 0:
                ...
                params_p = merge_parameters(expert0_sd, expert1_sd, perturbed_w, perturbed_delta)
                buffers_p = merge_buffers(expert0_sd, expert1_sd, perturbed_w)
                ...
                with torch.no_grad():
                    w_global -= eta_t * g_w
                    delta -= eta_t * (g_delta / (F_sens + damping))
                
        with torch.no_grad():
            final_p = merge_parameters(expert0_sd, expert1_sd, w_global, delta)
            final_b = merge_buffers(expert0_sd, expert1_sd, w_global)
    ...
    return rho, damping, lr_base, np.mean(overall_accs) * 100

if __name__ == "__main__":
    rhos = [0.0, 0.02, 0.05]
    dampings = [1e-4, 1e-3, 1e-2, 0.1]
    lrs = [0.01, 0.02, 0.05, 0.1]

    configs = list(itertools.product(rhos, dampings, lrs))
    print(f"Starting parallel sweep over {len(configs)} configurations on 4 workers...")
    
    with Pool(processes=4) as pool:
        results = pool.map(eval_single_config, configs)
        
    results.sort(key=lambda x: x[3], reverse=True)
    print("\n--- TOP 10 CONFIGURATIONS ---")
    out_lines = []
    for rank, res in enumerate(results[:10]):
         line = f"Rank {rank+1}: Rho={res[0]}, Damping={res[1]}, LR={res[2]} -> Acc={res[3]:.4f}%"
         print(line)
         out_lines.append(line + "\n")
         
    with open("parallel_top.txt", "w") as f:
