...
def eval_with_eval_mode(use_eval_mode=False):
    overall_accs = {
        "Clean MNIST": [],
        "Noisy MNIST": [],
        "Clean Fashion": [],
    ...
    for b_idx, (inputs, targets, seg_name) in enumerate(test_batches):
        ...
        # Optionally set skeleton to eval() mode (disables dropout!)
        if use_eval_mode:
            skeleton.eval()
...
print("Without skeleton.eval() (default):")
print(eval_with_eval_mode(use_eval_mode=False))

print("\nWith skeleton.eval() (Disabling Dropout during Test-Time Adaptation):")
print(eval_with_eval_mode(use_eval_mode=True))
