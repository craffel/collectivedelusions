...
def main():
    ...
    # Let's test batches from each domain
    def get_batches(dataset, num=3):
    ...
    mnist_batches = get_batches(mnist_test)
    kmnist_batches = get_batches(kmnist_test)
    fmnist_batches = get_batches(fmnist_test)
    ...
    def test_domain(name, batches):
        ...
        for idx, (inputs, targets) in enumerate(batches):
            ...
            for i in range(inputs.size(0)):
                ...
                c1[i] = sim1.max()
                c2[i] = sim2.max()
            print(f"Batch {idx+1}: cohesion1={c1.mean().item():.4f} cohesion2={c2.mean().item():.4f}")
            
    test_domain("MNIST", mnist_batches)
    test_domain("KMNIST", kmnist_batches)
    test_domain("FashionMNIST", fmnist_batches)
