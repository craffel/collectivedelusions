...
def adapt_and_evaluate(mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, method="fixed", beta=0.5, tau_base=50.0, use_init=True):
    ...
    for batch_idx, (data, target, task_label, noise_flag) in enumerate(stream_batches):
        ...
        # Initialize w_param based on w_prior or fixed 0.0
        if use_init:
            p = w_prior[0].item()
...
if __name__ == "__main__":
    ...
    print("Evaluating Adaptation with prior initialization...")
    ...
    print("\n" + "="*95)
    print(f"{'SEGMENT':<25} | {'FIXED INIT ACC':<15} | {'CPR-DTS INIT ACC':<15} | {'FIXED AVG LAMBDA':<16} | {'CPR AVG LAMBDA':<16}")
    print("="*95)
    
    for seg_name, (start, end) in segments.items():
        print(f"{seg_name:<25} | {np.mean(fixed_accs[start:end]):.2f}%         | {np.mean(cpr_accs[start:end]):.2f}%         | {np.mean(fixed_lambdas[start:end]):.4f}           | {np.mean(cpr_lambdas[start:end]):.4f}")
    print("="*95)
