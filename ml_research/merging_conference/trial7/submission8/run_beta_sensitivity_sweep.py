...
def run_simulation_beta(mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, norm_joint_fisher, beta):
    # This runs CLW-Fisher + SCTS config specifically, sweeping beta
    accuracies = []
    lambdas_log = []
...
if __name__ == "__main__":
    ...
    beta_values = [0.0, 0.5, 1.0, 1.5, 3.0, 5.0, 10.0]
    results = {}
    ...
    for beta in beta_values:
        ...
        accs, lambdas = run_simulation_beta(
            mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, norm_joint_fisher, beta
        )
        results[beta] = {"accs": accs, "lambdas": lambdas}
        
    # Save results to a file
    with open("beta_sensitivity_results.txt", "w") as f:
        f.write("Beta Regularization Sensitivity Results Table:\n")
        f.write("Beta," + ",".join(segments.keys()) + "\n")
        for beta in beta_values:
            acc_strs = [f"{np.mean(results[beta]['accs'][start:end]):.2f}" for seg_name, (start, end) in segments.items()]
            f.write(f"{beta:.1f}," + ",".join(acc_strs) + "\n")
            
        f.write("\nAverage Lambda Table:\n")
        f.write("Beta," + ",".join(segments.keys()) + "\n")
        for beta in beta_values:
            l_strs = [f"{np.mean(results[beta]['lambdas'][start:end]):.4f}" for seg_name, (start, end) in segments.items()]
            f.write(f"{beta:.1f}," + ",".join(l_strs) + "\n")
            
    print("\nSweep Complete! Results saved to beta_sensitivity_results.txt")
    print("\n" + "="*120)
    headers = [f"{'BETA':<10}"] + [f"{seg_name:<20}" for seg_name in segments.keys()]
    print(" | ".join(headers))
    print("="*120)
    for beta in beta_values:
        row_str = f"{beta:<10.1f}"
        for seg_name, (start, end) in segments.items():
            avg_acc = np.mean(results[beta]["accs"][start:end])
