...
if __name__ == "__main__":
    ...
    # 1. Fixed Temp, Reset-to-0.5
    f_res_acc, f_res_lam = adapt_and_evaluate(
        mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, method="fixed", beta=1.5, tau_base=1200.0, use_init=False
    )
    
    # 2. Fixed Temp, Prior-based Init
    f_ini_acc, f_ini_lam = adapt_and_evaluate(
    ...
    # 3. CPR-DTS, Reset-to-0.5
    c_res_acc, c_res_lam = adapt_and_evaluate(
        mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, method="cpr-dts", beta=1.5, tau_base=1200.0, use_init=False
    )
    
    # 4. CPR-DTS, Prior-based Init (Full Proposed Method)
    c_ini_acc, c_ini_lam = adapt_and_evaluate(
    ...
    print("\n" + "="*110)
    print(f"{'SEGMENT':<25} | {'FIXED+RESET':<12} | {'FIXED+INIT':<12} | {'CPR-DTS+RESET':<15} | {'CPR-DTS+INIT':<15}")
    print("="*110)
    for seg_name, (start, end) in segments.items():
        print(f"{seg_name:<25} | {np.mean(f_res_acc[start:end]):.2f}%       | {np.mean(f_ini_acc[start:end]):.2f}%       | {np.mean(c_res_acc[start:end]):.2f}%         | {np.mean(c_ini_acc[start:end]):.2f}%")
    print("="*110)
    
    # Save the ablation results to ablation_results.txt
    with open("ablation_results.txt", "w") as f:
        f.write("Ablation Study Results:\n")
        f.write(f"{'Segment':<25}, {'Fixed+Reset':<12}, {'Fixed+Init':<12}, {'CPR-DTS+Reset':<15}, {'CPR-DTS+Init':<15}\n")
        for seg_name, (start, end) in segments.items():
            f.write(f"{seg_name:<25}, {np.mean(f_res_acc[start:end]):.2f}%, {np.mean(f_ini_acc[start:end]):.2f}%, {np.mean(c_res_acc[start:end]):.2f}%, {np.mean(c_ini_acc[start:end]):.2f}%\n")
