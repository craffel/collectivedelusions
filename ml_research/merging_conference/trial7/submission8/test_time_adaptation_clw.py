...
def run_simulation(mnist_expert, fashion_expert, prototypes_mnist, prototypes_fashion, stream_batches, norm_joint_fisher, method_config):
    ...
    for batch_idx, (data, target, task_label, noise_flag) in enumerate(stream_batches):
        ...
        if method == "fixed":
            ...
            d_max = max(d_mnist, d_fashion)
            gap = d_max - d_min
            if running_gap is None:
                running_gap = gap
            else:
                gamma = 0.8
                running_gap = gamma * running_gap + (1.0 - gamma) * gap
            tau = running_gap / 3.0 + 150.0
            tau = max(10.0, tau)
...
if __name__ == "__main__":
    ...
    methods = {
        ...
        "DLW-Fisher": {"method": "cpr-dts", "use_init": True, "layer_wise": True, "use_fisher": True},
        "CLW-Fisher (Ours)": {"method": "cpr-dts", "use_init": True, "layer_wise": True, "use_fisher": True, "use_clw": True},
        "CLW-Fisher + SCTS (Ours)": {"method": "scts", "use_init": True, "layer_wise": True, "use_fisher": True, "use_clw": True},
        "CLW-Fisher + TS-SCTS (Ours)": {"method": "ts-scts", "use_init": True, "layer_wise": True, "use_fisher": True, "use_clw": True}
    }
    ...
    print("\n" + "="*160)
    headers = [f"{'SEGMENT':<25}"] + [f"{name:<18}" for name in methods.keys()]
    print(" | ".join(headers))
    print("="*160)
    ...
    for seg_name, (start, end) in segments.items():
        ...
        for name in methods.keys():
            ...
            col_width = max(18, len(name))
            row_str += f" | {avg_acc:.2f}%" + " " * (col_width - 7)
        print(row_str)
    print("="*160)
    ...
    for seg_name, (start, end) in segments.items():
        ...
        for name in methods.keys():
            ...
            col_width = max(18, len(name))
            row_str += f" | {avg_l:.4f}" + " " * (col_width - 7)
        print(row_str)
    print("="*160)
    
    # Save results to a file
    with open("results_clw.txt", "w") as f:
