...
def run_evaluation(stream, stream_domains, method_name, base_lr, gamma_fisher=0.1, alpha_damping=1.0, device='cpu', expert_list=None, s_fisher=None, expert_params=None, expert_buffers=None, base_model=None, parameter_names=None, buffer_names=None, base_buffers_dict=None):
    K = len(expert_list)
...
def main():
    ...
    # Let's sweep learning rates
    lrs = [0.001, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0]
    
    print("\n--- Sweeping Clean Sequential Stream ---")
    for lr in lrs:
        acc_dr = run_evaluation(seq_stream_clean, seq_domains, 'DR-Fisher', lr, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        acc_dtt = run_evaluation(seq_stream_clean, seq_domains, 'D-TT-Fisher', lr, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        print(f"LR: {lr:<5} | DR-Fisher Acc: {acc_dr:.4f}% | D-TT-Fisher Acc: {acc_dtt:.4f}%")

    print("\n--- Sweeping Noise Sequential Stream ---")
    for lr in lrs:
        acc_dr = run_evaluation(seq_stream_noise, seq_domains, 'DR-Fisher', lr, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        acc_dtt = run_evaluation(seq_stream_noise, seq_domains, 'D-TT-Fisher', lr, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        print(f"LR: {lr:<5} | DR-Fisher Acc: {acc_dr:.4f}% | D-TT-Fisher Acc: {acc_dtt:.4f}%")
