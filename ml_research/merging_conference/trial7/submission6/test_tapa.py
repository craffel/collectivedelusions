...
def run_evaluation_tapa(stream, stream_domains, method_name, base_lr, gamma_fisher=0.1, alpha_damping=1.0, use_tapa=True, device='cpu', expert_list=None, s_fisher=None, expert_params=None, expert_buffers=None, base_model=None, parameter_names=None, buffer_names=None, base_buffers_dict=None):
    K = len(expert_list)
    ...
    for t, (batch_X, batch_y) in enumerate(stream):
        ...
        # Task-Aware Persistent Adaptation (TAPA)
        Lambda_prior = torch.tensor([0.005, 0.005, 0.005], device=device)
        Lambda_prior[k_star] = 0.99

        if use_tapa:
            if t == 0 or k_star != previous_k_star:
                # Reset to prior on task transition
                with torch.no_grad():
                    for name in parameter_names:
                        coefficients[name].copy_(Lambda_prior)
            else:
                # Retain previous batch's updated coefficients
                pass
        else:
            # Standard DR-Fisher: reset on every batch
            with torch.no_grad():
                for name in parameter_names:
                    coefficients[name].copy_(Lambda_prior)

        previous_k_star = k_star
...
def main():
    ...
    # Sweep learning rates with and without TAPA
    lrs = [0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0]
    
    print("\n=== Without TAPA (Standard Resetting, Clean Stream) ===")
    for lr in [0.1, 0.5, 1.0]:
        acc_dr = run_evaluation_tapa(seq_stream_clean, seq_domains, 'DR-Fisher', lr, use_tapa=False, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        acc_dtt = run_evaluation_tapa(seq_stream_clean, seq_domains, 'D-TT-Fisher', lr, use_tapa=False, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        print(f"LR: {lr:<5} | DR-Fisher Acc: {acc_dr:.4f}% | D-TT-Fisher Acc: {acc_dtt:.4f}%")

    print("\n=== With TAPA (Task-Aware Persistent Adaptation, Clean Stream) ===")
    for lr in lrs:
        acc_dr = run_evaluation_tapa(seq_stream_clean, seq_domains, 'DR-Fisher', lr, use_tapa=True, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        acc_dtt = run_evaluation_tapa(seq_stream_clean, seq_domains, 'D-TT-Fisher', lr, use_tapa=True, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        print(f"LR: {lr:<5} | DR-Fisher Acc: {acc_dr:.4f}% | D-TT-Fisher Acc: {acc_dtt:.4f}%")

    print("\n=== With TAPA (Task-Aware Persistent Adaptation, Noise Stream) ===")
    for lr in lrs:
        acc_dr = run_evaluation_tapa(seq_stream_noise, seq_domains, 'DR-Fisher', lr, use_tapa=True, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        acc_dtt = run_evaluation_tapa(seq_stream_noise, seq_domains, 'D-TT-Fisher', lr, use_tapa=True, device=device, expert_list=expert_list, s_fisher=s_fisher, expert_params=expert_params, expert_buffers=expert_buffers, base_model=base_model, parameter_names=parameter_names, buffer_names=buffer_names, base_buffers_dict=base_buffers_dict)
        print(f"LR: {lr:<5} | DR-Fisher Acc: {acc_dr:.4f}% | D-TT-Fisher Acc: {acc_dtt:.4f}%")
