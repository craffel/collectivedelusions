...
def main():
    ...
    print("Starting stream evaluation WITH gradient update but proper BN synchronization...")
    for t, (batch_X, batch_y) in enumerate(seq_stream_clean):
        ...
        # BEFORE the forward pass: sync BN buffers with the average coefficients
        # Here, the average coefficients is Lambda_prior (which is coeff_avg at this point)
        # with torch.no_grad():
        ...
        if t == 0:
            norms = []
            for name in parameter_names:
                g = coefficients[name].grad
                if g is not None:
                    norms.append(g.norm().item())
            print(f"Gradient norms on batch 0: mean={np.mean(norms):.6f}, max={np.max(norms):.6f}")

        # Update coefficients with flat LR 1e-3
        with torch.no_grad():
            for name in parameter_names:
                grad = coefficients[name].grad
                if grad is not None:
                    updated = coefficients[name] - 1e-3 * grad
