...
def main():
    ...
    # We evaluate on a 1000-image subset to make the sweep 10x faster
    mnist_eval = Subset(mnist_test, list(range(1000)))
    fashion_eval = Subset(fashion_test, list(range(1000)))
    kmnist_eval = Subset(kmnist_test, list(range(1000)))
    
    test_loaders = {
        "mnist": DataLoader(mnist_eval, batch_size=256, shuffle=False, num_workers=4),
        "fashionmnist": DataLoader(fashion_eval, batch_size=256, shuffle=False, num_workers=4),
        "kmnist": DataLoader(kmnist_eval, batch_size=256, shuffle=False, num_workers=4)
    }
