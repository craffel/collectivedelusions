"""Focused Breadcrumbs sweep on ViT-L/14 — 4 configs near ViT-B/32 optimum."""
import argparse
import json
import os
import sys
...
from merging import breadcrumbs
...
def main():
    ...
    ap.add_argument("--out", default="/fsx/craffel/collectivedelusions/ml_research/testclaude/results/breadcrumbs_vitl14.json")
    ...
    # Focused 4-config sweep around ViT-B/32 optimum (ds=0.85, dl=0.005, alpha=0.5).
    # ViT-L/14 has larger d_in; optimal trim is typically less aggressive.
    sweep = [
        (0.85, 0.005, 0.3),
        (0.85, 0.005, 0.5),
        (0.80, 0.005, 0.5),
    ...
    best = None
    for i, (ds, dl, alpha) in enumerate(sweep):
        t0 = time.time()
        merged = breadcrumbs(pre_gpu, tsds_gpu, drop_small=ds, drop_large=dl, alpha=alpha)
        merged = {k: v.cpu() for k, v in merged.items()}
        per, avg = evaluate_sd(merged)
        name = f"bc_s{ds}_l{dl}_a{alpha}"
        out["configs"][name] = {"drop_small": ds, "drop_large": dl, "alpha": alpha, "per_task": per, "avg": avg}
        print(f"[{i+1}/{len(sweep)}] {name}: {avg*100:.2f}  ({time.time()-t0:.1f}s)", flush=True)
        if best is None or avg > best[1]:
            best = (name, avg, per, ds, dl, alpha)

    out["best"] = {"name": best[0], "avg": best[1], "per_task": best[2],
                   "drop_small": best[3], "drop_large": best[4], "alpha": best[5]}
