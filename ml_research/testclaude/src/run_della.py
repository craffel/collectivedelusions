"""Sweep DELLA baseline on the 8-task benchmark (preloaded loaders, GPU merge)."""
import argparse
import json
import os
import sys
...
def main():
    ...
    ap.add_argument("--out", default="/fsx/craffel/collectivedelusions/ml_research/testclaude/results/della.json")
    ...
    # 3x2x3 = 18 configs
    sweep = []
    for p_low in [0.05, 0.1, 0.2]:
        for p_high in [0.8, 0.9]:
            for alpha in [0.2, 0.3, 0.5]:
                sweep.append((p_low, p_high, alpha))
